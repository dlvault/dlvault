...
interface PluginManifest {
  ...
  apiVersion?: number;         // plugin API contract version (default: 1)
...
`apiVersion` declares which plugin contract your bundle was built against.
The host exports `PLUGIN_API_VERSION` (currently `1`) and refuses to load any
plugin whose `apiVersion` is greater than what the host supports — with a
clear "plugin too new, upgrade dlvault" diagnostic instead of a cryptic
factory crash.

The host bumps `PLUGIN_API_VERSION` only on **non-additive** changes
(permission removed, context member renamed, SourcePlugin signature
changed). Additive changes (new optional context member, new opt-in
permission) keep the same version. If you omit `apiVersion`, the host
treats your plugin as v1 — every published plugin keeps loading.

### Permissions

Permissions come in two flavours. **Capability** permissions (`secrets`,
`browser`) gate a member of `PluginContext`: if you don't declare it, the
corresponding member is `undefined` and accessing it throws. **Disclosure**
permissions (`filesystem`) are purely informational — the host doesn't
sandbox plugins today, so you can `require('fs')` either way, but declaring
the flag tells the user (and reviewers) what your plugin will touch before
they accept it. All declared permissions appear in the install dialog.

| Permission   | Flavour    | What it does                                                                | When to request                          |
|--------------|------------|-----------------------------------------------------------------------------|------------------------------------------|
| `browser`    | capability | Unlocks `context.browser.launch()` — headless Firefox via playwright-firefox | Source page requires JavaScript to render |
| `secrets`    | capability | Unlocks `context.secrets.get(key)` — user-configured shared API keys / tokens | Plugin needs an external service credential (an API key, OAuth token, etc.) |
| `filesystem` | disclosure | No runtime gate — declare it if your plugin reads/writes outside its bundle (e.g. `os.tmpdir()`) so the user is warned in the install dialog | Bundled-worker pattern (see § 7.2) or any other local file I/O |
