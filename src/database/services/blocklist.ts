...
/**
 * How many releases dlvault itself has already given up on for a title.
 *
 * Counts only automatic entries — a manual block is the user's judgement about
 * one release, not evidence that the source is exhausted.
 */
export function countAutoBlocklisted(movieId: number): number {
  const row = db.prepare(
    "SELECT COUNT(*) AS n FROM blocklist WHERE movie_id = ? AND reason LIKE 'auto:%'"
  ).get(movieId) as { n: number };
  return row?.n ?? 0;
}
