...
export function updateMovieStatus(id: number, status: Movie['status'], sourceUrl?: string): void {
  ...
  const dl = status === 'downloaded'
    ...
    : '';
  if (sourceUrl) {
    db.prepare(`UPDATE movies SET status = ?, source_url = ?, updated_at = datetime('now')${dl} WHERE id = ?`)
      .run(status, sourceUrl, id);
  } else {
    db.prepare(`UPDATE movies SET status = ?, updated_at = datetime('now')${dl} WHERE id = ?`)
      .run(status, id);
  }
}
