...
export interface Movie {
  ...
  /**
   * Which watchlist this entry came from. NULL for rows predating the column.
   *
   * Load-bearing for the Trakt deletion sweep: it removes entries that vanished
   * from the Trakt watchlist, and must never touch an entry Plex owns. Before
   * this existed, Plex rows carried a fake `trakt_id` (their TMDb id) and were
   * deleted and re-added on every 2-minute cycle in `both` mode.
   */
  watchlist_source: WatchlistSource | null;
  /** Only shows carry this; it is the sole id a Sonarr-shaped request supplies. */
  tvdb_id: number | null;
  /**
   * 1 when the tracked seasons were chosen explicitly (a Seerr request) rather
   * than implied by watchlisting the show. Suppresses season auto-discovery.
   */
  seasons_explicit: number | null;
}

/** Where a watchlist entry originated. */
export type WatchlistSource = 'trakt' | 'plex' | 'seerr' | 'manual';
 ...
 * Look up by TMDb id.
 *
 * `mediaType` is not optional decoration: TMDb keeps SEPARATE id spaces for
 * movies and TV, both starting at 1, so tmdb 1396 is a legitimate movie *and* a
 * legitimate show. Matching on the number alone made `manual-add` for a show
 * return an unrelated film's row ("already added") and let a library import flip
 * the wrong title to 'downloaded'. Callers that genuinely don't know the type
 * can pass undefined, but should be rare.
 */
export function getMovieByTmdbId(tmdbId: number, mediaType?: MediaType): Movie | undefined {
  if (mediaType) {
    return db.prepare('SELECT * FROM movies WHERE tmdb_id = ? AND media_type = ?')
      .get(tmdbId, mediaType) as Movie | undefined;
  }
  return db.prepare('SELECT * FROM movies WHERE tmdb_id = ?').get(tmdbId) as Movie | undefined;
}

export function getMovieByTvdbId(tvdbId: number): Movie | undefined {
  return db.prepare('SELECT * FROM movies WHERE tvdb_id = ?').get(tvdbId) as Movie | undefined;
}

export function setSeasonsExplicit(id: number, explicit: boolean): void {
  db.prepare('UPDATE movies SET seasons_explicit = ? WHERE id = ?').run(explicit ? 1 : 0, id);
}

export function setMovieTmdbId(id: number, tmdbId: number): void {
  db.prepare('UPDATE movies SET tmdb_id = ? WHERE id = ?').run(tmdbId, id);
}

export function setMovieTvdbId(id: number, tvdbId: number): void {
  db.prepare('UPDATE movies SET tvdb_id = ? WHERE id = ?').run(tvdbId, id);
}
...
type NewMovie = Omit<Movie,
  ...
  | 'repair' | 'quality_override' | 'watchlist_source' | 'tvdb_id' | 'seasons_explicit'>
  & { watchlist_source?: WatchlistSource | null; tvdb_id?: number | null };
...
export function addMovie(movie: NewMovie): Movie {
  ...
  const stmt = db.prepare(`
    INSERT OR IGNORE INTO movies (trakt_id, imdb_id, tmdb_id, tvdb_id, title, year, slug, media_type, status, desired_quality, watchlist_source)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const result = stmt.run(
    traktId, movie.imdb_id, movie.tmdb_id, movie.tvdb_id ?? null,
    movie.title, movie.year, movie.slug, mediaType,
    movie.status, movie.desired_quality, movie.watchlist_source ?? null
  );

  // If INSERT OR IGNORE was ignored (duplicate), lastInsertRowid is unreliable
  if (result.changes === 0) {
    // Fetch by unique keys instead. The TMDb lookup is media-type scoped for the
    // same reason getMovieByTmdbId is: a show and a film can share the number.
    const existing = (traktId ? getMovieByTraktId(traktId) : undefined)
      || (movie.imdb_id ? getMovieByImdbId(movie.imdb_id) : undefined)
      || (movie.tmdb_id ? getMovieByTmdbId(movie.tmdb_id, mediaType) : undefined);
    return existing!;
  }
 ...
 * a collision means it's a duplicate we must not silently merge. Reports
 * whether the id was written so the caller can name the collision.
 */
export function setMovieImdbId(id: number, imdbId: string): boolean {
...
/**
 * Complete the year of a row added without one. Seerr's Sonarr payload carries
 * no year, so a show handed over that way read "(0)" in every log line and
 * view until something looked it up.
 */
export function setMovieYear(id: number, year: number): void {
  db.prepare("UPDATE movies SET year = ?, updated_at = datetime('now') WHERE id = ?")
    .run(year, id);
}
