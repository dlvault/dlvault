...
import type { NotFoundReason } from './movies';
...
export interface Season {
  ...
  /** Why this season is 'not_found' (only meaningful while status === 'not_found'). */
...
export function updateSeasonStatus(
  ...
  // Carry not_found_reason only while not_found, clear it otherwise (mirrors
  // updateMovieStatus) so re-found seasons never keep a stale reason.
  const reason = status === 'not_found' ? (notFoundReason ?? null) : null;
  if (sourceUrl) {
    db.prepare("UPDATE seasons SET status = ?, source_url = ?, not_found_reason = ?, updated_at = datetime('now') WHERE id = ?")
      .run(status, sourceUrl, reason, id);
  } else {
    db.prepare("UPDATE seasons SET status = ?, not_found_reason = ?, updated_at = datetime('now') WHERE id = ?")
