...
// Low-quality video SOURCES — a camcorder/screener rip is unwatchable no matter
// its resolution (a "1080p TELESYNC" is a phone-in-a-cinema recording upscaled).
// The source sites themselves tag these as a low tier ("rip") and bury them under
// a warning banner. We reject them by default on ALL plugins; only a per-title
// 'any' override lets them through (the user explicitly wants *something*).
//
// Matched as dot/space/dash/bracket-delimited scene tokens so we don't false-match
// inside words or codecs — e.g. "DTS" never trips "ts" (the char before is "D",
// not a delimiter), "CAMELOT" never trips "cam".
const LOW_QUALITY_SOURCE_RE =
  /(?:^|[.\s_\-\[(])(?:hdcam|camrip|hqcam|cam|telesync|telecine|hdts|hdtc|dvdscr|bdscr|screener|scr|workprint|r5|r6|ts|tc)(?:[.\s_\-\])]|$)/i;

export function isLowQualitySource(title: string): boolean {
  return LOW_QUALITY_SOURCE_RE.test(title || '');
}

// Subtitle-only releases: the title names the wanted language, but only as
// SUBTITLES — the audio stays the original. A "…German.Subbed…" release is an
// English film with German subs, and a plugin that decides the language from a
// "German" substring reports it as german. It then passes every filter, lands
// in the library, and the first person to press play hears English.
//
// The dub tokens have to be checked too, because a genuinely dubbed release may
// still advertise subtitles ("German.DL.EngSubs" = German audio + English subs).
// A dub token anywhere means the audio is there, so the sub token is harmless.
//
// "WEB-DL" is stripped first: its DL means Download, not Dual Language, and
// leaving it in would make every WEB-DL release look dubbed.
const SUBTITLE_ONLY_RE =
  /(?:^|[.\s_\-\[(])(?:(?:multi|ger|german|deutsch|eng|english|fr|french|it|italian|es|spanish)[.\s_\-]?subs?(?:bed)?|subbed|omu)(?:[.\s_\-\])]|$)/i;
const DUB_TOKEN_RE =
  /(?:^|[.\s_\-\[(])(?:dl|ml|dubbed|dubbing|synchro|md|ld|ac3d|eac3d|dtsd|dd51d)(?:[.\s_\-\])]|$)/i;
const SOURCE_DL_RE = /(?:web|bd|hd|dvd|uhd)[.\s_\-]?dl/ig;

/**
 * True when a release carries the wanted language as subtitles only, i.e. it
 * ships no dubbed audio track.
 */
export function isSubtitleOnlyRelease(title: string): boolean {
  const t = title || '';
  if (!SUBTITLE_ONLY_RE.test(t)) return false;
  return !DUB_TOKEN_RE.test(t.replace(SOURCE_DL_RE, ' '));
}
...
export function buildFilterContext(mediaType: 'movie' | 'show' = 'movie', override: QualityOverride | null = null): FilterContext {
  ...
  // Only the explicit 'any' override accepts a cam/telesync/screener source —
  // 'relaxed' lowers the resolution/audio bar but a cam rip is still worthless.
  const allowLowQualitySource = override === 'any';
  if (override === 'any') {
    anyLanguage = true;
  ...
  return {
    ...
    excludeTypes, allowLowQualitySource, language, anyLanguage, strictLanguage,
...
interface AxisVerdict {
  ...
  audioOk: boolean;
  languageOk: boolean;
  /** Wanted language present as subtitles only — audio is the original. */
...
function judgeRelease(r: ScrapedRelease, ctx: FilterContext): AxisVerdict {
  ...
  // Only meaningful when a specific language is wanted: with 'any' the original
  // audio is acceptable by definition.
  const subtitleOnly = !ctx.anyLanguage && isSubtitleOnlyRelease(r.title);
  return {
    ...
    sourceOk: ctx.allowLowQualitySource || !isLowQualitySource(r.title),
...
export function releaseRejectionReasons(r: ScrapedRelease, ctx: FilterContext): string[] {
  ...
  if (!v.languageOk) {
    ...
    if (v.subtitleOnly) reasons.push(`Nur untertitelt (Originalton), keine ${wanted}e Tonspur`);
    else reasons.push(r.language === 'unknown'
  ...
  if (!v.sourceOk) reasons.push('Minderwertige Aufnahmequelle (CAM/TS/TELESYNC/Screener)');
...
export function filterReleasesWithStats(releases: ScrapedRelease[], mediaType: 'movie' | 'show' = 'movie', override: QualityOverride | null = null): { releases: ScrapedRelease[]; stats: FilterStats } {
  ...
  const stats: FilterStats = {
    ...
    typeFail: 0, sourceFail: 0, dvFail: 0, noLinksFail: 0,
  };
  ...
  const filtered = releases.filter(r => {
    ...
    if (!v.sourceOk) stats.sourceFail++;
    if (v.dvExcluded) stats.dvFail++;
    if (!v.hasLinks) stats.noLinksFail++;

    return v.qualityOk && v.audioOk && v.languageOk && v.typeOk && v.sourceOk && !v.dvExcluded && v.hasLinks;
  });
