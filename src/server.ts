...
import { registerBuiltinPlugins, loadDynamicPlugins } from './plugins/bootstrap';
...
// Register bundled plugins before any middleware so their CSP domains are
// known when helmet's CSP directives are evaluated below. Dynamic plugins
// from data/plugins/ are then loaded — bundled plugins keep priority on
// id-collisions.
...
app.get('/api/health/detailed', async (_req, res) => {
  ...
  const diskInfo: Record<string, { path: string; totalGB: number; freeGB: number; usedPercent: number } | { path: string; error: string }> = {};
  ...
  for (const key of pathKeys) {
    ...
    try {
      // Use df for disk space (works in Docker/Linux). execFile avoids shell
      // interpolation — paths from settings are user-controlled config.
      ...
      if (parts.length >= 4) {
        ...
        diskInfo[key] = { path: p, error: 'Konnte Speicherplatz nicht lesen' };
      }
    } catch {
      diskInfo[key] = { path: p, error: 'df-Befehl fehlgeschlagen' };
    }
  }
...
// Update check — compares running GIT_COMMIT against latest GitHub commit (cached 1h)
let updateCache: { ts: number; latest: string | null; current: string } = { ts: 0, latest: null, current: '' };
app.get('/api/update-check', async (req, res) => {
  ...
  // Return cached result if fresh (1 hour) — skip cache with ?force=true
  if (!force && updateCache.ts > now - 300000 && updateCache.current === current) {
    const updateAvailable = !!updateCache.latest && updateCache.latest !== current;
    return res.json({ updateAvailable, current, latest: updateCache.latest, cached: true, platform });
  }
  ...
  try {
    ...
    updateCache = { ts: now, latest, current };
...
const server = app.listen(PORT, async () => {
  ...
  if (jdownloaderService.isConfigured()) {
    // Auto-overwrite existing files during extraction (prevents blocking dialog)
    jdownloaderService.configureExtractionOverwrite().catch(err => {
      logger.error('Failed to configure JD extraction overwrite:', err.message);
    });
