...
export const PLUGIN_TIMEOUTS = {
  ...
  discover: 30_000,
  healthCheck: 15_000,
  canaryCheck: 300_000, // two full-path probes (movie + show site), each may need a
                        // challenge-solver round + browser scrape — backstop only
  /**
   * A self-downloading plugin transfers whole albums/files itself, so this is
   * generous — but it must exist. Without a ceiling a wedged transfer left its
   * fetch job in 'processing' forever, which the dashboard reads as an active
   * download and never clears; the only recovery was a container restart.
   */
  fetchRelease: 4 * 60 * 60_000, // 4 hours
} as const;
