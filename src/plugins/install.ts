...
import { validateManifest, validateManifestForPreview, hostVersionSatisfies, ManifestValidationError, HOST_VERSION } from './manifest';
import type { PluginManifest } from './manifest';
import { extractManifestFromSource } from './staticManifest';
import { createPluginContext, type PluginFactory } from './context';
import { recordDisclaimerAccepted, defaultPluginsDir, invalidatePluginScanCache } from './loader';
...
/**
 * Plugin ids arrive from route params (`/api/plugins/:id/accept`, `DELETE
 * /api/plugins/:id`) and are interpolated into a filesystem path. Express
 * URL-decodes path params, so `..%2F..%2Ftmp%2Fevil` reaches us as
 * `../../tmp/evil` — enough to execute or delete any `*.dlvault.js` on the
 * filesystem. Manifest ids are already required to be kebab-case; hold the
 * route params to the same shape before they touch a path.
 */
const SAFE_PLUGIN_ID = /^[a-z0-9][a-z0-9-]*[a-z0-9]$/;

function assertSafePluginId(pluginId: string): void {
  if (typeof pluginId !== 'string' || !SAFE_PLUGIN_ID.test(pluginId)) {
    throw new InstallError(`invalid plugin id "${pluginId}"`, 400);
  }
}
...
function parseAndValidateBuffer(buf: Buffer): { manifest: PluginManifest; factory: PluginFactory } {
  ...
  // Capture the resolved cache key while the file still exists (require.resolve
  // would throw after we unlink it). On macOS the real path differs from tmpFile
  // (/var → /private/var), so this — not tmpFile — is the actual cache key.
  ...
  try {
    ...
    try {
      // Evict the loaded module — each install mints a fresh unique tmp path, so
      // without this every install would leak a permanent require.cache entry.
      // The returned factory keeps working: it's held by reference, not via cache.
      delete require.cache[resolved];
 ...
 *
 * The manifest is read statically — this endpoint must never execute the
 * candidate. Preview is the step where the user decides whether to trust the
 * plugin *at all*, so running its top-level code here handed arbitrary code
 * execution to anyone who could POST a file, before any accept had happened.
 */
export async function previewPlugin(input: { url?: string; contentBase64?: string }): Promise<PluginPreview> {
  ...
  const raw = extractManifestFromSource(buf.toString('utf-8'));
  if (raw === null) {
    throw new InstallError('could not read the plugin manifest — it must be a plain object literal');
  }
  let manifest: PluginManifest;
  try {
    manifest = validateManifestForPreview(raw);
  } catch (err) {
    const msg = err instanceof ManifestValidationError ? err.message : String(err);
    throw new InstallError(`invalid manifest: ${msg}`);
  }
  if (!hostVersionSatisfies(HOST_VERSION, manifest.minHostVersion)) {
    throw new InstallError(`plugin requires host ${manifest.minHostVersion}, this build is ${HOST_VERSION}`);
  }
