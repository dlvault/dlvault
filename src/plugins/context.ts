...
export interface ChallengeSolveResult {
  /** True when the solver returned a cleared session for the URL. */
  ok: boolean;
  /** HTTP status the solver's browser saw for the target page. */
  status?: number;
  /** Fully-rendered page HTML after the challenge was cleared. */
  html?: string;
  /** Clearance cookies (e.g. `cf_clearance`) to replay on later requests. */
  cookies: ChallengeCookie[];
  /** User-Agent the solver used — clearance cookies are UA-bound, replay it verbatim. */
  userAgent?: string;
  error?: string;
}

/**
 * Always-available capability: clear an interstitial browser challenge (e.g.
 * Cloudflare "managed challenge") for a URL by delegating to an external,
 * source-agnostic solver service (FlareSolverr-compatible API at
 * `challenge_solver.url`, default the host-bundled instance).
 *
 * The host stays neutral: it knows nothing about which sites are protected or
 * why — the plugin decides when a URL needs solving. The returned clearance
 * cookies + User-Agent can be replayed on plain `http` requests (they are
 * IP + UA bound, valid for the cookie's lifetime), or injected into a
 * `browser` page so the existing scrape flow loads an already-cleared page.
 *
 * Degrades gracefully: `ok:false` (never throws) when no solver is configured
 * or the solve failed, so callers fall back to their unsolved path.
 */
export interface PluginChallengeSolver {
  solve(url: string, opts?: { maxTimeoutMs?: number }): Promise<ChallengeSolveResult>;
}
...
export interface PluginContext {
  ...
  challengeSolver: PluginChallengeSolver;
...
// Default endpoint for the bundled FlareSolverr-compatible challenge solver.
// Overridable via the `challenge_solver.url` setting (empty → this default).
const CHALLENGE_SOLVER_DEFAULT_URL = 'http://127.0.0.1:8191';

function buildChallengeSolver(pluginId: string): PluginChallengeSolver {
  return {
    async solve(url, opts): Promise<ChallengeSolveResult> {
      const base = (getSetting('challenge_solver.url') || CHALLENGE_SOLVER_DEFAULT_URL).replace(/\/+$/, '');
      if (!base) return { ok: false, cookies: [], error: 'no challenge solver configured' };
      const maxTimeout = opts?.maxTimeoutMs ?? 60_000;
      try {
        const res = await axios.post(
          `${base}/v1`,
          { cmd: 'request.get', url, maxTimeout },
          { timeout: maxTimeout + 15_000, headers: { 'Content-Type': 'application/json' } },
        );
        const sol = res.data?.solution;
        if (res.data?.status !== 'ok' || !sol) {
          return { ok: false, cookies: [], status: sol?.status, error: res.data?.message || 'solver returned no solution' };
        }
        const cookies: ChallengeCookie[] = Array.isArray(sol.cookies)
          ? sol.cookies.map((c: any) => ({ name: c.name, value: c.value, domain: c.domain, path: c.path }))
          : [];
        return { ok: true, status: sol.status, html: sol.response, cookies, userAgent: sol.userAgent };
      } catch (err: any) {
        rootLogger.debug(`[plugin:${pluginId}] challenge solve failed for ${url}: ${err?.message || err}`);
        return { ok: false, cookies: [], error: err?.message || String(err) };
      }
    },
  };
}
  ...
  return {
    async launch() {
      ...
      try {
        ...
        // A failed browser launch surfaces to the plugin as an empty link set,
        // which the user perceives as "hoster filters don't work" — with no
        // obvious cause. Make the root reason explicit. The common offender on
        // Docker Desktop / Windows is too little RAM in the WSL2 VM (Firefox
        // gets OOM-killed); on a bare host it's missing browser binaries
        // (`npx playwright install firefox`).
        rootLogger.error(
          `[plugin:${pluginId}] Firefox launch failed: ${err?.message || err}. ` +
          `Hoster link resolution needs a browser. Likely causes: too little container ` +
          `memory (raise Docker Desktop / WSL2 RAM limit) or missing Firefox binaries.`,
        );
        throw err;
      }
    },
...
function buildSecretsService(pluginId: string, declared: Set<string>): PluginSecretsService {
  return {
    get(key: string): string {
      ...
      // Enforce the manifest's declared requiredSecrets as an allowlist, so the
      // install-time disclosure is truthful: a plugin can only read the secrets it
      // asked the user to grant, not every shared secret in the store.
      if (!declared.has(key)) {
        rootLogger.warn(`[plugin:${pluginId}] secrets.get("${key}") denied — not in manifest.requiredSecrets`);
        return '';
      }
...
export function createPluginContext(manifest: PluginManifest): PluginContext {
  ...
  const ctx: PluginContext = {
    ...
    challengeSolver: buildChallengeSolver(pluginId),
  ...
  if (perms.has('secrets')) {
    const declaredSecrets = new Set((manifest.requiredSecrets || []).map(s => s.key));
    ctx.secrets = buildSecretsService(pluginId, declaredSecrets);
  }
