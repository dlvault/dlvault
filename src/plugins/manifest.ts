...
const VALID_MEDIA_TYPES = new Set<MediaType>(['movie', 'show', 'music']);
const VALID_FIELD_TYPES = new Set(['string', 'number', 'boolean', 'secret', 'multi-select']);

/**
 * A cspDomains entry: a bare hostname (`example.com`, `*.cdn.example.com`) or an
 * https origin, optionally with a port. Nothing else — the value is concatenated
 * into helmet's `img-src` directive, and helmet REJECTS a directive value
 * containing `;` or `,` by throwing at configuration time. Since that config is
 * built during module init, one plugin manifest with `"evil.com; script-src *"`
 * made the whole process fail to start on the next boot: no UI, so no way to
 * uninstall the offending plugin without shell access to the data volume.
 */
const CSP_DOMAIN_PATTERN = /^(https?:\/\/)?(\*\.)?[a-z0-9-]+(\.[a-z0-9-]+)*(:\d{1,5})?$/i;
...
export function validateManifest(raw: unknown): PluginManifest {
  ...
  for (const t of m.mediaTypes as unknown[]) {
    requireField(typeof t === 'string' && VALID_MEDIA_TYPES.has(t as MediaType),
      `manifest.mediaTypes entry "${t}" must be one of "movie", "show", "music"`);
  }
  ...
  if (m.cspDomains !== undefined) {
    ...
    for (const d of m.cspDomains as unknown[]) {
      ...
      requireField(CSP_DOMAIN_PATTERN.test(d as string),
        `manifest.cspDomains entry "${d}" must be a bare hostname or https:// origin`);
    }
  }
...
/**
 * Validate the subset of a manifest that can be read WITHOUT executing the
 * plugin — the identity and disclosure fields the install dialog shows.
 *
 * Deep structures (`settingsSchema`, `requiredSecrets`) are deliberately dropped
 * rather than validated: bundles commonly build them from module-level constants
 * that a static read resolves to `undefined`, and rejecting a plugin over a field
 * we merely couldn't see would turn every such plugin into an "error" the user
 * can't act on. Those fields only matter once the plugin is registered, and the
 * full {@link validateManifest} runs on the real runtime manifest at that point.
 */
export function validateManifestForPreview(raw: unknown): PluginManifest {
  requireField(raw && typeof raw === 'object', 'manifest must be an object');
  const m = raw as Record<string, unknown>;
  ...
  // Carry over the remaining display fields only when they have the right shape.
  if (typeof m.description === 'string') preview.description = m.description;
  if (typeof m.author === 'string') preview.author = m.author;
  if (typeof m.homepage === 'string') preview.homepage = m.homepage;
  if (typeof m.minHostVersion === 'string') preview.minHostVersion = m.minHostVersion;
  if (typeof m.apiVersion === 'number' && Number.isInteger(m.apiVersion)) preview.apiVersion = m.apiVersion;
  if (Array.isArray(m.permissions)) {
    preview.permissions = (m.permissions as unknown[]).filter(
      (p): p is PluginPermission => typeof p === 'string' && VALID_PERMISSIONS.has(p as PluginPermission),
    );
  }
  if (Array.isArray(m.cspDomains)) {
    preview.cspDomains = (m.cspDomains as unknown[]).filter((d): d is string => typeof d === 'string');
  }

  return preview as unknown as PluginManifest;
}
