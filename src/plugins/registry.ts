...
class PluginRegistry {
   ...
   *
   * Entries are re-validated here even though the manifest validator already
   * checks them: this string goes straight into a CSP directive, and helmet
   * throws on a value containing `;` or `,`. A throw at this point would happen
   * mid-request (the directive is evaluated per request), so anything that
   * doesn't look like a host is dropped with a warning rather than trusted.
   */
  getCspDomains(): string[] {
    const SAFE_ORIGIN = /^(https?:\/\/)?(\*\.)?[a-z0-9-]+(\.[a-z0-9-]+)*(:\d{1,5})?$/i;
    const domains = new Set<string>();
    for (const p of this.plugins) {
      for (const d of p.cspDomains || []) {
        if (typeof d !== 'string' || !SAFE_ORIGIN.test(d)) {
          logger.warn(`Plugin "${p.id}" declares an unusable cspDomains entry ${JSON.stringify(d)} — ignoring`);
          continue;
        }
