...
router.get('/', (_req: Request, res: Response) => {
  ...
  // Mask sensitive values for the frontend. The fixed list below covers core
  // service credentials; everything under `secret-store.*` (shared plugin
  // secrets) is masked via the prefix rule that follows.
  ...
  for (const key of Object.keys(masked)) {
    if (key.startsWith('secret-store.') && masked[key]) {
      masked[key] = '••••••••';
    }
  }
  res.json(masked);
