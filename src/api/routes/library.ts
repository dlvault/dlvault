...
import { getMusicAlbums, getMusicAlbumTracks, deleteMusicAlbum, isMusicLibraryConfigured } from '../../services/musicLibrary';
...
// Media-server item identifiers arriving as route params. Every route that
// interpolates one into an upstream Plex/Jellyfin URL must test it first:
// Express URL-decodes path params, so `..%2F..%2FUsers%2F<id>` becomes a real
// path traversal against the media server — carried out with the stored admin
// token. Declared here (not next to the poster route) because the /music routes
// above need it too.
...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Serve from the provider cache unless the client explicitly asks for fresh
    // data (`?refresh=1`). Forcing a full re-fetch on every page view defeated
    // the 10-minute cache entirely: on a 3000-item Plex library that is 15+
    // paginated requests with 30s timeouts, per view, per user.
    const forceRefresh = req.query.refresh === '1' || req.query.refresh === 'true';
    const movies = await provider.getMovies(forceRefresh);

    // Build a per-item deep-link to the media server's web UI. Jellyfin needs
    // only the item ID; Plex needs the server's machineIdentifier (cached).
    let deepLinkBuilder: (id: string) => string | null = () => null;
    if (providerType === 'jellyfin') {
      const base = (getSetting('jellyfin.url') || '').replace(/\/+$/, '');
      if (base) deepLinkBuilder = (id) => `${base}/web/index.html#/details?id=${encodeURIComponent(id)}`;
    } else if (providerType === 'plex') {
      const base = (getSetting('plex.server_url') || '').replace(/\/+$/, '');
      const mid = base ? await plexLibraryService.getMachineIdentifier() : null;
      if (base && mid) {
        const midEnc = encodeURIComponent(mid);
        deepLinkBuilder = (id) =>
          `${base}/web/index.html#!/server/${midEnc}/details?key=${encodeURIComponent(`/library/metadata/${id}`)}`;
      }
    }
    ...
    const items = movies
      ...
      .map(m => ({
        ...
        deepLinkUrl: deepLinkBuilder(m.id),
      }));
...
// GET /api/library/music — album collection from the media server's music
// library (Jellyfin). Covers reuse the /api/library/:id/poster proxy. Static
// path, declared before /:id/poster so it isn't shadowed.
router.get('/music', async (_req: Request, res: Response) => {
  try {
    if (!isMusicLibraryConfigured()) {
      res.json({ items: [], total: 0, source: 'none' });
      return;
    }
    const albums = await getMusicAlbums();
    const items = albums.map(a => ({
      ...a,
      coverUrl: a.imageTag ? `/api/library/${a.id}/poster?tag=${a.imageTag}` : null,
    }));
    res.json({ items, total: items.length, source: 'jellyfin' });
  } catch (error: any) {
    logger.error(`Failed to load music library: ${error?.message || error}`);
    res.status(500).json({ error: 'Failed to load music library' });
  }
});

// GET /api/library/music/:id/tracks — track list of one album (for the detail
// view). Static prefix before /:id/poster so it isn't shadowed.
router.get('/music/:id/tracks', async (req: Request, res: Response) => {
  try {
    if (!isMusicLibraryConfigured()) {
      res.json({ tracks: [] });
      return;
    }
    // Same guard the poster proxy uses — the id is interpolated into an upstream
    // Jellyfin URL, and Express decodes `%2F` inside a path segment.
    const albumId = String(req.params.id);
    if (!ID_RE.test(albumId)) { res.status(400).json({ error: 'invalid id' }); return; }
    const tracks = await getMusicAlbumTracks(albumId);
    res.json({ tracks });
  } catch (error: any) {
    logger.error(`Failed to load album tracks: ${error?.message || error}`);
    res.status(500).json({ error: 'Failed to load album tracks' });
  }
});

// DELETE /api/library/music/:id — remove an album (and its files) via Jellyfin.
router.delete('/music/:id', async (req: Request, res: Response) => {
  try {
    if (!isMusicLibraryConfigured()) {
      res.status(400).json({ error: 'Musik-Bibliothek nicht konfiguriert' });
      return;
    }
    const albumId = String(req.params.id);
    if (!ID_RE.test(albumId)) { res.status(400).json({ error: 'invalid id' }); return; }
    await deleteMusicAlbum(albumId);
    addLogEntry(null, 'music_deleted', `Album entfernt (${albumId})`);
    res.json({ ok: true });
  } catch (error: any) {
    logger.error(`Failed to delete album: ${error?.message || error}`);
    res.status(502).json({ error: error?.message || 'Löschen fehlgeschlagen' });
  }
});
