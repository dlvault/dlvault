...
router.post('/start', async (req: Request, res: Response) => {
  ...
  // If the operator enabled token auth (API_TOKEN set), enforce it on this
  // container-replacing route too — matching /backup. When no token is configured
  // (the trusted-LAN default) the confirm header above stays the only gate, so the
  // one-click UI update keeps working without any frontend token plumbing.
  ...
  // Registry credentials travel via a mode-0600 file in the shared data
  // volume instead of a container env var — env vars stay readable through
  // `docker inspect` for the updater's whole lifetime. The updater consumes
  // and deletes the file right after login; a stale copy from an aborted run
  // is removed here when no auth is configured anymore.
  const registryAuthFile = path.join(DATA_DIR_IN_CONTAINER, '.registry-auth');
  if (process.env.REGISTRY_AUTH) {
    try {
      fs.writeFileSync(registryAuthFile, process.env.REGISTRY_AUTH, { mode: 0o600 });
    } catch (err) {
      logger.error('Failed to write registry auth file:', (err as Error).message);
      return res.status(500).json({ error: 'registry_auth_write_failed' });
    }
    env.push(`REGISTRY_AUTH_FILE=/status/${path.basename(registryAuthFile)}`);
  } else {
    try { fs.unlinkSync(registryAuthFile); } catch { /* not present */ }
  }
...
// Cap concurrent update-stream connections — each opens its own fs.watch + 1s
// poller, so an uncapped fan-out (many open tabs / a script) is wasteful. The
// underlying update still completes; capped-out clients just don't get the live
// tail and can fall back to polling /api/update/state.
let updateStreamClients = 0;
const MAX_UPDATE_STREAM_CLIENTS = 5;
...
router.get('/stream', (req: Request, res: Response) => {
  ...
  if (updateStreamClients >= MAX_UPDATE_STREAM_CLIENTS) {
    res.write(`event: error\ndata: ${JSON.stringify({ message: 'Too many update streams open' })}\n\n`);
    res.end();
    return;
  }
  updateStreamClients++;
