...
async function hasExtractionTempFiles(dirPath: string): Promise<boolean> {
  ...
  // `part\d*` — the bare `.part` (no digits) is JDownloader's in-progress file for
  // a plain direct-hoster download; requiring a digit matched multipart archives only.
  const ARCHIVE_PART_RE = /\.(part\d*|r\d{2}|[0-9]{3}|rev|!ut|crdownload)$/i;
...
router.get('/packages', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    // JD unreachable → report disconnected so the UI doesn't render a misleading empty list.
    if (packages === null) {
      res.json({ connected: false, packages: [] });
      return;
    }
...
router.get('/linkgrabber', async (_req: Request, res: Response) => {
  try {
    ...
    if (packages === null) {
      res.status(503).json({ error: 'JDownloader unreachable' });
      return;
    }
    res.json(packages);
  } catch (error: any) {
...
router.post('/speed-limit', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (typeof limitKbps === 'number') {
      limitOk = await jdownloaderService.setSpeedLimit(limitKbps);
    }
    if (typeof enabled === 'boolean') {
      enabledOk = await jdownloaderService.setSpeedLimitEnabled(enabled);
    }

    // Read back what JD actually holds so the response reflects reality, not just
    // "the call returned". A JD build whose DownloadSpeedLimit key/path differs
    // would accept the /config/set silently yet keep the old rate — this surfaces
    // that instead of reporting a false success (the symptom on some setups).
    const appliedKbps = await jdownloaderService.getSpeedLimit();
    const isLimited = await jdownloaderService.isSpeedLimited();
    const ok = limitOk && enabledOk;
    if (!ok) {
      logger.warn(`JDownloader did not accept the speed-limit change (limitOk=${limitOk}, enabledOk=${enabledOk}); JD now reports ${appliedKbps} KB/s, enabled=${isLimited}`);
    } else if (typeof limitKbps === 'number' && enabled !== false && appliedKbps !== Math.round(limitKbps)) {
      logger.warn(`Speed limit applied but JD reports ${appliedKbps} KB/s, expected ${Math.round(limitKbps)} KB/s — likely a different JD config key/version or the wrong device.`);
    }
    res.json({ success: ok, appliedKbps, enabled: isLimited });
  } catch (error: any) {
