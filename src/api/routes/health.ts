...
import { getUndecryptableKeys } from '../../database/encryption';
...
/**
 * Are the stored credentials actually readable?
 *
 * When the encryption key in the data volume no longer matches the database,
 * every secret decrypts to '' and the whole app reports Trakt/JDownloader/Plex
 * as "not configured" — indistinguishable from a fresh install, so users
 * re-entered everything without ever learning what happened. Critical: nothing
 * downstream works, and the fix (restore the .key file) is only available while
 * the user knows that is the problem.
 */
function checkSecretsReadable(): CheckOutcome {
  const failed = getUndecryptableKeys();
  if (failed.length === 0) return { ok: true, critical: true };
  return {
    ok: false,
    critical: true,
    error:
      `${failed.length} stored secret(s) could not be decrypted (${failed.join(', ')}). ` +
      'The .key file in the data volume does not match this database — restore it from a backup, ' +
      'or re-enter these credentials in Settings.',
  };
}

/**
 * Is the watchlist provider's authentication intact?
 *
 * A rejected credential takes down watchlist sync, "mark as collected" AND the
 * movie/show search catalog at once — and nothing about that is visible in the
 * UI. One such outage ran for two and a half days purely because the only
 * symptom was a log line repeating every two minutes.
 */
function checkWatchlistAuth(): CheckOutcome {
  const failure = traktService.getAuthFailure();
  if (!failure) return { ok: true, critical: true };
  const hours = Math.round((Date.now() - failure.since) / 3_600_000);
  return {
    ok: false,
    critical: true,
    error:
      `Trakt rejects the stored credentials (HTTP ${failure.status}, failing for ~${hours}h). `
      + 'Watchlist sync, collection marking and the movie/show search catalog are all unavailable. '
      + 'A 403 typically means the OAuth application no longer exists on the Trakt account — '
      + 'reconnect under Settings → Watchlist.',
  };
}
...
router.get('/deep', async (req: Request, res: Response) => {
  ...
  const checkEntries: Array<[string, Promise<CheckResult>]> = [
    ...
    ['secrets_readable', timed(async () => checkSecretsReadable())],
    ['watchlist_auth', timed(async () => checkWatchlistAuth())],
  ];
  if (!quick) {
