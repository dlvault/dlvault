...
/**
 * 2Captcha balance check: free API call that validates the API key is alive
 * and the account has credits. Catches expired keys before movies start
 * failing in production.
 */
async function check2CaptchaBalance(): Promise<CheckOutcome> {
  const apiKey = getSetting('secret-store.2captcha-api-key');
  if (!apiKey) {
    return { ok: true, critical: false, detail: 'not configured' };
  }
  try {
    ...
    if (data.status !== 1) {
      return { ok: false, critical: true, error: `2Captcha API error: ${data.request}` };
    }
    const balance = Number(data.request);
    if (balance < 0.5) {
      return { ok: false, critical: false, error: `Low balance: $${balance.toFixed(2)}`, balance };
    }
    return { ok: true, critical: false, balance };
  } catch (err: any) {
    return { ok: false, critical: false, error: `2Captcha unreachable: ${err.message}` };
  }
}
...
router.get('/deep', async (req: Request, res: Response) => {
  ...
  const checkEntries: Array<[string, Promise<CheckResult>]> = [
    ['2captcha_balance', timed(check2CaptchaBalance)],
