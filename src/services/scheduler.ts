...
// The relay lives outside this machine and hiccups on its own schedule, so it
// gets a longer fuse than a device that has genuinely disappeared.
const JD_RELAY_ALERT_THRESHOLD = 3;
...
export async function processMovie(movie: Movie): Promise<void> {
  ...
  // Runs AFTER the backoff gate, and that ordering is the whole point.
  //
  // At retry_count == 10 the backoff is min(2^9, 24h) = 24h. Sitting before the
  // gate, this block fired on every 30-minute retry-monitor tick while the gate
  // turned straight back around WITHOUT incrementing — so the counter stuck at 10
  // for a full day and the wind-down ran ~50 times instead of once. Telegram
  // survived that (notifyGaveUp dedupes internally); Seerr did not, and got 50
  // identical "no release yet" issues per title, 550 in total across 11 titles.
  //
  // Behind the gate, the only tick that reaches it is one that actually goes on to
  // search — and that tick then increments past 10, so the crossing happens once.
  /**
  ...
  if (freshMovie.retry_count === MAX_RETRIES) {
    ...
    // Deliberately no return: this tick goes on to search as usual.
  }
  ...
  // A title under repair is deliberately reset to fetch a replacement, and the
  // media server still lists the old copy — it has not rescanned, and on a
  // report from Seerr it never will until the file actually changes. Without
  // this guard the very next sync flips the title back to 'downloaded' and the
  // repair is over before a single search ran — a report was seen cancelled by
  // this line 18 minutes after it was filed. The two library checks in
  // postprocess already skip repairs; this one was the hole.
  if (libraryProvider.isConfigured() && !isShow && freshMovie.repair !== 1) {
    try {
      if (!libraryPreloaded) {
  ...
  try {
    ...
    // A Seerr hand-over carries a tmdb id and a localized title, never an
    // imdbId — and the nightly backfill could not turn that title into an id
    // via OMDb. So every search for such a title went out by title alone, for
    // as long as it stayed unresolved. Resolve it here, before the first
    // search, so the plugins get the id-based lookup from attempt one.
    if (!freshMovie.imdb_id && freshMovie.tmdb_id) {
      const resolved = await resolveMovieImdbId(freshMovie);
      if (resolved.imdb_id) {
        freshMovie.imdb_id = resolved.imdb_id;
        movie.imdb_id = resolved.imdb_id;
        if (!movie.year && resolved.year) movie.year = resolved.year;
        logger.info(`${movie.title}: resolved imdb id ${resolved.imdb_id} from tmdb id ${freshMovie.tmdb_id}`);
      }
    }
...
function downloadsForNewestRelease(downloads: Download[]): Download[] {
  // The completed-row guard below has to cover the single-row case too. This
  // early return used to hand a lone 'completed' row straight back, so a title
  // left in 'found' with one finished download had its links re-sent to JD and
  // downloaded a second time. getDownloadsByMovieIds does a plain SELECT *, so
  // completed rows really do reach here.
  if (downloads.length <= 1) return downloads.filter(d => d.status !== 'completed');
  ...
  try {
    ...
    if (needImdb.length > 0) {
      ...
      for (const m of needImdb) {
        ...
        else unresolved.push(m.year ? `${m.title} (${m.year})` : m.title);
      }
      // A silent miss here left rows id-less for weeks with nothing in the log
      // to show for it. Name what is still open, once per sync.
      if (unresolved.length > 0) {
        logger.warn(`imdb id still unresolved for ${unresolved.length} of ${needImdb.length}: ${unresolved.join(', ')}`);
      } else {
        logger.info(`Resolved imdb id for all ${needImdb.length}`);
      }
    }
    ...
    if (needMeta.length > 0) {
      ...
      for (const m of needMeta) {
        const enriched = await enrichMovieMetadata(m);
        if (!enriched.metadata_fetched_at) stillMissing.push(`${m.title} (${m.imdb_id})`);
      }
      if (stillMissing.length > 0) {
        logger.warn(`Metadata still missing for ${stillMissing.length} of ${needMeta.length}: ${stillMissing.join(', ')}`);
      }
    }
...
export async function runJdMonitor(): Promise<void> {
  ...
  // Probe, don't just connect: a failed connect() only proves we could not reach
  // api.jdownloader.org. Blaming that on JD and telling the user to restart it
  // sent them after the wrong thing (2026-08-24: six minutes of relay timeouts
  // while the JD container had been up for 26 hours).
  const probe = await jdownloaderService.probe();
  const reachable = probe.state === 'ok';
  const relayDown = probe.state === 'relay_unreachable';
  ...
  if (!reachable) {
    ...
    const now = Date.now();
    // A relay outage needs a longer debounce than a missing device: it is
    // routinely transient (two windows on 2026-08-24, three and six minutes,
    // both self-healing) and nothing about it is actionable, whereas a device
    // that has vanished from a working relay really does need JD restarting.
    const threshold = relayDown ? JD_RELAY_ALERT_THRESHOLD : JD_OFFLINE_ALERT_THRESHOLD;
    // Alert only once the outage has persisted across enough probes (debounce),
    // and only once per outage (jdOfflineAlerted), rate-limited to 1×/hour so a
    // flapping relay can't spam. A lone transient probe timeout never alerts.
    if (
      jdConsecutiveFailures >= threshold
      && !jdOfflineAlerted
      && (now - jdOfflineAlertedAt) > JD_OFFLINE_ALERT_COOLDOWN_MS
    ) {
      jdOfflineAlertedAt = now;
      jdOfflineAlerted = true;
      logger.warn(relayDown
        ? `JD monitor: MyJDownloader relay not reachable (${jdConsecutiveFailures} consecutive probe failures) — JD itself may well be running`
        : `JD monitor: JDownloader is not reachable (${jdConsecutiveFailures} consecutive probe failures)`);
      try {
        const { sendTelegramSystemAlert } = await import('./telegram');
        await sendTelegramSystemAlert(relayDown
          ? '🌐 <b>MyJDownloader nicht erreichbar</b>\n'
            + 'dlvault kommt gerade nicht an den MyJDownloader-Dienst (my.jdownloader.org) heran. '
            + 'JDownloader selbst läuft möglicherweise ganz normal weiter — hier ist nichts zu tun. '
            + 'Das legt sich üblicherweise von selbst; laufende Downloads werden danach automatisch fortgesetzt.'
