...
export async function processMovie(movie: Movie): Promise<void> {
  ...
  try {
    ...
    if (filtered.length === 0) {
      ...
      addLogEntry(
        movie.id,
        releases.length === 0 ? 'not_found' : allFailedOnLinks ? 'no_hoster' : 'quality_mismatch',
...
async function processShowSeasons(movie: Movie, url: string, filtered: ScrapedRelease[], activePlugin: SourcePlugin | null): Promise<void> {
  ...
  if (anyProcessed) {
    ...
    // Every known season is not_found → advance the show's retry clock so the
    // existing movie-level backoff/give-up (it runs for shows too, before
...
export function startScheduler(): void {
  ...
  logger.info(`Scheduler started: full sync every ${intervalHours}h, watchlist monitor every 2 min, JD monitor every 5 min, retry monitor every 30 min${healthMonitorEnabled ? ', health monitor every 15 min' : ''}`);
}
 ...
 * fault the owner can act on — it only produced notification noise.
 *
...
export async function runHealthMonitor(): Promise<void> {
  ...
  const counts = db.prepare(`
    ...
    FROM activity_log
    WHERE created_at >= ?
      AND action IN ('jdownloader_failed', 'captcha_pending')
    GROUP BY action
