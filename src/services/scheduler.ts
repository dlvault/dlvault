...
import { filterReleases } from '../scraper/filter';
...
let healthMonitorTask: cron.ScheduledTask | null = null;
...
export async function processMovie(movie: Movie): Promise<void> {
  ...
  if (freshMovie.retry_count > 0 && freshMovie.last_checked_at) {
    const lastChecked = new Date(freshMovie.last_checked_at).getTime();
  ...
  try {
    ...
    for (const plugin of candidates) {
      ...
      if (result.releases.length > 0) {
        ...
        activePlugin = plugin;
        logger.info(`Found ${releases.length} release(s) for ${movie.title} via plugin "${plugin.id}"`);
        break;
      }
    ...
    if (!url) {
      ...
      addLogEntry(movie.id, 'not_found', `${movie.title} not found (retry ${movie.retry_count + 1}/${MAX_RETRIES})`);
    ...
    if (filtered.length === 0) {
      ...
      logger.info(`${movie.title}: ${releases.length} release(s) found but none match filters — ${qualitySummary} [hosters: ${availableHosters}]`);
      addLogEntry(movie.id, 'quality_mismatch',
        `Found ${releases.length} release(s) but none match quality requirements`);
      return;
    }
    ...
    // --- Movie processing ---
    // Step 4: Find first release with resolved direct download links
...
async function processShowSeasons(movie: Movie, url: string, filtered: ScrapedRelease[], activePlugin: SourcePlugin | null): Promise<void> {
  ...
  // Also check "downloaded" seasons for new episodes (weekly release scenario)
  const downloadedSeasons = seasons.filter(s => s.status === 'downloaded');
  for (const season of downloadedSeasons) {
    const pending = getPendingEpisodes(season.id);
    if (pending.length > 0) {
      // Has new pending episodes — reopen this season
      updateSeasonStatus(season.id, 'pending', url);
      pendingSeasons.push({ ...season, status: 'pending' });
      logger.info(`${movie.title} S${String(season.season_number).padStart(2, '0')}: ${pending.length} new episode(s) detected — reopening`);
    }
  }
  ...
  for (const season of pendingSeasons) {
    ...
    const packRelease = seasonPacks.find(r => r.season === sNum);
    if (packRelease) {
      ...
      for (const ep of pending) {
        ...
        if (directLinks.length === 0 && activePlugin) {
          ...
          if (candidates.length > 0) {
            ...
            const resolved = await activePlugin.resolveLinks(candidates);
...
async function tryDownloadRelease(
  ...
  // (no-op for plugins that already return direct URLs from findReleases).
  if (directLinks.length === 0 && activePlugin) {
    const candidates = release.links.filter(l => (enabledHosters.length === 0 || enabledHosters.includes(l.hoster)));
    if (candidates.length > 0) {
...
async function retryFailedJDownloaderSends(allMoviesArg?: Movie[]): Promise<void> {
  ...
  if (foundMovies.length > 0) {
    ...
    for (const movie of foundMovies) {
      ...
      if (success) {
        ...
        const stuckSince = Date.now() - new Date(movie.last_checked_at).getTime();
  ...
  for (const show of shows) {
    ...
    for (const season of foundSeasons) {
      const seasonDownloads = downloads.filter(d => d.download_url);
      ...
      if (success) {
        ...
        const stuckSince = season.last_checked_at
          ? Date.now() - new Date(season.last_checked_at).getTime() : Infinity;
        if (stuckSince > 24 * 60 * 60 * 1000) {
 ...
 */
async function checkQualityUpgrades(): Promise<void> {
  ...
  const downloadedMovies = getMoviesByStatus('downloaded');
  ...
  for (const movie of downloadedMovies) {
    ...
    try {
      ...
      const directLinks = bestRelease.links
        .filter(l => (enabledHosters.length === 0 || enabledHosters.includes(l.hoster)))
        .map(l => l.url);
...
export function startScheduler(): void {
  ...
  if (healthMonitorEnabled) {
    ...
    healthMonitorTask = cron.schedule('*/15 * * * *', async () => {
      // node-cron doesn't track in-flight runs — if a prior tick is still
      // active (e.g. event loop blocked by slow plugin resolution), skip rather than stack.
      if (healthMonitorRunning) {
        logger.debug('Health monitor already running, skipping tick');
        return;
      }
...
export async function runHealthMonitor(): Promise<void> {
  const now = Date.now();
  const windowStartIso = new Date(now - 60 * 60 * 1000).toISOString(); // last hour
  ...
  const counts = db.prepare(`
    ...
    GROUP BY action
  `).all(windowStartIso) as { action: string; count: number }[];
