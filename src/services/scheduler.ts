...
import { enrichMovieMetadata } from './metadata';
...
export async function processMovie(movie: Movie): Promise<void> {
  ...
  if (freshMovie.retry_count >= MAX_RETRIES) {
    ...
    // Let whoever requested this via Telegram know we've genuinely given up
  ...
  try {
    ...
    if (filtered.length === 0) {
      ...
      // Source page exists but nothing usable: no releases at all OR releases that
      // all failed to yield links → "kein Download verfügbar"; otherwise the
      // releases are there but fail the quality/audio/language filter.
      const notFoundReason = (releases.length === 0 || allFailedOnLinks) ? 'no_download' : 'quality_mismatch';
      ...
      addLogEntry(
        ...
        allFailedOnLinks
          ...
          : releases.length === 0
            ...
            : `${releases.length} Release(s) gefunden, aber keins erfüllt die Anforderungen (${reasonSummary}; gewünscht ≥ ${wanted})`,
      );
      return;
    }
...
export function startScheduler(): void {
  ...
  logger.info(`Scheduler started: full sync every ${intervalHours}h, watchlist monitor every 2 min, JD monitor every 5 min${healthMonitorEnabled ? ', health monitor every 15 min' : ''}`);
}
...
export async function runJdMonitor(): Promise<void> {
  ...
  if (!reachable) {
    ...
    jdUpdateAvailable = false;
    const now = Date.now();
    // Alert on the transition into unreachable (wasReachable !== false covers
    // both "was online" and "first probe finds it down"), rate-limited per outage.
    if (wasReachable !== false && (now - jdOfflineAlertedAt) > JD_OFFLINE_ALERT_COOLDOWN_MS) {
      jdOfflineAlertedAt = now;
      logger.warn('JD monitor: JDownloader is not reachable');
      try {
        const { sendTelegramSystemAlert } = await import('./telegram');
