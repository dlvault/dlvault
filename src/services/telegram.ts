...
import { addMovie, deleteMovie, getMovieById, getMovieByTmdbId, getMovieByImdbId, getAllMovies, getMoviesByStatus, type MediaType, type Movie } from '../database/services/movies';
 ...
 * bare title to plugin searches (many full-text search indexes don't
 * understand "Title YYYY" and return 0 results). The year is returned
...
async function handleMessage(chatId: number, text: string): Promise<void> {
  ...
  if (trimmed.startsWith('/remove ')) {
    ...
    const match = getAllMovies().find(m =>
...
async function addManualEntry(
  ...
  if (existing) {
    await sendMessage(chatId, alreadyKnownMessage(existing));
...
async function handleCallback(chatId: number, messageId: number, callbackQueryId: string, data: string): Promise<void> {
  ...
  if (data.startsWith('add_manual:')) {
    ...
    if (existing) {
      await editMessage(chatId, messageId, alreadyKnownMessage(existing));
      if (existing.id) trackRequester(chatId, existing.title, existing.year ?? 0, existing.id);
      return;
    }
