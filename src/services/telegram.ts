...
async function handleMessage(chatId: number, text: string): Promise<void> {
  ...
  if (trimmed.startsWith('/add ')) {
    ...
    await sendMessage(chatId,
      `✅ <b>${escapeHtml(movieTitle)}</b> wurde in die Warteschlange gesetzt.`
    );
...
async function handleCallback(chatId: number, messageId: number, callbackQueryId: string, data: string): Promise<void> {
  ...
  if (data.startsWith('add_manual:')) {
    ...
    await editMessage(chatId, messageId, `⏳ <b>${escapeHtml(movieTitle)}</b> wird hinzugefuegt...`);
  ...
  if (data.startsWith('select:')) {
    ...
    if (film.poster) {
      await sendPhoto(chatId, film.poster, `🎬 <b>${escapeHtml(film.title)}</b> (${film.year})`);
    }

    // Build rich confirmation text
    let detailText = `🎬 <b>${escapeHtml(film.title)}</b> (${film.year})\n`;
...
async function handleSelection(chatId: number, messageId: number, film: FilmResult): Promise<void> {
  ...
  if (existing) {
    ...
    await editMessage(chatId, messageId,
      `ℹ️ <b>${escapeHtml(film.title)}</b> (${film.year}) ist bereits ${status}.`
    );
    return;
  }
  ...
  try {
    ...
    if (libraryProvider.isConfigured()) {
      ...
      if (inLibrary) {
        ...
        await editMessage(chatId, messageId,
          `✅ <b>${escapeHtml(film.title)}</b> (${film.year}) ist bereits in der ${providerName}-Bibliothek!`
        );
        return;
      }
    }
  ...
  try {
    ...
    await editMessage(chatId, messageId,
      `✅ <b>${escapeHtml(film.title)}</b> (${film.year}) hinzugefuegt!`
    );

    // Send progress tracking message
    const progressMsgId = await sendMessageReturningId(chatId,
      `<b>${escapeHtml(film.title)}</b> (${film.year})\n🔍 Suche wird gestartet...`
    );
