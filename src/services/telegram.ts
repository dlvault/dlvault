...
import { deleteMovie, getMovieById, getMovieByTmdbId, getMovieByImdbId, getAllMovies, getMoviesByStatus, resetRetryCount, updateMovieStatus, setSeasonCutoff, type MediaType, type Movie } from '../database/services/movies';
...
/**
 * Per-chat search results behind the inline keyboards.
 *
 * `nonce` is what makes an old keyboard safe. Callback data used to carry only
 * an array index, and a newer search silently replaced the session under it — so
 * scrolling up and tapping "Alien (1979)" from a previous search resolved index
 * 2 against the CURRENT results and queued a completely unrelated title, with
 * nothing in the conversation explaining why. Buttons now carry the nonce of the
 * session they were built from and are rejected once it no longer matches.
 */
const searchSessions = new Map<number, { results: FilmResult[]; expires: number; nonce: string }>();
let sessionCounter = 0;

/** Short, collision-free within a chat — callback_data is capped at 64 bytes. */
function nextSessionNonce(): string {
  sessionCounter = (sessionCounter + 1) % 100000;
  return sessionCounter.toString(36);
}

/**
 * Titles staged for the "Trotzdem auf die Liste" button.
 *
 * Telegram caps `callback_data` at 64 BYTES. The old payload embedded the title
 * itself (`add_manual:<title>|<year>|<type>`), so anything past roughly 40 ASCII
 * characters — fewer with umlauts, which are two bytes — made Telegram reject
 * the ENTIRE sendMessage with BUTTON_DATA_INVALID. The user got no reply at all,
 * which for a non-technical user reads as "the bot is broken". Only a short key
 * travels in the callback now.
 */
const pendingManualAdds = new Map<string, { title: string; year: number | null; mediaType: MediaType; expires: number }>();
let manualAddCounter = 0;

function stageManualAdd(title: string, year: number | null, mediaType: MediaType): string {
  manualAddCounter = (manualAddCounter + 1) % 1_000_000;
  const key = manualAddCounter.toString(36);
  pendingManualAdds.set(key, { title, year, mediaType, expires: Date.now() + 30 * 60 * 1000 });
  // Bound the map: drop anything expired whenever we add.
  const now = Date.now();
  for (const [k, v] of pendingManualAdds) if (now >= v.expires) pendingManualAdds.delete(k);
  return key;
}

type SessionPick =
  | { ok: true; film: FilmResult; index: number }
  | { ok: false; reason: 'expired' | 'stale' | 'invalid' };

/**
 * Resolve a `<nonce>:<index>` callback payload against the chat's current
 * session. A payload from an older keyboard resolves to 'stale', never to
 * whatever title happens to sit at that index now.
 */
function resolveSessionPick(chatId: number, payload: string): SessionPick {
  const sep = payload.indexOf(':');
  if (sep === -1) return { ok: false, reason: 'stale' }; // pre-nonce keyboard
  const nonce = payload.slice(0, sep);
  const index = parseInt(payload.slice(sep + 1), 10);

  const session = searchSessions.get(chatId);
  if (!session || Date.now() >= session.expires) return { ok: false, reason: 'expired' };
  if (session.nonce !== nonce) return { ok: false, reason: 'stale' };
  if (!Number.isInteger(index) || index < 0 || index >= session.results.length) {
    return { ok: false, reason: 'invalid' };
  }
  return { ok: true, film: session.results[index], index };
}
...
async function handleEmpfehlungen(chatId: number): Promise<void> {
  ...
  try {
    ...
    const nonce = nextSessionNonce();
    searchSessions.set(chatId, { results: picks, expires: Date.now() + 15 * 60 * 1000, nonce });
    const keyboard = picks.map((r, i) => ([
      { text: `${r.mediaType === 'show' ? '📺' : '🎬'} ${r.title}${r.year ? ' (' + r.year + ')' : ''}`, callback_data: `select:${nonce}:${i}` }
    ]));
...
async function searchAndShowResults(
  ...
  try {
    ...
    if (resultList.length === 0) {
      ...
      // Short key only — the title itself would blow Telegram's 64-byte
      // callback_data limit and make the whole message fail to send.
      const manualKey = stageManualAdd(title, year ?? null, mediaType === 'show' ? 'show' : 'movie');
      await sendMessageWithKeyboard(chatId,
        `Konnte zu "<b>${escapeHtml(display)}</b>" nichts finden.\n${tip}`,
        [[{ text: '📝 Trotzdem auf die Liste', callback_data: `add_manual:${manualKey}` }]]
      );
      return;
    }
    ...
    const nonce = nextSessionNonce();
    searchSessions.set(chatId, { results: resultList, expires: Date.now() + 15 * 60 * 1000, nonce });
...
async function handleCallback(chatId: number, messageId: number, callbackQueryId: string, data: string): Promise<void> {
  ...
  // Manual add from the "not found" screen. The callback carries a short staging
  // key (see stageManualAdd); the pipe-separated form is the legacy payload from
  // a keyboard that predates that change and is still tappable for a while.
  if (data.startsWith('add_manual:')) {
    ...
    const staged = pendingManualAdds.get(payload);
    if (staged && Date.now() < staged.expires) {
      ({ title: movieTitle, year, mediaType } = staged);
      pendingManualAdds.delete(payload);
    } else if (payload.includes('|') || !staged) {
      const parts = payload.split('|');
      movieTitle = parts[0];
      const parsedYear = parts[1] ? parseInt(parts[1], 10) : NaN;
      year = Number.isInteger(parsedYear) ? parsedYear : null;
      mediaType = parts[2] === 'show' ? 'show' : 'movie';
    } else {
      await answerCallbackQuery(callbackQueryId, 'Das ist zu lange her');
  ...
  if (data.startsWith('select:')) {
    const pick = resolveSessionPick(chatId, data.substring(7));
    if (!pick.ok) {
      await answerCallbackQuery(callbackQueryId, pick.reason === 'invalid' ? 'Ungueltige Auswahl' : 'Auswahl abgelaufen');
      if (pick.reason !== 'invalid') {
        // Distinct wording: 'expired' means the 15-minute window lapsed,
        // 'stale' means a NEWER search replaced this keyboard's results.
        await editMessage(chatId, messageId, pick.reason === 'expired'
          ? '⏰ Das ist zu lange her — schreib mir den Titel einfach nochmal.'
          : '🔄 Diese Liste ist nicht mehr aktuell — schreib mir den Titel einfach nochmal.');
      }
      return;
    }
    const { film, index } = pick;
    await answerCallbackQuery(callbackQueryId);
    ...
    // Build details + confirm button(s). Shows get a season-cutoff picker.
    ...
    let keyboard: any[][];
    if (film.mediaType === 'show') {
      detailText += '\nAb welcher Staffel soll ich laden?';
      // Every button carries the session nonce so a keyboard from an earlier
      // search can never be resolved against the current results.
      const sn = searchSessions.get(chatId)?.nonce ?? '';
      keyboard = [
        [{ text: '✅ Alle Staffeln', callback_data: `confirm:${sn}:${index}` }],
        [
          { text: 'Ab S2', callback_data: `confirm_season:${sn}:${index}:2` },
          { text: 'Ab S3', callback_data: `confirm_season:${sn}:${index}:3` },
          { text: 'Ab S4', callback_data: `confirm_season:${sn}:${index}:4` },
          { text: 'Ab S5', callback_data: `confirm_season:${sn}:${index}:5` },
          { text: 'Ab S6', callback_data: `confirm_season:${sn}:${index}:6` },
        ],
      ...
      detailText += '\nSoll ich den holen?';
      keyboard = [
        [{ text: '✅ Ja, bitte!', callback_data: `confirm:${searchSessions.get(chatId)?.nonce ?? ''}:${index}` }],
        [{ text: '🔙 Andere Auswahl', callback_data: 'back_to_results' }],
      ];
    }
  ...
  // Confirm add (all seasons) — uses the host-wide quality setting.
  if (data.startsWith('confirm:')) {
    if (!canMutate(chatId)) { await denyMutation(chatId, callbackQueryId); return; }
    const pick = resolveSessionPick(chatId, data.substring(8));
    if (!pick.ok) {
      await answerCallbackQuery(callbackQueryId, pick.reason === 'invalid' ? 'Ungueltige Auswahl' : 'Auswahl abgelaufen');
      if (pick.reason !== 'invalid') {
        // Distinct wording: 'expired' means the 15-minute window lapsed,
  ...
  // Confirm show with season cutoff: confirm_season:<index>:<season>
  if (data.startsWith('confirm_season:')) {
    if (!canMutate(chatId)) { await denyMutation(chatId, callbackQueryId); return; }
    // Payload is `<nonce>:<index>:<season>`.
    ...
    const pick = resolveSessionPick(chatId, `${parts[0]}:${parts[1]}`);
    if (!pick.ok) {
      await answerCallbackQuery(callbackQueryId, pick.reason === 'invalid' ? 'Ungueltige Auswahl' : 'Auswahl abgelaufen');
      if (pick.reason !== 'invalid') {
        // Distinct wording: 'expired' means the 15-minute window lapsed,
    ...
    const film = pick.film;
    searchSessions.delete(chatId);
    await answerCallbackQuery(callbackQueryId, `Ab Staffel ${season} — alles klar!`);

    await handleSelection(chatId, messageId, film, season);
    return;
  }
...
async function handleSelection(
  ...
  try {
    ...
    if (inserted?.id && seasonCutoff != null) {
      setSeasonCutoff(inserted.id, seasonCutoff);
    }

    trackRequester(chatId, film.title, film.year, inserted?.id);
    const seasonNote = seasonCutoff != null ? ` (ab S${String(seasonCutoff).padStart(2, '0')})` : '';
    addLogEntry(inserted?.id ?? null, 'movie_added', `Telegram: ${film.title}${film.year ? ' (' + film.year + ')' : ''} [${film.mediaType}]${seasonNote}`);
    ...
    const seasonHint = seasonCutoff != null ? ` <i>(ab Staffel ${seasonCutoff})</i>` : '';
    await editMessage(chatId, messageId,
      `${typeIcon} <b>${escapeHtml(film.title)}</b>${film.year ? ' (' + film.year + ')' : ''}${seasonHint}\n🔍 Ich schau mich um...`
    );
    trackProgress(chatId, messageId, film.title, film.year);

    logger.info(`Telegram: ${film.mediaType} requested - ${film.title} (${film.year || '—'})${seasonNote} by chat ${chatId}`);
