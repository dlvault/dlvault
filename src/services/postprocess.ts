...
function findMovieByJdPackageName(allMovies: Movie[], pkgName: string): Movie | null {
  for (const m of allMovies) {
    if (pkgName.startsWith(jdPackagePrefix(m))) return m;
  }
  return null;
}
...
async function moveShowContainer(
  ...
  for (const c of candidates) {
    ...
    if (season === null || episode === null || episode === undefined) {
      logger.warn(`${logPrefix}: ${movie.title} — could not parse S/E from "${path.basename(c.file)}" in "${container.name}" — skipping that file`);
      continue;
    }
    planned++;
  ...
  // Remove the source folder only when every planned episode moved.
  if (container.isDir && planned > 0 && done === planned) {
    ...
    try {
      ...
      for (const movie of downloadingMovies) {
        ...
        if (inLinkGrabber) {
          ...
          const titlePrefix = movie.year != null ? `${movie.title} (` : movie.title;
