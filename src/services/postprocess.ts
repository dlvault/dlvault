 ...
 */
function diagnoseDownloadMountMismatch(dlPackages: JDPackage[], downloadFolders: string[], downloadPath: string): void {
  ...
  if (jdownloaderService.isConfigured()) {
    try {
      ...
      if (ddPath && fs.existsSync(ddPath)) {
        ...
        diagnoseDownloadMountMismatch(dlPackages, ddFolders, ddPath);
      }
