 ...
 */
async function cleanupDeadLinkGrabberPackages(): Promise<void> {
  if (!jdownloaderService.isConfigured()) return;
  try {
    const lgPackages = await jdownloaderService.getLinkGrabberPackages();
    // null = JD unreachable (can't tell dead from "no info") — skip this round.
    ...
    logger.debug(`Linkgrabber janitor: query failed: ${err.message}`);
  }
}
...
async function runPostProcessCycle(): Promise<void> {
  ...
  await cleanupDeadLinkGrabberPackages();
