...
export function createBackup(): { filename: string; size: number } | null {
  try {
    ...
    //
    // Atomic write: write to a temp file then rename into place. A crash
    // mid-write leaves a stray .tmp (ignored by listBackups, which only lists
    // *.db) instead of a truncated final .db that could be picked for restore.
    const tmp = dest + '.tmp';
    fs.writeFileSync(tmp, db.serialize());
