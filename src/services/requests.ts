...
export async function requestTitle(
  ...
  const existing =
    ...
    // Media-type scoped: TMDb movie and TV ids overlap, so an unscoped match
    // returns a film for a requested show (and reports it as "already added").
    (input.tmdb_id ? getMovieByTmdbId(input.tmdb_id, input.media_type) : undefined);
  ...
  const movie = addMovie({
    ...
    // A trakt_id means it came from the Trakt watchlist sync; anything else
    // reaching this primitive is a user request (Telegram, request UI).
    watchlist_source: input.trakt_id ? 'trakt' : 'manual',
  });
