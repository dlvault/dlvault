...
export class TmdbService {
  ...
  /**
   * The imdb id behind a TMDb id. A title that arrived keyed by TMDb alone
   * (Seerr's Radarr/Sonarr contract carries no imdbId) needs it for the
   * id-based source search, the poster and the metadata backfill. Null when
   * unconfigured, unmapped or unreachable — callers fall back.
   */
  async getImdbId(type: 'movie' | 'tv', tmdbId: number): Promise<string | null> {
    if (!this.isConfigured() || !tmdbId) return null;
    try {
      const res = await this.getClient().get(`/${type}/${tmdbId}/external_ids`);
      const id = res.data?.imdb_id;
      return typeof id === 'string' && /^tt\d+$/i.test(id) ? id : null;
    } catch (error: any) {
      logger.debug(`TMDb: no external ids for ${type} ${tmdbId}: ${error?.message || error}`);
      return null;
    }
  }
