...
export async function getGermanTitleFromWikidata(imdbId: string): Promise<string | null> {
  // Guard against SPARQL injection — imdbId is interpolated into the query
  // string below. Trakt/OMDb supply well-formed `tt#######` ids; a value
  // carrying a `"` could otherwise break out of the literal and rewrite the
  // query. Reject anything that isn't a canonical IMDb id.
