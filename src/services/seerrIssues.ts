...
const YEAR_IN_NAME_RE = /(?:^|\D)(?:19|20)\d{2}(?:\D|$)/;

/**
 * Visits every library file belonging to a title — in its own folder OR flat in
 * the library root.
 *
 * The flat case is not an edge case: a library that lets the post-processor
 * rename ends up with "Title (Year).mkv" sitting directly in the root, which on
 * a real library is a large share of it. The previous walker skipped every
 * non-directory in the root, so for those titles quarantine found nothing and a
 * Seerr report did nothing — "quarantined 0 file(s)" in the log, with the flaw
 * still on disk.
 *
 * Matching requires every title word, and — when the name carries a year at all
 * — that it be the right one, so a sequel or remake next to it is left alone.
 */
function forEachLibraryFile(
  movie: Movie,
  ...
  const matchesTitle = (name: string): boolean => {
    const norm = normalizeTitle(name.replace(/\./g, ' '));
    if (!titleWords.every(w => norm.includes(w))) return false;
    // Only judge the year when the name states one — a hand-made folder often
    // does not, and demanding it there would make the report a no-op again.
    if (movie.year && YEAR_IN_NAME_RE.test(name) && !norm.includes(String(movie.year))) return false;
    return true;
  };
  // A season-scoped report must leave the other seasons alone.
  const inScope = (name: string): boolean =>
    seasonNumber == null || new RegExp(`s0*${seasonNumber}(?![0-9])`, 'i').test(name);
  ...
  for (const entry of entries) {
    if (!matchesTitle(entry.name)) continue;
    if (entry.isDirectory()) sweep(path.join(root, entry.name), 1);
    else if (inScope(entry.name)) visit(path.join(root, entry.name), entry.name);
  }
}
 ...
 *
 * This is a courtesy, not the protection against the library check undoing the
 * repair — that is the `repair` flag, which every library check now honours.
 * Quarantining alone never could be: the check asks Jellyfin/Plex, and they
 * keep listing a file until they rescan.
 *
 * @returns how many files were moved aside.
 */
function quarantineReportedFiles(movie: Movie, seasonNumber: number | null): number {
  let moved = 0;
  forEachLibraryFile(movie, seasonNumber, (full, name) => {
    if (!MEDIA_EXT_RE.test(name)) return;
    try {
      fs.renameSync(full, full + QUARANTINE_SUFFIX);
      moved++;
      logger.info(`Seerr issue: quarantined "${name}"`);
    } catch (error: any) {
      logger.warn(`Seerr issue: could not quarantine "${name}": ${error?.message || error}`);
    }
  });
  return moved;
}
...
export function restoreQuarantinedFiles(movie: Movie): number {
  let restored = 0;
  forEachLibraryFile(movie, null, (full, name) => {
    if (!name.endsWith(QUARANTINE_SUFFIX)) return;
    try {
      fs.renameSync(full, full.slice(0, -QUARANTINE_SUFFIX.length));
      restored++;
      logger.info(`Seerr issue: restored "${name}" — no replacement was found`);
    } catch (error: any) {
      logger.warn(`Seerr issue: could not restore "${name}": ${error?.message || error}`);
    }
  });
  return restored;
}
...
/**
 * Whether the title now holds a release other than the ones the report blocked.
 *
 * No recorded release at all means "cannot tell" — and an issue left open is a
 * better outcome than one closed on a promise nobody kept.
 */
function hasReplacementRelease(movie: Movie, seasonNumber: number | null): boolean {
  return getDownloadsByMovieId(movie.id).some(d => !!d.release_name
    && (seasonNumber == null || d.season_number == null || d.season_number === seasonNumber)
    && !isReleaseBlocklisted(d.release_name));
}
...
async function resolveFixedIssues(issues: SeerrIssue[]): Promise<number> {
  ...
  for (const issue of issues) {
    ...
    // Status is not evidence. `updateMovieStatus(…, 'downloaded')` clears the
    // repair flag as a side effect, so ANY check that flips a repairing title
    // back to downloaded also makes it look repaired — and dlvault then tells
    // the reporter their problem is solved while the same file sits on disk.
    // Seen in the field: filed, flipped back by the library check 18 minutes
    // later, closed as fixed a minute after that — with the reported file
    // untouched. Require the real thing: a release that is not one of the ones
    // this report blocked.
    if (!hasReplacementRelease(movie, issue.problemSeason || null)) continue;
