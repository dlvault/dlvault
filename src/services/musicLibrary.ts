...
/**
 * Which Jellyfin instance {@link cachedUserId} was resolved against. Without it
 * the id was memoized for the whole process lifetime with nothing clearing it —
 * not even invalidateMusicLibraryCache() — so repointing dlvault at a different
 * Jellyfin kept every album/track request addressed to a user id that server has
 * never heard of, and the Musik tab stayed empty until the container restarted.
 * Same trap the JDownloader device id had (see jdownloader/index.ts).
 */
let cachedUserIdFor: string | null = null;
...
export function invalidateMusicLibraryCache(): void {
  cache = null;
  // Drop the resolved user too, so this really is "forget what you cached".
  // Leaving it behind was how a stale id from a previous Jellyfin survived; the
  // re-resolve costs one cheap /Users call on the next read.
  cachedUserId = null;
  cachedUserIdFor = null;
}
...
async function getUserId(): Promise<string | null> {
  const instance = `${jellyfinUrl()}|${jellyfinApiKey()}`;
  if (cachedUserId && cachedUserIdFor === instance) return cachedUserId;
  cachedUserId = null;
  cachedUserIdFor = instance;
  try {
