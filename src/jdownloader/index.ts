...
export class JDownloaderService {
  ...
  /**
   * Short, non-secret description of a payload we failed to decrypt.
   *
   * The distinction worth logging is "this was never ciphertext" — an error page
   * or a plain-JSON body that came back with a 200, so axios never threw — versus
   * "right shape, wrong key", which means a token rotated under an in-flight
   * request. Only echoes the body when it is clearly not base64.
   */
  private describeCipherFailure(raw: unknown): string {
    if (typeof raw !== 'string') return `body is not a string (${typeof raw})`;
    const body = raw.trim();
    if (!body) return 'empty body';
    if (!/^[A-Za-z0-9+/]+={0,2}$/.test(body)) {
      return `body is not base64 (${body.length} chars): ${body.slice(0, 200)}`;
    }
    const bytes = Math.floor((body.length * 3) / 4);
    if (bytes === 0 || bytes % 16 !== 0) {
      return `base64 body is not a whole number of AES blocks (${bytes} bytes)`;
    }
    return `well-formed ciphertext (${bytes} bytes) — key mismatch`;
  }
  ...
  async connect(force = false): Promise<boolean> {
    ...
    // Safe to share even with a forced caller: doConnect always mints a brand-new
    // session server-side, so whoever joins gets a fresh token, never the dead one
    // that made them force. Opening a second connect here instead would only
    // multiply attempts during exactly the relay outages that trigger the forcing.
  ...
  /**
   * Why a reachability check failed — the monitor needs this to say something
   * TRUE to the user.
   *
   * `connect()` talks to api.jdownloader.org, NOT to JDownloader. A timeout
   * there means we never reached the relay, which says nothing at all about
   * whether JD is running: telling someone to restart it is then wrong advice
   * (live case 2026-08-24 — six minutes of relay timeouts while the JD container
   * had been up for 26 hours). Only once the relay answers can a missing device
   * be blamed on JD.
   */
  async probe(): Promise<{ state: 'ok' | 'relay_unreachable' | 'device_missing' }> {
    if (!(await this.connect())) return { state: 'relay_unreachable' };
    // Reuses getDeviceId so the device-name matching stays in one place.
    return { state: (await this.getDeviceId()) ? 'ok' : 'device_missing' };
  }
  ...
  async listDevices(retryCount = 0): Promise<JDDevice[]> {
    ...
    // Pin the session for the whole round trip. Reading `this.serverEncryptionToken`
    // again after the await was the bug behind the recurring
    // "bad decrypt" — every connect() rotates it, so a reconnect triggered by any
    // other caller while this request was in flight meant decrypting a response
    // with a key minted after it was encrypted. It then failed twice in a row and
    // reported "No JDownloader devices found".
    const sessionToken = this.sessionToken;
    const encryptionToken = this.serverEncryptionToken;

    try {
      const query = `/my/listdevices?sessiontoken=${sessionToken}`;
      const signature = this.sign(query, encryptionToken);
      const url = `${MY_JD_API}${query}&signature=${signature}`;

      const response = await axios.get(url, { responseType: 'text', timeout: 15000, transformResponse: [(data: string) => data] });
      let decrypted: any;
      try {
        decrypted = JSON.parse(this.decrypt(response.data, encryptionToken));
      } catch (cipherError: any) {
        throw new Error(`could not read the listdevices response — ${this.describeCipherFailure(response.data)}`);
      }
  ...
  private async callDevice(action: string, params?: any[], retryCount = 0): Promise<any> {
    ...
    // Same reason as listDevices: pin the session across the round trip so a
    // concurrent reconnect can't swap the key out from under the response.
    const sessionToken = this.sessionToken;
    const encryptionToken = this.deviceEncryptionToken;
    // Which session this request is being made with, so the 403 handler can tell
    // "my session went stale" from "someone already replaced it while I was in flight".
    const sessionStamp = this.lastConnectTime;
    ...
    const encrypted = this.encrypt(postData, encryptionToken);
    const path = `/t_${sessionToken}_${deviceId}${action}`;
    ...
    try {
      ...
      let payload: string;
      try {
        payload = this.decrypt(response.data, encryptionToken);
      } catch {
        // Rethrow rather than bail: a bare crypto error carries no HTTP status,
        // so the ladder below treats it as transient and retries with a freshly
        // read token — which is the recovery. Only the message needs enriching,
        // so the final log says what was wrong with the payload.
        throw new Error(`could not read the response — ${this.describeCipherFailure(response.data)}`);
      }
      ...
      // Re-auth on 403.
      //
      // /my/connect mints a new session and invalidates the previous one, so a
      // reconnect by ANY caller 403s every request already in flight. Those all
      // land within a second or two of each other (live: 15 TOKEN_INVALIDs in 5
      // tight clusters, up to 5 across 4 different actions inside 2s). If each
      // one then forces its own reconnect they invalidate each other in turn and
      // some are guaranteed to fail the retry too — which is exactly what the
      // give-up log showed. So only mint a session if nobody has replaced the one
      // we actually used; otherwise adopt the fresh one and just retry.
      if (status === 403 && retryCount === 0) {
        this.invalidateDevice();
        if (this.lastConnectTime === sessionStamp) {
          this.sessionToken = null;
          this.lastConnectTime = 0;
          await this.connect(true);
        } else {
      ...
      if (error.response?.data) {
        try {
          detail = this.decrypt(error.response.data, encryptionToken);
        } catch {
