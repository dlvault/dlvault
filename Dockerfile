...
# Install Chromium for Puppeteer (main scraper) + Xvfb for non-headless Playwright
...
# Data and logs directories + Xvfb permissions
...
CMD ["sh", "-c", "rm -f /tmp/.X99-lock && Xvfb :99 -screen 0 1280x800x24 -nolisten tcp &>/dev/null & export DISPLAY=:99 && node dist/server.js"]
