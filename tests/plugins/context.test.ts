...
describe('createPluginContext', () => {
  ...
  describe('challengeSolver', () => {
    beforeEach(() => { mockAxios.post.mockReset(); });
    ...
    const flareOk = {
      ...
      solution: {
        ...
        cookies: [
          { name: 'cf_clearance', value: 'abc123', domain: '.example.org', path: '/' },
          { name: 'other', value: 'x' },
        ],
      },
    };

    it('is always available and solves via the configured FlareSolverr URL', async () => {
      mockSettings['challenge_solver.url'] = 'http://solver:8191/';
      mockAxios.post.mockResolvedValueOnce({ data: flareOk });
      const ctx = createPluginContext(baseManifest);
      const res = await ctx.challengeSolver.solve('https://example.org/x');
      expect(res.ok).toBe(true);
      expect(res.userAgent).toBe('Mozilla/5.0 Chrome/148');
      expect(res.html).toBe('<html>solved</html>');
      expect(res.cookies.find(c => c.name === 'cf_clearance')?.value).toBe('abc123');
      // Trailing slash trimmed, /v1 appended, FlareSolverr command shape.
      const [url, body] = mockAxios.post.mock.calls[0];
      expect(url).toBe('http://solver:8191/v1');
      expect(body).toMatchObject({ cmd: 'request.get', url: 'https://example.org/x' });
    });

    it('falls back to the bundled default URL when unset', async () => {
      mockAxios.post.mockResolvedValueOnce({ data: flareOk });
      const ctx = createPluginContext(baseManifest);
      await ctx.challengeSolver.solve('https://example.org/x');
      expect(mockAxios.post.mock.calls[0][0]).toBe('http://127.0.0.1:8191/v1');
    });

    it('returns ok:false (never throws) when the solver errors', async () => {
    ...
    it('returns ok:false when the solver reports a non-ok status', async () => {
      mockAxios.post.mockResolvedValueOnce({ data: { status: 'error', message: 'timeout' } });
      const ctx = createPluginContext(baseManifest);
      const res = await ctx.challengeSolver.solve('https://example.org/x');
      expect(res.ok).toBe(false);
  ...
  it('secrets.get reads only declared secrets, rejecting malformed and undeclared keys', () => {
    mockSettings['secret-store.test-key'] = 'shared-value';
    mockSettings['secret-store.other-key'] = 'should-not-be-readable';
    const ctx = createPluginContext({ ...baseManifest, permissions: ['secrets'], requiredSecrets: [{ key: 'test-key', label: 'Test Key' }] });
    expect(ctx.secrets!.get('test-key')).toBe('shared-value');
    expect(ctx.secrets!.get('')).toBe('');
    expect(ctx.secrets!.get('../trakt.client_id')).toBe('');
    expect(ctx.secrets!.get('other-key')).toBe(''); // present in store but not declared → denied
  });
  ...
  it('exposes browser when "browser" permission is declared', () => {
