...
describe('reconcileEpisodesWithLibrary', () => {
  ...
  it('drift (B): upgrades pending episodes already present in the library', async () => {
    // Trakt pre-seeded the whole season as 'pending' before we looked at the library;
    // the files are actually all there. Reconcile must mark them downloaded so the
    // scheduler doesn't re-download what already exists. (The "Wednesday" bug.)
    mockProvider.getShowEpisodes.mockResolvedValue({
      found: true,
      episodes: new Set(['S01E01', 'S01E02', 'S01E03', 'S01E04']),
    });
    const show = makeShow();
    const season = addSeason(show.id, 1);
    addEpisodes(season.id, [1, 2, 3, 4]); // all default 'pending'
    testDb.prepare("UPDATE seasons SET status = 'not_found' WHERE id = ?").run(season.id);
  ...
  it('drift (B): partial — upgrades only the present pending episodes, leaves the gaps', async () => {
    mockProvider.getShowEpisodes.mockResolvedValue({
      found: true,
      episodes: new Set(['S01E01', 'S01E02']), // library has 2 of 4
    });
    const show = makeShow();
    const season = addSeason(show.id, 1);
    addEpisodes(season.id, [1, 2, 3, 4]); // all pending
    testDb.prepare("UPDATE seasons SET status = 'pending' WHERE id = ?").run(season.id);

    const r = await reconcileEpisodesWithLibrary(show);
    expect(r.addedCount).toBe(2);
    const byNum = Object.fromEntries(getEpisodesBySeasonId(season.id).map(e => [e.episode_number, e.status]));
    expect(byNum[1]).toBe('downloaded');
    expect(byNum[2]).toBe('downloaded');
    expect(byNum[3]).toBe('pending');
    expect(byNum[4]).toBe('pending');
    expect(getSeasonStatus(season.id)).toBe('pending');
  });
