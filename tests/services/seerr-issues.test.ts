...
vi.mock('../../src/database/services/blocklist', () => ({
  addBlocklistEntry: (e: any) => { blocked.add(e.release_name); return addBlocklistEntry(e); },
  isReleaseBlocklisted: (name: string) => blocked.has(name),
}));
...
describe('Seerr viewer reports', () => {
  ...
  describe('acting on a report', () => {
    ...
    it('moves aside a film that sits flat in the library root', async () => {
      // The post-processor renames to "Title (Year).mkv" straight into the root,
      // so a large share of a real library has no folder at all. The walker used
      // to skip every non-directory here: quarantine reported "0 file(s)" and the
      // reported file stayed exactly where it was.
      movies[0].year = 1999;
      fsMock.dirs['/movies'] = [{ name: 'Fight Club (1999).mkv', dir: false }];
      issues.value = [issue()];

      await processSeerrIssues();

      expect(fsMock.renamed).toEqual(['/movies/Fight Club (1999).mkv.incomplete']);
      expect(lastComment()).toContain('beiseitegelegt');
    });

    it('does not mistake a sequel in the same root for the reported film', async () => {
      movies[0].year = 1999;
      fsMock.dirs['/movies'] = [
        { name: 'Fight Club (1999).mkv', dir: false },
        { name: 'Fight Club 2 (2015).mkv', dir: false },
      ];
      issues.value = [issue()];
    ...
    it('puts a flat file back when the repair is abandoned', () => {
      movies[0].year = 1999;
      fsMock.dirs['/movies'] = [{ name: 'Fight Club (1999).mkv.incomplete', dir: false }];

      expect(restoreQuarantinedFiles(movies[0])).toBe(1);
      expect(fsMock.renamed).toEqual(['/movies/Fight Club (1999).mkv']);
    });
  ...
  describe('closing the loop', () => {
    ...
    it('stays open when the title was flipped back to downloaded without a new release', async () => {
      // updateMovieStatus(..., 'downloaded') clears the repair flag as a side
      // effect, so a library check that undoes a repair also makes the title look
      // repaired. Closing on that told a viewer their problem was fixed while the
      // same file sat on disk.
      blocked.add('Fight.Club.1999.GERMAN.1080p');
      movies[0].status = 'downloaded';
      movies[0].repair = 0;
      issues.value = [acted];
    ...
    it('closes once a release that is not the blocked one has landed', async () => {
      blocked.add('Fight.Club.1999.GERMAN.1080p');
      downloads.push({ id: 2, movie_id: 7, season_number: null, release_name: 'Fight.Club.1999.German.DL.1080p-NEW' });
      movies[0].status = 'downloaded';
      movies[0].repair = 0;
      issues.value = [acted];
