...
/** A second session, so a reconnect visibly rotates the derived tokens. */
const SESSION_TOKEN_2 = 'c'.repeat(64);
...
const serverEncryptionToken2 = updateTokens(loginSecret, SESSION_TOKEN_2);
const deviceEncryptionToken2 = updateTokens(deviceSecret, SESSION_TOKEN_2);
...
/** A /my/connect body that hands out the SECOND session, rotating the tokens. */
function connectResponse2() {
  return encryptedResponse(
    { sessiontoken: SESSION_TOKEN_2, regaintoken: REGAIN_TOKEN },
    loginSecret
  );
}
...
describe('JDownloaderService', () => {
  ...
  describe('listDevices', () => {
    ...
    it('recovers from an expired session by reconnecting once', async () => {
      // This was the one JD call that could not heal itself: on failure it kept
      // the dead sessionToken, so the next attempt reused it. With the session
      // TTL matching the five-minute JD monitor that produced a steady ~12
      // failures an hour — 388 over 33 hours on a live instance — while the
      // download calls beside it recovered through their 403 path.
      setCredentials();
      mockedAxios.get
        .mockResolvedValueOnce(connectResponse())            // initial connect
        .mockRejectedValueOnce(new Error('session expired')) // stale token
        .mockResolvedValueOnce(connectResponse())            // forced reconnect
    ...
    it('decrypts with the token the request was signed with, not one rotated mid-flight', async () => {
      // The live "Provider routines::bad decrypt" (59× in 16 days): the encryption
      // token was read off `this` twice, straddling the await. Every connect()
      // rotates it, so a reconnect fired by any other caller while this request
      // was in flight meant decrypting a response with a key minted after it was
      // encrypted. listDevices then failed twice in a row and the pipeline logged
      // "No JDownloader devices found".
      setCredentials();

      let deliverDevices!: (v: unknown) => void;
      const heldRequest = new Promise(resolve => { deliverDevices = resolve; });
      let devicesRequested!: () => void;
      const requestInFlight = new Promise<void>(resolve => { devicesRequested = resolve; });

      mockedAxios.get
        .mockResolvedValueOnce(connectResponse())                                  // initial connect → token A
        .mockImplementationOnce(() => { devicesRequested(); return heldRequest as any; })
        .mockResolvedValueOnce(connectResponse2());                                // someone else reconnects → token B

      const devicesPromise = service.listDevices();
      await requestInFlight;

      // A concurrent caller rotates the session while our request is still open.
      await service.connect(true);
      expect((service as any).serverEncryptionToken.equals(serverEncryptionToken2)).toBe(true);

      // The relay answers the ORIGINAL request — encrypted with token A.
      deliverDevices(listDevicesResponse([DEFAULT_DEVICE]));

      expect(await devicesPromise).toEqual([DEFAULT_DEVICE]);
    });

    it('reports why a response could not be decrypted instead of a bare crypto error', async () => {
      setCredentials();
      mockedAxios.get
        .mockResolvedValueOnce(connectResponse())
        .mockResolvedValueOnce({ data: '<html>502 Bad Gateway</html>' })  // 200, but not ciphertext
        .mockResolvedValueOnce(connectResponse())
      ...
      const message = vi.mocked(logger.error).mock.calls.at(-1)?.[0] as string;
      expect(message).toContain('body is not base64');
    ...
    it('gives up after one retry rather than looping', async () => {
      ...
      expect(await service.listDevices()).toEqual([]);
      // connect, list, connect, list — no third attempt.
  ...
  describe('callDevice 403 re-auth', () => {
    /** An axios-style 403 the way the MyJD relay sends TOKEN_INVALID. */
    ...
    it('adopts a session another caller already refreshed instead of minting a third', async () => {
      // Live signature: /my/connect invalidates the previous session, so ONE
      // reconnect 403s every request already in flight — 15 TOKEN_INVALIDs came
      // in 5 tight clusters, up to 5 across 4 actions inside 2s. When each of
      // those forced its own reconnect they invalidated one another and some
      // failed the retry too.
      await primeConnected(service);
      const stamp = (service as any).lastConnectTime;

      mockedAxios.post
        .mockImplementationOnce(async () => {
          // A concurrent caller's reconnect lands mid-flight.
          (service as any).sessionToken = SESSION_TOKEN_2;
          (service as any).serverEncryptionToken = serverEncryptionToken2;
          (service as any).deviceEncryptionToken = deviceEncryptionToken2;
          (service as any).lastConnectTime = stamp + 1000;
          throw tokenInvalid();
        })
        .mockResolvedValueOnce(encryptedResponse({ data: [] }, deviceEncryptionToken2));

      // The 403 path invalidates the device, so the retry re-resolves it.
      mockedAxios.get.mockResolvedValueOnce(
        encryptedResponse({ list: [DEFAULT_DEVICE] }, serverEncryptionToken2),
      );

      expect(await service.getDownloadPackages()).toEqual([]);

      const connects = mockedAxios.get.mock.calls.filter(c => String(c[0]).includes('/my/connect'));
      expect(connects).toHaveLength(0);
    });

    it('still mints a new session when nothing else refreshed it', async () => {
      await primeConnected(service);

      mockedAxios.post
        .mockRejectedValueOnce(tokenInvalid())
        .mockResolvedValueOnce(deviceResponse({ data: [] }));
      mockedAxios.get
        .mockResolvedValueOnce(connectResponse())                 // forced /my/connect
        .mockResolvedValueOnce(listDevicesResponse([DEFAULT_DEVICE]));
  ...
  describe('getDeviceId', () => {
    ...
    it('re-resolves when the configured device name changes', async () => {
      // The id was memoized for the process lifetime with nothing clearing it,
      // so pointing dlvault at a different JD instance kept every call addressed
      // to the old device until the container was restarted.
      const first = await primeConnected(service);
      expect(first).toBe('dev-1');

      mockSettings['jdownloader.device_name'] = 'Other';
      mockedAxios.get.mockResolvedValueOnce(
        listDevicesResponse([DEFAULT_DEVICE, { id: 'dev-2', name: 'Other', type: 'jd' }]),
      );

      const second = await (service as any).getDeviceId();
      expect(second).toBe('dev-2');
    });

    it('forgets the device on an explicit invalidation', async () => {
      await primeConnected(service);
      (service as any).invalidateDevice();

      mockedAxios.get.mockResolvedValueOnce(
        listDevicesResponse([{ id: 'dev-new', name: 'JD-Main', type: 'jd' }]),
      );

      const id = await (service as any).getDeviceId();
      expect(id).toBe('dev-new');
    });
  ...
  describe('addLinks', () => {
    ...
    it('does NOT dedup a 2160p upgrade against the 1080p package it replaces', async () => {
      await primeConnected(service);
      // The original is still in JD's list while the upgrade is sent. Folding the
      // resolution into the dedup prefix made this match, so addLinks answered
      // 'sent' without adding anything — the caller then flipped the movie to
      // 'downloading' and the upgrade silently never happened.
      mockedAxios.post
        .mockResolvedValueOnce(deviceResponse({ data: [{ uuid: 1, name: 'Movie (2024) - 1080p', bytesTotal: 9999, childCount: 5 }] }))
        .mockResolvedValueOnce(deviceResponse({ data: [] }))
        .mockResolvedValueOnce(deviceResponse({ data: { id: 1 } }));

      const ok = await service.addLinks(['http://a'], 'Movie (2024) - 2160p [UPGRADE]');
      expect(ok).toBe('sent');
      // Third call = the links were actually added.
      expect(mockedAxios.post).toHaveBeenCalledTimes(3);
    });

    it('does NOT dedup a season upgrade across resolutions either', async () => {
      await primeConnected(service);
      mockedAxios.post
        .mockResolvedValueOnce(deviceResponse({ data: [{ uuid: 1, name: 'Show (2020) - S02 - 720p', bytesTotal: 9999, childCount: 5 }] }))
        .mockResolvedValueOnce(deviceResponse({ data: [] }))
        .mockResolvedValueOnce(deviceResponse({ data: { id: 1 } }));

      const ok = await service.addLinks(['http://a'], 'Show (2020) - S02 - 1080p');
      expect(ok).toBe('sent');
      expect(mockedAxios.post).toHaveBeenCalledTimes(3);
    });
