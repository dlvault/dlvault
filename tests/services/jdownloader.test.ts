...
describe('JDownloaderService', () => {
  ...
  describe('device queries (connected)', () => {
    ...
    it('getSpeed should return decrypted state', async () => {
      const state = { speed: 1234 };
      mockedAxios.post.mockResolvedValueOnce(deviceResponse(state));
      expect(await service.getSpeed()).toEqual(state);
    });
    ...
    it('getSpeedLimit should return numeric value', async () => {
      mockedAxios.post.mockResolvedValueOnce(deviceResponse(3000));
    ...
    it('isSpeedLimited should return true when value is true', async () => {
      mockedAxios.post.mockResolvedValueOnce(deviceResponse(true));
      expect(await service.isSpeedLimited()).toBe(true);
    });

    it('isSpeedLimited should return false for non-true value', async () => {
      mockedAxios.post.mockResolvedValueOnce(deviceResponse(false));
    ...
    it('configureExtractionOverwrite should return true', async () => {
      mockedAxios.post.mockResolvedValueOnce(deviceResponse({ data: null }));
      expect(await service.configureExtractionOverwrite()).toBe(true);
    });
  ...
  describe('callDevice error handling', () => {
    ...
    it('should give up after max retries on persistent transient errors', async () => {
      ...
      await vi.runAllTimersAsync();
      const result = await promise;
      expect(result).toEqual([]); // null -> getDownloadPackages returns []
      // initial + 3 retries
