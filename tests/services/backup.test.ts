...
vi.mock('../../src/database/index', () => ({
  default: { serialize: vi.fn(() => Buffer.from('serialized-db-image')) },
...
beforeEach(() => {
  ...
  vi.mocked(fs.readFileSync).mockReturnValue(Buffer.from('') as any);
...
describe('createBackup', () => {
  it('creates the backup dir when missing and writes a .tar.gz archive', () => {
    mockFs.existsSync.mockReturnValue(false);
    ...
    // Archive, not a bare database: it also carries the encryption key and the
    // installed plugins, without which a restore cannot produce a working instance.
    expect(result!.filename).toMatch(/^dlvault-.*\.tar\.gz$/);
  ...
  it('writes the archive atomically (tmp + rename, no -wal/-shm)', () => {
    mockFs.existsSync.mockImplementation((p: any) => String(p).endsWith('backups'));
    createBackup();

    // Written atomically: build to a .tmp, then rename into the final archive.
    const writeTargets = mockFs.writeFileSync.mock.calls.map(c => String(c[0]));
    expect(writeTargets.some(t => t.endsWith('.tar.gz.tmp'))).toBe(true);
    expect(writeTargets.some(t => t.endsWith('-wal'))).toBe(false);
    expect(writeTargets.some(t => t.endsWith('-shm'))).toBe(false);
    const renameTargets = mockFs.renameSync.mock.calls.map(c => String(c[1]));
    expect(renameTargets.some(t => t.endsWith('.tar.gz'))).toBe(true);
    // No raw file copy of the live WAL-mode DB.
    expect(mockFs.copyFileSync).not.toHaveBeenCalled();
  });

  it('returns null and logs when writing fails', () => {
  ...
  it.each([
    ...
    ['abc', 'non-numeric'],
    ['-3', 'negative'],
  ])('falls back to 5 instead of wiping every backup when max_backups is %s (%s)', (value) => {
    // `files.slice(0)` and `files.slice(NaN)` are both the ENTIRE list, so an
    // unvalidated setting deleted every backup — including the one just written.
    mockSettings['backup.max_backups'] = value;
    mockFs.existsSync.mockImplementation((p: any) => String(p).endsWith('backups'));
    mockFs.readdirSync.mockReturnValue(
      Array.from({ length: 7 }, (_, i) => `dlvault-2026-0${i + 1}.db`) as any,
    );
    ...
    // 7 files, floor of 5 kept → exactly 2 deletions, never 7.
    expect(mockFs.unlinkSync).toHaveBeenCalledTimes(2);
  });
...
describe('startBackupScheduler / stopBackupScheduler', () => {
  ...
  it('uses a fixed 03:00 daily cron for intervals of 24h or more', () => {
    mockSettings['backup.enabled'] = 'true';
  ...
  it('defaults interval to 24h when unset (daily at 03:00)', () => {
    mockSettings['backup.enabled'] = 'true';
    mockFs.existsSync.mockReturnValue(true);
    mockFs.readdirSync.mockReturnValue(['dlvault-existing.db'] as any);
  ...
  it('falls back to daily when interval_hours is non-numeric', () => {
    mockSettings['backup.enabled'] = 'true';
    mockSettings['backup.interval_hours'] = 'abc';
    mockFs.existsSync.mockReturnValue(true);
    mockFs.readdirSync.mockReturnValue(['dlvault-existing.db'] as any);
  ...
  it('creates an initial backup when none exist yet', () => {
    ...
    startBackupScheduler();
    expect(mockFs.writeFileSync).toHaveBeenCalled(); // initial createBackup ran
  });
  ...
  it('the scheduled callback runs a backup', () => {
    mockSettings['backup.enabled'] = 'true';
    mockFs.existsSync.mockReturnValue(true);
    mockFs.readdirSync.mockReturnValue(['dlvault-existing.tar.gz'] as any);
