...
describe('createBackup', () => {
  it('creates the backup dir when missing and writes the serialized db file', () => {
    mockFs.existsSync.mockReturnValue(false);
  ...
  it('writes a single self-contained .db image atomically (tmp + rename, no -wal/-shm)', () => {
    mockFs.existsSync.mockImplementation((p: any) => String(p).endsWith('backups'));
    createBackup();

    // Written atomically: serialize to a .tmp, then rename into the final .db.
    const writeTargets = mockFs.writeFileSync.mock.calls.map(c => String(c[0]));
    expect(writeTargets.some(t => t.endsWith('.db.tmp'))).toBe(true);
    ...
    expect(renameTargets.some(t => t.endsWith('.db'))).toBe(true);
