...
describe('TraktService', () => {
  ...
  beforeEach(() => {
    ...
    // A client_id is what "Trakt is set up" means: every network method now
    // refuses to fire without one, so an unconfigured provider stays completely
    // inert instead of logging failures on an instance that never uses Trakt.
    // Tests covering the unconfigured case clear it again explicitly.
    mockSettings['trakt.client_id'] = 'test-client-id';
  ...
  describe('getMovieWatchlist', () => {
    ...
    it('fetches all pages when the watchlist is paginated', async () => {
      mockSettings['trakt.username'] = 'user';
      mockSettings['trakt.client_id'] = 'id';

      const page1 = [{ movie: { title: 'A', year: 2024, ids: { trakt: 1 } } }];
      const page2 = [{ movie: { title: 'B', year: 2023, ids: { trakt: 2 } } }];
      // Page 1 advertises 2 total pages via the Trakt pagination header — the loop
      // must follow it and concatenate, else page 2 would be silently dropped.
      mockedAxios.get.mockResolvedValueOnce({ data: page1, headers: { 'x-pagination-page-count': '2' } });
      mockedAxios.get.mockResolvedValueOnce({ data: page2, headers: { 'x-pagination-page-count': '2' } });
  ...
  describe('getTranslation', () => {
    it('should return null without a client id (no request made)', async () => {
      delete mockSettings['trakt.client_id'];
...
// ---------------------------------------------------------------------------
// syncWatchlist's deletion pass. Everything here exists to stop a bad sync from
// emptying the user's queue, and none of it was covered.
// ---------------------------------------------------------------------------
describe('TraktService — deleting entries that left the watchlist', () => {
  let service: TraktService;
  const tracked = (over: Record<string, unknown> = {}) => ({
    id: 1, title: 'Alt', year: 2020, trakt_id: 111,
    status: 'pending', watchlist_source: 'trakt', media_type: 'movie', ...over,
  }) as any;

  /** One watchlist entry, shaped the way Trakt returns it. */
  const entry = (traktId: number) => ({
    movie: { title: 'Neu', year: 2024, ids: { trakt: traktId, imdb: `tt${traktId}`, tmdb: traktId } },
  });
  ...
  beforeEach(() => {
    ...
    service = new TraktService();
    vi.mocked(moviesSvc.getMovieByTraktId).mockReturnValue({ id: 9 } as any);   // already tracked → no adds
  });

  function watchlists(movies: any[] | null, shows: any[] | null = []) {
    vi.spyOn(service, 'getMovieWatchlist').mockResolvedValue(movies as any);
    vi.spyOn(service, 'getShowWatchlist').mockResolvedValue(shows as any);
  }

  it('removes a tracked title that is no longer on the watchlist', async () => {
    vi.mocked(moviesSvc.getAllMovies).mockReturnValue([tracked()] as any);
    watchlists([entry(999)]);   // 111 is gone
  ...
  it('deletes nothing when the watchlist fetch failed', async () => {
    // An empty id set would otherwise read as "everything was removed".
    vi.mocked(moviesSvc.getAllMovies).mockReturnValue([tracked()] as any);
    watchlists(null);

    await service.syncWatchlist();

    expect(moviesSvc.deleteMovie).not.toHaveBeenCalled();
  });

  it('deletes nothing when only the show fetch failed', async () => {
    vi.mocked(moviesSvc.getAllMovies).mockReturnValue([tracked()] as any);
    watchlists([entry(999)], null);
  ...
  it('defers deletion for one cycle when the id count collapses', async () => {
    // Guards against a silently truncated fetch mass-deleting real entries.
    vi.mocked(moviesSvc.getAllMovies).mockReturnValue([tracked()] as any);

    // First sync: 30 ids INCLUDING our title, so nothing is due for removal and
    // prevCount lands at 30.
    watchlists([entry(111), ...Array.from({ length: 29 }, (_, i) => entry(1000 + i))]);
    await service.syncWatchlist();
    expect(moviesSvc.deleteMovie).not.toHaveBeenCalled();

    watchlists([entry(1000)]);                           // 30 → 1
    await service.syncWatchlist();
    expect(moviesSvc.deleteMovie).not.toHaveBeenCalled();

    // The new count was still recorded, so the next honest sync proceeds.
    watchlists([entry(1000)]);
    await service.syncWatchlist();
    expect(moviesSvc.deleteMovie).toHaveBeenCalledWith(1);
  });

  it('leaves manually added titles alone', async () => {
    vi.mocked(moviesSvc.getAllMovies).mockReturnValue([tracked({ trakt_id: null })] as any);
    watchlists([entry(999)]);
  ...
  it('leaves entries another watchlist owns alone', async () => {
    // In `both` mode the Plex sync runs right after: deleting its entries here
    // made the two fight, re-adding and re-searching the same titles every cycle.
    vi.mocked(moviesSvc.getAllMovies).mockReturnValue([tracked({ watchlist_source: 'plex' })] as any);
    watchlists([entry(999)]);
  ...
  it('leaves a title alone while processMovie is holding it', async () => {
    // Deleting the row mid-search makes its later status/log writes fail a
  ...
  it.each(['searching', 'found', 'downloading', 'downloaded'])(
    'leaves a title in %s alone even when it left the watchlist', async (status) => {
      vi.mocked(moviesSvc.getAllMovies).mockReturnValue([tracked({ status })] as any);
      watchlists([entry(999)]);
