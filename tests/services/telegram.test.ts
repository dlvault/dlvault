...
const resetRetryCountMock = vi.fn();
...
vi.mock('../../src/database/services/movies', () => ({
  ...
  resetRetryCount: (...a: any[]) => resetRetryCountMock(...a),
  updateMovieStatus: (...a: any[]) => updateMovieStatusMock(...a),
}));
...
beforeEach(() => {
  ...
  resetRetryCountMock.mockClear();
  updateMovieStatusMock.mockClear();
...
/**
 * The callback_data the bot most recently put on a button starting with
 * `prefix`. Search keyboards carry a per-session nonce (`select:<nonce>:<i>`)
 * so a stale keyboard can't resolve against a newer search, which means tests
 * must use the emitted value rather than a hardcoded `select:0`.
 */
function lastCallbackData(prefix: string): string {
  for (const call of [...mockedAxios.post.mock.calls].reverse()) {
    const kb = (call[1] as any)?.reply_markup?.inline_keyboard;
    if (!Array.isArray(kb)) continue;
    for (const row of kb) {
      for (const btn of row) {
        if (typeof btn?.callback_data === 'string' && btn.callback_data.startsWith(prefix)) {
          return btn.callback_data;
        }
      }
    }
  }
  throw new Error(`no button found with callback_data starting "${prefix}"`);
}
...
describe('pollLoop — access requests (self-service unlock)', () => {
  it('prompts an unknown chat and pings the admin once (deduped)', async () => {
    await runPollOnce([
      textUpdate(999, 'Inception', 1),
      textUpdate(999, 'still me', 2),
    ], { chatIds: '111' });

    const sends = mockedAxios.post.mock.calls.filter(c => String(c[0]).endsWith('/sendMessage'));
    const toStranger = sends.filter(c => (c[1] as any).chat_id === 999);
    const toAdmin = sends.filter(c => (c[1] as any).chat_id === 111);
    // Exactly one prompt + one admin ping, even though two messages arrived.
    expect(toStranger).toHaveLength(1);
    expect((toStranger[0][1] as any).text).toContain('privat');
    expect(toAdmin).toHaveLength(1);
    expect((toAdmin[0][1] as any).reply_markup.inline_keyboard[0][0].callback_data).toBe('approve:999');
  });

  it('approve: adds the chat to the allowlist and welcomes the new user', async () => {
    await runPollOnce([callbackUpdate(111, 'approve:999')], { chatIds: '111' });
    expect(vi.mocked(setSetting)).toHaveBeenCalledWith('telegram.allowed_chat_ids', '111,999');
    const sends = mockedAxios.post.mock.calls.filter(c => String(c[0]).endsWith('/sendMessage'));
    expect(sends.some(c => (c[1] as any).chat_id === 999 && (c[1] as any).text.includes('freigeschaltet'))).toBe(true);
  });

  it('approve: is refused for a non-admin chat', async () => {
    // 111 is the admin (first allowlist entry); 222 is just a friend.
    await runPollOnce([callbackUpdate(222, 'approve:999')], { chatIds: '111,222' });
    expect(vi.mocked(setSetting)).not.toHaveBeenCalledWith('telegram.allowed_chat_ids', expect.stringContaining('999'));
    const answered = mockedAxios.post.mock.calls.some(
      c => String(c[0]).endsWith('/answerCallbackQuery') && String((c[1] as any).text).includes('Betreiber'));
...
describe('pollLoop — free-text search', () => {
  it('shows results from plugin search with an inline keyboard', async () => {
    ...
    expect(kb[0][0].callback_data).toMatch(/^select:[a-z0-9]+:0$/);
  });
  ...
  it('falls back to OMDb search when plugins return nothing', async () => {
    ...
    const kbCall = mockedAxios.post.mock.calls.find(
      c => String(c[0]).endsWith('/sendMessage') && (c[1] as any).reply_markup?.inline_keyboard
        && /^select:[a-z0-9]+:0$/.test((c[1] as any).reply_markup.inline_keyboard[0]?.[0]?.callback_data ?? '')
    );
    expect(kbCall).toBeTruthy();
  });
...
describe('pollLoop — callback queries', () => {
  ...
  it('select with an expired session tells the user it expired', async () => {
    ...
    await runPollOnce([callbackUpdate(111, 'select:zz:0')]);
    const edit = postCallFor('editMessageText');
  ...
  it('select → confirm flow adds the chosen film', async () => {
    ...
    let served = 0;
    // Lazy: the nonce only exists once the search has produced its keyboard.
    const batches: Array<() => any[]> = [
      () => [textUpdate(111, 'Inception', 1)],
      () => [callbackUpdate(111, lastCallbackData('select:'), 2)],
      () => [callbackUpdate(111, lastCallbackData('confirm:'), 3)],
    ];
    configure({ chatIds: '111' });
    mockedAxios.get.mockImplementation(async (u: any) => {
      if (String(u).endsWith('/getUpdates')) {
  ...
  it('rejects a non-numeric select index (NaN guard) without loading details or adding', async () => {
    ...
    let served = 0;
    // Real nonce, bad index — so the payload reaches the index guard rather than
    // being rejected earlier as a stale keyboard.
    const batches: Array<() => any[]> = [
      () => [textUpdate(111, 'Inception', 1)],
      () => [callbackUpdate(111, `select:${lastCallbackData('select:').split(':')[1]}:bad`, 2)],
    ];
    configure({ chatIds: '111' });
    mockedAxios.get.mockImplementation(async (u: any) => {
      if (String(u).endsWith('/getUpdates')) {
...
describe('pollLoop — select renders single confirm button', () => {
  it('select shows a "Ja, bitte!" button and the host-wide quality applies on confirm', async () => {
    ...
    // Detail screen should carry confirm:<nonce>:0 (not qual:0:*)
    const edits = mockedAxios.post.mock.calls.filter(c => String(c[0]).endsWith('/editMessageText'));
    const detailScreen = edits.find(c => {
      const kb = (c[1] as any).reply_markup?.inline_keyboard;
      return /^confirm:[a-z0-9]+:0$/.test(kb?.[0]?.[0]?.callback_data ?? '');
    });
...
// ════════════════════════════════════════════════════════════════════════════════
// retry: callback (re-queue a not-found / gave-up item)
// ════════════════════════════════════════════════════════════════════════════════

describe('pollLoop — retry: callback', () => {
  it('resets the retry counter, re-queues and live-tracks the message', async () => {
    setMovies([{ id: 5, title: 'Tenet', year: 2020, status: 'not_found', media_type: 'movie' }]);
    await runPollOnce([callbackUpdate(111, 'retry:5')], { chatIds: '111' });
    expect(resetRetryCountMock).toHaveBeenCalledWith(5);
    expect(updateMovieStatusMock).toHaveBeenCalledWith(5, 'pending');
    const edit = postCallFor('editMessageText');
    expect((edit![1] as any).text).toContain('nochmal');
  });

  it('refuses retry of an item the chat does not own (non-admin)', async () => {
    setMovies([{ id: 9, title: 'Heat', year: 1995, status: 'not_found', media_type: 'movie' }]);
    await runPollOnce([callbackUpdate(222, 'retry:9')], { chatIds: '111,222' });
    expect(resetRetryCountMock).not.toHaveBeenCalled();
    const answered = mockedAxios.post.mock.calls.some(
      c => String(c[0]).endsWith('/answerCallbackQuery') && (c[1] as any).text === 'Nicht deine Anfrage');
    expect(answered).toBe(true);
  });
});

// ════════════════════════════════════════════════════════════════════════════════
// rm:<id> ownership scoping
// ════════════════════════════════════════════════════════════════════════════════

describe('pollLoop — rm: ownership', () => {
  it('refuses to remove an item the chat does not own (non-admin)', async () => {
    setMovies([{ id: 7, title: 'Heat', year: 1995, status: 'pending', media_type: 'movie' }]);
    await runPollOnce([callbackUpdate(222, 'rm:7')], { chatIds: '111,222' });
    expect(deleteMovieMock).not.toHaveBeenCalled();
    const answered = mockedAxios.post.mock.calls.some(
      c => String(c[0]).endsWith('/answerCallbackQuery') && (c[1] as any).text === 'Nicht deine Anfrage');
    expect(answered).toBe(true);
...
// ════════════════════════════════════════════════════════════════════════════════
// notifyGaveUp — once-only "we gave up" message with retry button
// ════════════════════════════════════════════════════════════════════════════════

describe('notifyGaveUp', () => {
  it('tells the requester once, with a retry button, and dedupes the second call', async () => {
    // /add wires chat 111 as the requester of the new movie (id 1 from addMovieMock).
    await runPollOnce([textUpdate(111, '/add Foo')]);
    mockedAxios.post.mockClear();

    await notifyGaveUp({ id: 1, title: 'Foo', year: null, media_type: 'movie' });
    const send = mockedAxios.post.mock.calls.find(
      c => String(c[0]).endsWith('/sendMessage') && (c[1] as any).reply_markup?.inline_keyboard);
    expect(send).toBeTruthy();
    expect((send![1] as any).chat_id).toBe(111);
    expect((send![1] as any).reply_markup.inline_keyboard[0][0].callback_data).toBe('retry:1');

    // Second call is a no-op (gaveUpNotified guard).
    mockedAxios.post.mockClear();
    await notifyGaveUp({ id: 1, title: 'Foo', year: null, media_type: 'movie' });
    expect(mockedAxios.post).not.toHaveBeenCalled();
  });

  it('stays silent when nobody requested the movie', async () => {
    configure({ chatIds: '111' });
    await notifyGaveUp({ id: 4242, title: 'Unwanted', year: 2001, media_type: 'movie' });
    expect(mockedAxios.post).not.toHaveBeenCalled();
  });
});

// ---------------------------------------------------------------------------
// canMutate is the gate in front of every action that queues or deletes. Unlike
// isChatAllowed it NEVER fails open: with no allowlist configured, mutations are
// denied for everyone, so a stranger who finds the bot cannot start downloads.
// ---------------------------------------------------------------------------
describe('Telegram Bot — the mutation gate', () => {
  const MUTATING = ['retry:1', 'rm:1', 'add_manual:Inception', 'confirm:abc:0', 'confirm_season:abc:0:2'];

  // A chat that is not on a configured allowlist never reaches this handler at
  // all — it is intercepted earlier by the access-request flow. The gate's real
  // job is the OPEN-MODE case below, where reading is deliberately allowed.
  it.each(MUTATING)('refuses %s even in open mode, where reading is allowed', async (data) => {
    // The whole point: no allowlist means a fresh install stays browsable, but
    // nobody gets to queue or delete until the operator says so.
    await runPollOnce([callbackUpdate(999, data)], { chatIds: '' });

    const send = postCallFor('sendMessage');
    expect((send![1] as any).text).toContain('noch nicht freigeschaltet');
  });

  it('lets an allowlisted chat through the gate', async () => {
    await runPollOnce([callbackUpdate(111, 'retry:99999')], { chatIds: '111' });

    const send = postCallFor('sendMessage');
    expect(send ? (send[1] as any).text : '').not.toContain('noch nicht freigeschaltet');
  });
});
