...
describe('PlexService', () => {
  ...
  describe('syncWatchlist', () => {
    ...
    it('tracks an item that carries only an IMDb id', async () => {
      // Fresh Discover entries often have no TMDb id yet. These used to be
      // dropped on every sync (the id was borrowed as a trakt_id substitute, so
      // "no TMDb id" meant "no identity"), and the title silently never appeared.
      mockSettings['plex.token'] = 'token123';
      ...
      mockedAxios.get.mockResolvedValueOnce({
        data: {
          MediaContainer: {
            totalSize: 1,
            Metadata: [
              { title: 'Imdb Only Movie', year: 2024, ratingKey: '1', type: 'movie', Guid: [{ id: 'imdb://tt999' }] },
            ],
          },
        },
      });
    ...
    it('never writes the TMDb id into the trakt_id column', async () => {
      // The old code stored tmdbId as a "unique identifier substitute" in
      // trakt_id, which collided with real Trakt ids and made Trakt's deletion
      // sweep delete Plex entries on every cycle.
      mockSettings['plex.token'] = 'token123';
      ...
      mockedAxios.get.mockResolvedValueOnce({
        data: {
          MediaContainer: {
            totalSize: 1,
            Metadata: [
              { title: 'Some Movie', year: 2024, ratingKey: '1', type: 'movie', Guid: [{ id: 'tmdb://27205' }] },
            ],
          },
        },
      });
    ...
    it('should remove movies no longer on watchlist when provider is plex', async () => {
      ...
      // Post-migration shape: Plex-owned rows are tagged and carry no trakt_id
      // (the old TMDb-id-as-trakt_id substitution is what the migration clears).
      vi.mocked(getAllMovies).mockReturnValue([
        { id: 1, trakt_id: null, tmdb_id: 111, title: 'Old Movie', year: 2020, status: 'pending', watchlist_source: 'plex' },
        { id: 2, trakt_id: null, tmdb_id: 222, title: 'Still Here', year: 2021, status: 'pending', watchlist_source: 'plex' },
      ] as any);
