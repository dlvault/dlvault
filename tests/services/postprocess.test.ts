...
vi.mock('../../src/jdownloader/index', () => ({
  jdownloaderService: {
    ...
    getDownloadPackages: vi.fn(() => Promise.resolve(jdState.downloadPackages)),
    getLinkGrabberPackages: vi.fn(() => Promise.resolve(jdState.linkGrabberPackages)),
...
describe('PostProcessor — runPostProcessCycle (orchestration)', () => {
  ...
  it('skips a folder that is still extracting (archive present)', async () => {
    configurePaths();
    ...
    // Not moved — folder still present, no downloaded status set
    expect(fs.existsSync(path.join(folder, 'movie.mkv'))).toBe(true);
  ...
  it('does NOT clean up download folders when not confirmed in library', async () => {
    ...
    fs.mkdirSync(leftover);
    writeMedia(path.join(leftover, 'movie.mkv'), 400);
    // Archive file keeps it "still extracting" → checkCompletedDownloads won't move it
    // and cleanup defers it; combined with empty library (not confirmed) it must survive.
...
describe('PostProcessor — syncDownloadingStatus (JD + library)', () => {
  ...
  it('detects offline via individual linkgrabber link availability', async () => {
    ...
    expect(updateMovieStatus).toHaveBeenCalledWith(1, 'not_found');
  });
