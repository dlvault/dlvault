...
describe('OMDb Service', () => {
  ...
  describe('findImdbId', () => {
    it('returns null for an empty title without calling the API', async () => {
      mockSettings['omdb.api_key'] = 'KEY';
      expect(await findImdbId('')).toBeNull();
      expect(mockedAxios.get).not.toHaveBeenCalled();
    });

    it('returns null when no API key is configured', async () => {
      expect(await findImdbId('Kraken', 2025)).toBeNull();
      expect(mockedAxios.get).not.toHaveBeenCalled();
    });

    it('resolves the imdb id via an exact-title lookup with title, type and year', async () => {
      mockSettings['omdb.api_key'] = 'KEY';
      mockedAxios.get.mockResolvedValueOnce(ok({ imdbID: 'tt12345', Type: 'movie' }));
      const id = await findImdbId('Kraken Find', 2025);
      expect(id).toBe('tt12345');
      expect(mockedAxios.get).toHaveBeenCalledWith(
        'https://www.omdbapi.com/',
        expect.objectContaining({ params: expect.objectContaining({ t: 'Kraken Find', type: 'movie', y: '2025' }) }),
      );
    });

    it('omits the year param when none is given and supports series type', async () => {
      mockSettings['omdb.api_key'] = 'KEY';
      mockedAxios.get.mockResolvedValueOnce(ok({ imdbID: 'tt67890', Type: 'series' }));
      const id = await findImdbId('Poltergeist Show', null, 'series');
      expect(id).toBe('tt67890');
      const params = mockedAxios.get.mock.calls[0][1].params;
      expect(params.type).toBe('series');
      expect(params.y).toBeUndefined();
    });

    it('returns null when the response carries no usable imdb id', async () => {
      mockSettings['omdb.api_key'] = 'KEY';
      mockedAxios.get.mockResolvedValueOnce(ok({ Title: 'Whatever', Type: 'movie' }));
      expect(await findImdbId('No Id Title', 2001)).toBeNull();
    });

    it('returns null and swallows network errors', async () => {
      mockSettings['omdb.api_key'] = 'KEY';
      mockedAxios.get.mockRejectedValueOnce(new Error('boom'));
      expect(await findImdbId('Neterr Title', 1999)).toBeNull();
    });
  });
