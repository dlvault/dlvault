...
describe('Scheduler — processMovie (movies)', () => {
  beforeEach(() => {
    ...
    vi.mocked(jdownloaderService.addLinks).mockResolvedValue(true as any);
  ...
  it('marks not_found when every plugin returns only filtered-out releases', async () => {
    ...
    expect(movies.updateMovieStatus).toHaveBeenCalledWith(m.id, 'not_found', 'https://a/page');
    expect(addLogEntry).toHaveBeenCalledWith(m.id, 'quality_mismatch', expect.any(String));
  });
  ...
  it('logs jdownloader_error when addLinks fails', async () => {
    ...
    vi.mocked(jdownloaderService.getDownloadPackages).mockResolvedValue([] as any);
    vi.mocked(jdownloaderService.getLinkGrabberPackages).mockResolvedValue([] as any);
    vi.mocked(jdownloaderService.addLinks).mockResolvedValue(false as any);
...
describe('Scheduler — processMovie (shows)', () => {
  ...
  it('season pack: resolves unresolved /external/ links via the plugin before sending to JD', async () => {
    ...
    const pack = makeRelease({
      ...
      links: [{ hoster: 'rapidgator', url: 'https://source-b.example/external/2/abc' }],
    });
    const plugin = setSinglePlugin(
    ...
    expect(plugin.resolveLinks).toHaveBeenCalledWith([{ hoster: 'rapidgator', url: 'https://source-b.example/external/2/abc' }]);
    expect(jdownloaderService.addLinks).toHaveBeenCalledWith(['https://rg.example/direct/file'], expect.any(String));
  });
