...
import {
  ...
  updateMovieTraktId, updateMovieTitle, setSeasonCutoff, setMovieImdbId, setMovieYear,
...
describe('Movie DB Service', () => {
  ...
  it('defaults season_cutoff to null and sets/clears it', () => {
    const m = addMovie({ trakt_id: 84, imdb_id: '', tmdb_id: 84, title: 'A Show', year: 2024, slug: 'show', media_type: 'show', status: 'pending', desired_quality: '1080p' });
    expect(getMovieById(m.id)?.season_cutoff ?? null).toBeNull();
    setSeasonCutoff(m.id, 21);
    expect(getMovieById(m.id)?.season_cutoff).toBe(21);
    setSeasonCutoff(m.id, null);
    expect(getMovieById(m.id)?.season_cutoff ?? null).toBeNull();
  });
});
...
describe('Movie DB Service — id and year backfill', () => {
  beforeEach(() => { testDb.exec('DELETE FROM movies'); });

  const base = { slug: 'x', media_type: 'movie' as const, status: 'pending' as const, desired_quality: '1080p' };

  it('writes the imdb id and reports it', () => {
    const m = addMovie({ ...base, trakt_id: 901, imdb_id: '', tmdb_id: 4242, title: 'Keyed by TMDb', year: 2026 });
    expect(setMovieImdbId(m.id, 'tt4242')).toBe(true);
    expect(getMovieById(m.id)?.imdb_id).toBe('tt4242');
  });

  it('refuses to take an imdb id another row already owns', () => {
    const owner = addMovie({ ...base, trakt_id: 902, imdb_id: 'tt4242', tmdb_id: 1, title: 'Owner', year: 2026 });
    const dup = addMovie({ ...base, trakt_id: 903, imdb_id: '', tmdb_id: 2, title: 'Duplicate', year: 2026 });
    expect(setMovieImdbId(dup.id, 'tt4242')).toBe(false);
    expect(getMovieById(dup.id)?.imdb_id).toBeFalsy();
    expect(getMovieById(owner.id)?.imdb_id).toBe('tt4242');
  });

  it('completes a missing year', () => {
    const m = addMovie({ ...base, trakt_id: 904, imdb_id: '', tmdb_id: 3, title: 'No Year', year: 0 });
    setMovieYear(m.id, 2023);
    expect(getMovieById(m.id)?.year).toBe(2023);
  });
});
