...
// The restore route records an activity entry; without this the real module
// runs against the mocked db stub and throws, surfacing as a bogus 400.
...
const mockSeerr = vi.hoisted(() => ({
  isConfigured: vi.fn(() => false),
  testConnection: vi.fn(async () => ({ ok: true, version: '2.7.3' })),
  getAuthFailure: vi.fn(() => null),
}));
vi.mock('../../src/services/seerr', () => ({ seerrService: mockSeerr }));
...
const backupFns = vi.hoisted(() => ({
  ...
  listBackups: vi.fn(() => [{ filename: 'b.json', size: 10, created: '2026-01-01', legacy: false }]),
  deleteBackup: vi.fn(() => true),
  startBackupScheduler: vi.fn(),
  backupDir: vi.fn(() => '/data/backups'),
  resolveBackupPath: vi.fn((f: string) => (f === 'missing.tar.gz' ? null : `/data/backups/${f}`)),
}));
const restoreFns = vi.hoisted(() => ({
  stageRestore: vi.fn(() => ({
    manifest: { archiveVersion: 1, createdAt: '2026-01-01T00:00:00Z', includesKey: true, pluginCount: 1 },
    restoredKey: true,
    pluginCount: 1,
  })),
}));
vi.mock('../../src/services/restore', () => restoreFns);
...
beforeEach(() => {
  ...
  mockSeerr.isConfigured.mockReturnValue(false);
  mockTrakt.isConfigured.mockReturnValue(true);
...
describe('PUT /api/settings', () => {
  ...
  it('accepts the documented challenge_solver.url override', async () => {
    // createPluginContext reads this key and documents it as user-configurable,
    // but the prefix was missing from the allowlist — so the save was silently
    // dropped and the solver stayed pinned to the in-image instance.
    const { setSetting } = await import('../../src/database/index');
    const res = await request(app).put('/api/settings').send({
      'challenge_solver.url': 'http://192.168.1.50:8191',
    });
    expect(res.status).toBe(200);
    expect(setSetting).toHaveBeenCalledWith('challenge_solver.url', 'http://192.168.1.50:8191');
  });
  ...
  it('restarts scheduler for ANY scheduler.* key — the frontend sends deltas', async () => {
    // The old interval_hours/enabled check only worked because the former
    // full-object save always contained those keys.
    await request(app).put('/api/settings').send({ 'scheduler.quiet_hours_start': '23' });
    expect(schedFns.startScheduler).toHaveBeenCalled();
  });

  it('does not restart scheduler when no scheduler key is in the delta', async () => {
    await request(app).put('/api/settings').send({ 'quality.default': '1080p' });
    expect(schedFns.startScheduler).not.toHaveBeenCalled();
  });
...
describe('backup-schedule endpoints', () => {
  it('GET returns schedule settings', async () => {
    ...
    // effectivePath tells the UI where backups actually land when the path is
    // left empty; include_key drives whether the archive can restore credentials.
    expect(res.body).toMatchObject({ enabled: 'true', interval_hours: '24', max_backups: '5' });
    expect(res.body).toHaveProperty('effectivePath');
    expect(res.body).toHaveProperty('include_key');
  });
...
describe('POST /api/settings/backups/:filename/restore', () => {
  it('stages the restore and reports that it is restarting', async () => {
    const exitSpy = vi.spyOn(process, 'exit').mockImplementation((() => undefined) as any);
    // shouldAdvanceTime keeps the awaited supertest request able to settle while
    // still letting the test drive the deferred exit timer explicitly.
    vi.useFakeTimers({ shouldAdvanceTime: true });
    try {
      const res = await request(app).post('/api/settings/backups/dlvault-2026.tar.gz/restore');

      expect(res.status).toBe(200);
      expect(res.body.restarting).toBe(true);
      expect(res.body.restoredKey).toBe(true);
      expect(restoreFns.stageRestore).toHaveBeenCalled();

      // The swap itself happens on the next boot — the process has to go down
      // first, because the database handle is open for its whole lifetime.
  ...
  it('404s for a backup that does not exist', async () => {
    const res = await request(app).post('/api/settings/backups/missing.tar.gz/restore');
    expect(res.status).toBe(404);
  });

  it('refuses a legacy .db backup and explains the manual route', async () => {
    // Those predate the archive format: database only, no key and no plugins,
    // so restoring one automatically would leave credentials unreadable.
    const res = await request(app).post('/api/settings/backups/dlvault-old.db/restore');
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/alten Format/i);
  });

  it('surfaces a rejected archive as a 400', async () => {
    restoreFns.stageRestore.mockImplementationOnce(() => {
      throw new Error('Keine Datenbank im Archiv.');
    });
    const res = await request(app).post('/api/settings/backups/broken.tar.gz/restore');
    expect(res.status).toBe(400);
    expect(res.body.error).toContain('Keine Datenbank');
  });
});

describe('PUT /api/settings/backup-schedule — path validation', () => {
  it('rejects a path that cannot be written', async () => {
    mockFs.writeFileSync.mockImplementationOnce(() => { throw new Error('EACCES'); });
    const res = await request(app).put('/api/settings/backup-schedule').send({ path: '/nope' });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/nicht beschreibbar/i);
  });

  it('stores a writable path and reports the effective target', async () => {
    const { setSetting } = await import('../../src/database/index');
    const res = await request(app).put('/api/settings/backup-schedule').send({ path: '/backups' });
    expect(res.status).toBe(200);
    expect(setSetting).toHaveBeenCalledWith('backup.path', '/backups');
    expect(res.body).toHaveProperty('effectivePath');
  });

  it('accepts an empty path (back to the default location)', async () => {
    const { setSetting } = await import('../../src/database/index');
    const res = await request(app).put('/api/settings/backup-schedule').send({ path: '  ' });
    expect(res.status).toBe(200);
    expect(setSetting).toHaveBeenCalledWith('backup.path', '');
  });
});
...
/**
 * Switching `watchlist.provider` only decides which provider *runs* — the
 * credentials stay behind. That is how a Trakt app that no longer exists kept
 * its tokens in the database with no way to remove them from the UI.
 */
describe('POST /api/settings/watchlist/:provider/disconnect', () => {
  it('wipes every credential the provider owns', async () => {
    mockSettings['trakt.client_id'] = 'cid';
    mockSettings['trakt.client_secret'] = 'sec';
    mockSettings['trakt.access_token'] = 'tok';
    mockSettings['trakt.refresh_token'] = 'ref';
    mockSettings['trakt.username'] = 'me';

    const res = await request(app).post('/api/settings/watchlist/trakt/disconnect');

    expect(res.status).toBe(200);
    expect(mockSettings['trakt.client_id']).toBe('');
    expect(mockSettings['trakt.client_secret']).toBe('');
    expect(mockSettings['trakt.access_token']).toBe('');
    expect(mockSettings['trakt.refresh_token']).toBe('');
    expect(mockSettings['trakt.username']).toBe('');
  });

  it('moves the selection to the one provider still connected', async () => {
    mockSettings['watchlist.provider'] = 'trakt';
    mockSeerr.isConfigured.mockReturnValue(true);

    const res = await request(app).post('/api/settings/watchlist/trakt/disconnect');

    expect(res.body.switchedTo).toBe('seerr');
    expect(mockSettings['watchlist.provider']).toBe('seerr');
  });

  it('leaves "both" pointing at whichever of the pair remains', async () => {
    mockSettings['watchlist.provider'] = 'both';

    await request(app).post('/api/settings/watchlist/trakt/disconnect');

    expect(mockSettings['watchlist.provider']).toBe('plex');
  });

  it('does not touch the selection when an inactive provider is disconnected', async () => {
    mockSettings['watchlist.provider'] = 'seerr';

    const res = await request(app).post('/api/settings/watchlist/plex/disconnect');

    expect(res.body.switchedTo).toBeNull();
    expect(mockSettings['watchlist.provider']).toBe('seerr');
  });

  it('clears only the named provider, never a bystander', async () => {
    mockSettings['plex.token'] = 'plex-tok';
    mockSettings['seerr.api_key'] = 'seerr-key';

    await request(app).post('/api/settings/watchlist/trakt/disconnect');

    expect(mockSettings['plex.token']).toBe('plex-tok');
    expect(mockSettings['seerr.api_key']).toBe('seerr-key');
  });

  it('rejects an unknown provider', async () => {
    const res = await request(app).post('/api/settings/watchlist/bogus/disconnect');
    expect(res.status).toBe(400);
  });
});
