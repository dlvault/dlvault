...
vi.mock('../../src/jdownloader/index', () => ({
  jdownloaderService: {
    ...
    configureDownloadPolicies: vi.fn(() => Promise.resolve(true)),
  },
}));
...
vi.mock('../../src/api/routes/update', async () => {
  ...
  return {
    ...
    // null = "can't determine" (no docker.sock in the test env) → update-check
    // falls back to the commit-SHA verdict, which is what these tests assert.
    registryHasNewerImage: vi.fn(() => Promise.resolve(null)),
  };
});
...
describe('startup (listen callback)', () => {
  ...
  it('pushes 2Captcha key to JD on listen when configured', async () => {
    vi.mocked(jdownloaderService.isConfigured).mockReturnValue(true);
    mockSettings['secret-store.2captcha-api-key'] = 'test-key-abc';
    vi.useFakeTimers();
    try {
      await listenCallback.fn!();
      await vi.advanceTimersByTimeAsync(6000);
      ...
      delete mockSettings['secret-store.2captcha-api-key'];
    }
    expect(jdownloaderService.configure2CaptchaSolver).toHaveBeenCalledWith('test-key-abc');
  });
});
...
// ===========================================================================
// trust proxy. Behind a reverse proxy the client IP only arrives in
// X-Forwarded-For; without this every remote request reports the proxy's
// address and the rate limiters key all of them into one bucket — ten fumbled
// logins by any single user then lock out everyone, owner included.
// ===========================================================================
describe('trust proxy', () => {
  ...
  async function loadApp(value?: string) {
    if (value === undefined) delete process.env.TRUST_PROXY;
    else process.env.TRUST_PROXY = value;
    vi.resetModules();
    return (await import('../../src/server')).default;
  }

  it('does not trust the header by default — safe when exposed directly', async () => {
    expect((await loadApp()).get('trust proxy')).toBe(false);
  });

  it('trusts exactly the configured number of hops', async () => {
    expect((await loadApp('1')).get('trust proxy')).toBe(1);
  });

  it('refuses true, which would let a client forge its own address', async () => {
    // With `true` Express believes the LEFT-most X-Forwarded-For entry, which
    // the client writes — an attacker could rotate forged addresses and walk
    // straight through the login limiter. A hop count reads the entry the proxy
    // itself wrote instead.
    const app = await loadApp('true');
    expect(app.get('trust proxy')).toBe(false);
    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('TRUST_PROXY=true is refused'));
  });

  it('passes a named setting through for more involved topologies', async () => {
    expect((await loadApp('loopback')).get('trust proxy')).toBe('loopback');
  });
});
