...
function setupHttp() {
  mockHttpRequest.mockImplementation((_opts: any, cb?: (res: any) => void) => {
    ...
    req.write = vi.fn((chunk: unknown) => { httpScript.calls.push(String(chunk)); });
    req.end = vi.fn(() => {
...
beforeEach(() => {
  ...
  delete process.env.REGISTRY_AUTH;
...
describe('POST /api/update/start', () => {
  ...
  it('passes registry auth via mode-0600 file, never as container env', async () => {
    process.env.HOST_DATA_DIR = '/host/data';
    process.env.REGISTRY_AUTH = 'dXNlcjp0b2tlbg==';
    httpScript.queue = [
      { statusCode: 200 },                                  // /_ping
      { statusCode: 200 },                                  // imageExists hadBefore=true
      { statusCode: 200 },                                  // pullImage ok
    ...
    expect(mockFs.writeFileSync).toHaveBeenCalledWith('/app/data/.registry-auth', 'dXNlcjp0b2tlbg==', { mode: 0o600 });

    // The container-create payload must point at the file — the secret itself
    // must not appear (env vars stay readable via `docker inspect`).
    const createPayload = httpScript.calls.find(c => c.includes('"Image"'));
    expect(createPayload).toBeDefined();
    expect(createPayload).toContain('REGISTRY_AUTH_FILE=/status/.registry-auth');
    expect(createPayload).not.toContain('dXNlcjp0b2tlbg==');
  });

  it('removes a stale auth file when REGISTRY_AUTH is not configured', async () => {
    process.env.HOST_DATA_DIR = '/host/data';
    httpScript.queue = [
      { statusCode: 200 },                                  // /_ping
      { statusCode: 200 },                                  // imageExists hadBefore=true
    ...
    const res = await request(app).post('/api/update/start').set('x-confirm-update', 'yes');
    expect(res.status).toBe(202);
    expect(mockFs.unlinkSync).toHaveBeenCalledWith('/app/data/.registry-auth');
  });
