...
// The route builds a FilterContext (scraper/filter → getSetting) to annotate
// releases with per-release rejection reasons — settings come from this mock.
vi.mock('../../src/database/index', () => {
  const settings: Record<string, string> = {
    'quality.minimum': '1080p',
    'quality.maximum': '2160p',
    'quality.audio_minimum': '',
    'quality.exclude_types': '',
    'quality.language': 'german',
    'quality.language_strict': 'false',
    'quality.series_override': 'false',
  };
  return {
...
describe('POST /api/search', () => {
  ...
  it('annotates each release with its filter verdict (rejectionReasons)', async () => {
    mocks.plugins = [{
      id: 'p',
      findReleases: vi.fn(async () => ({
        sourceUrl: 'http://src',
        releases: [
          // Passes: meets min resolution, right language, has links.
          { title: 'Good', quality: '1080p', audio: '5.1', language: 'german', size: '8GB', releaseType: 'rip', links: [{ hoster: 'h', url: 'http://x' }] },
          // Fails twice: below min resolution + wrong language.
          { title: 'Bad', quality: '720p', audio: '5.1', language: 'english', size: '2GB', releaseType: 'rip', links: [{ hoster: 'h', url: 'http://y' }] },
        ],
      })),
    }];
    ...
    const res = await request(app).post('/api/search').send({ query: 'Foo' });
    expect(res.status).toBe(200);
    expect(res.body.releases[0].rejectionReasons).toEqual([]);
    expect(res.body.releases[1].rejectionReasons).toHaveLength(2);
    expect(res.body.releases[1].rejectionReasons[0]).toContain('Auflösung unter Minimum');
    expect(res.body.releases[1].rejectionReasons[1]).toContain('Falsche Sprache');
  });

  it('respects the per-title qualityOverride when judging releases', async () => {
    mocks.plugins = [{
      id: 'p',
      findReleases: vi.fn(async () => ({
        releases: [
          // Below the 1080p floor — rejected globally, accepted under 'relaxed'.
          { title: 'Low', quality: '720p', audio: '5.1', language: 'german', size: '2GB', releaseType: 'rip', links: [{ hoster: 'h', url: 'http://y' }] },
        ],
      })),
    }];

    const res = await request(app).post('/api/search').send({ query: 'Foo', qualityOverride: 'relaxed' });
    expect(res.status).toBe(200);
    expect(res.body.releases[0].rejectionReasons).toEqual([]);
  });
...
describe('POST /api/search/download — flow', () => {
  ...
  it('looks up by tmdbId when imdbId not found', async () => {
    ...
    expect(res.status).toBe(200);
    // Scoped by media type — TMDb's movie and TV id spaces overlap, so an
    // unscoped lookup can return a show for a requested film and vice versa.
    expect(movies.getMovieByTmdbId).toHaveBeenCalledWith(555, 'movie');
  });
