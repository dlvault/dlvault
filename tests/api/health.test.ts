...
describe('GET /api/health/deep', () => {
  ...
  it('returns 200 degraded when 2Captcha balance is low (non-critical)', async () => {
    mockAxiosGet.mockResolvedValue({ data: { status: 1, request: '0.10' } });
    const res = await request(makeApp()).get('/api/health/deep');
    expect(res.status).toBe(200);
    expect(res.body.overall).toBe('degraded');
    expect(res.body.checks['2captcha_balance'].ok).toBe(false);
    expect(res.body.checks['2captcha_balance'].error).toMatch(/Low balance/i);
  });

  it('reports 2Captcha as not configured (passing) when key is missing', async () => {
    delete mockSettings['secret-store.2captcha-api-key'];
    const res = await request(makeApp()).get('/api/health/deep');
    expect(res.body.checks['2captcha_balance'].ok).toBe(true);
    expect(res.body.checks['2captcha_balance'].detail).toContain('not configured');
  });
  ...
  it('skips Playwright and Wikidata when quick=true', async () => {
    ...
    expect(res.body.checks).toHaveProperty('2captcha_balance');
