...
// The health route reads Trakt's auth state. Mock the service so the test
// doesn't construct a real axios client (and so the state is controllable).
const traktAuthFailure = vi.hoisted(() => ({ value: null as null | { since: number; status: number; detail: string } }));
vi.mock('../../src/services/trakt', () => ({
  traktService: { getAuthFailure: () => traktAuthFailure.value },
}));
