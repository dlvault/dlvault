...
const mockLogs = vi.hoisted(() => [
  ...
  // Deliberately the LEGACY "Release(s)" format: old DB rows keep it, and the
  // reason extraction must still pick the trailing parenthetical, not the "(s)".
  { id: 3, movie_id: 1, action: 'quality_mismatch', details: '3 Release(s) gefunden, aber keins erfüllt die Anforderungen (2 unter Audio-Mindest; gewünscht ≥ 1080p)', created_at: '2026-04-01T02:00:00Z' },
]);
...
vi.mock('../../src/database/services/activityLog', () => ({
  ...
  addLogEntry: vi.fn(),
  getWeekDelta: vi.fn(() => ({ added: 0, completed: 0 })),
}));
...
vi.mock('../../src/services/libraryProvider', () => ({
  getLibraryProvider: vi.fn(() => ({ isConfigured: vi.fn(() => false), getMovies: vi.fn(async () => []), deleteItem: vi.fn(async () => false), getCachedMovieCount: vi.fn(() => 0) })),
...
describe('GET /api/sync/status', () => {
  it('returns status breakdown by movie status', async () => {
    ...
    // Counted server-side over the full activity_log, not the truncated feed.
    expect(res.body.weekDelta).toMatchObject({
...
describe('GET /api/sync/logs', () => {
  ...
  it('respects custom limit parameter', async () => {
    ...
    expect(mockedGetRecentLogs).toHaveBeenCalledWith(50, undefined);
  });
  ...
  it('treats limit=0 as default (0 is falsy, falls back to 100)', async () => {
    ...
    expect(mockedGetRecentLogs).toHaveBeenCalledWith(100, undefined);
  });
  ...
  it('clamps limit to maximum of 1000', async () => {
    ...
    expect(mockedGetRecentLogs).toHaveBeenCalledWith(1000, undefined);
  });
  ...
  it('passes the action filter through to the query', async () => {
    const { getRecentLogs } = await import('../../src/database/services/activityLog');
    const mockedGetRecentLogs = vi.mocked(getRecentLogs);

    await request(app).get('/api/sync/logs?action=quality_mismatch');
    expect(mockedGetRecentLogs).toHaveBeenCalledWith(100, 'quality_mismatch');
  });
});
...
describe('GET /api/movies/:id', () => {
  it('returns movie with downloads and logs for valid id', async () => {
    ...
    // Search candidates are aggregated from downloads (found) + not_found logs.
    expect(Array.isArray(res.body.candidates)).toBe(true);
    expect(res.body.candidates).toContainEqual(
      expect.objectContaining({ name: 'Test.Movie.2024.1080p', source: 'demo', found: true }),
    );
    // quality_mismatch rounds appear as misses carrying the per-axis reason
    // (the parenthetical of the log line).
    expect(res.body.candidates).toContainEqual(
      expect.objectContaining({
        found: false,
        rejectionReason: '2 unter Audio-Mindest; gewünscht ≥ 1080p',
      }),
    );
  });
...
describe('PUT /api/movies/:id/season-cutoff', () => {
  it('returns 404 for a non-existent movie', async () => {
    const res = await request(app).put('/api/movies/999/season-cutoff').send({ cutoff: 5 });
    expect(res.status).toBe(404);
  });

  it('rejects a cutoff on a movie (shows only)', async () => {
    // mockMovies[0] has no media_type → treated as a movie
    const res = await request(app).put('/api/movies/1/season-cutoff').send({ cutoff: 5 });
    expect(res.status).toBe(400);
  });

  it('sets a cutoff on a show and persists it', async () => {
    const movies = await import('../../src/database/services/movies');
    vi.mocked(movies.getMovieById).mockReturnValueOnce({ id: 1, title: 'A Show', media_type: 'show' } as any);
    const res = await request(app).put('/api/movies/1/season-cutoff').send({ cutoff: 21 });
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ success: true, season_cutoff: 21 });
    expect(movies.setSeasonCutoff).toHaveBeenCalledWith(1, 21);
  });

  it('clears the cutoff when given null', async () => {
    const movies = await import('../../src/database/services/movies');
    vi.mocked(movies.getMovieById).mockReturnValueOnce({ id: 1, title: 'A Show', media_type: 'show' } as any);
    const res = await request(app).put('/api/movies/1/season-cutoff').send({ cutoff: null });
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ success: true, season_cutoff: null });
    expect(movies.setSeasonCutoff).toHaveBeenCalledWith(1, null);
  });

  it('rejects an invalid cutoff (zero is not a valid season)', async () => {
    const movies = await import('../../src/database/services/movies');
    vi.mocked(movies.getMovieById).mockReturnValueOnce({ id: 1, title: 'A Show', media_type: 'show' } as any);
    const res = await request(app).put('/api/movies/1/season-cutoff').send({ cutoff: 0 });
    expect(res.status).toBe(400);
  });
});
...
describe('PUT /api/movies/:id/quality-override', () => {
  it('returns 404 for a non-existent movie', async () => {
    const res = await request(app).put('/api/movies/999/quality-override').send({ mode: 'relaxed' });
    expect(res.status).toBe(404);
  });

  it("sets 'relaxed' and persists it", async () => {
    const movies = await import('../../src/database/services/movies');
    const res = await request(app).put('/api/movies/1/quality-override').send({ mode: 'relaxed' });
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ success: true, quality_override: 'relaxed' });
    expect(movies.updateQualityOverride).toHaveBeenCalledWith(1, 'relaxed');
  });

  it("sets 'any' and persists it", async () => {
    const movies = await import('../../src/database/services/movies');
    const res = await request(app).put('/api/movies/1/quality-override').send({ mode: 'any' });
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ success: true, quality_override: 'any' });
    expect(movies.updateQualityOverride).toHaveBeenCalledWith(1, 'any');
  });
  ...
  it('clears the override when given null', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ success: true, quality_override: null });
    expect(movies.updateQualityOverride).toHaveBeenCalledWith(1, null);
  });

  it('rejects an unknown mode', async () => {
    const res = await request(app).put('/api/movies/1/quality-override').send({ mode: 'yolo' });
    expect(res.status).toBe(400);
  });
});
...
describe('POST /api/sync/run', () => {
  it('kicks off a background sync', async () => {
    const { runFullSync } = await import('../../src/services/scheduler');
    const res = await request(app).post('/api/sync/run');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ success: true, message: 'Sync started' });
    expect(runFullSync).toHaveBeenCalled();
  });

  it('returns 409 when a sync is already running', async () => {
    const { isSyncRunning } = await import('../../src/services/scheduler');
    vi.mocked(isSyncRunning).mockReturnValueOnce(true);
    const res = await request(app).post('/api/sync/run');
...
describe('POST /api/sync/scheduler/start + /stop', () => {
  it('starts the scheduler', async () => {
    const { startScheduler } = await import('../../src/services/scheduler');
    const res = await request(app).post('/api/sync/scheduler/start');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ success: true, running: true });
  ...
  it('stops the scheduler', async () => {
    ...
    const res = await request(app).post('/api/sync/scheduler/stop');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ success: true, running: false });
...
describe('POST /api/movies/:id/retry', () => {
  it('404s for an unknown movie', async () => {
    const res = await request(app).post('/api/movies/999/retry');
    expect(res.status).toBe(404);
    expect(res.body).toHaveProperty('error', 'Movie not found');
  });

  it('429s while still inside the retry cooldown', async () => {
    const { getMovieById } = await import('../../src/database/services/movies');
    vi.mocked(getMovieById).mockReturnValueOnce({
      id: 1, title: 'Test Movie', year: 2024, status: 'not_found',
  ...
  it('resets the movie and re-processes it on success', async () => {
    const { updateMovieStatus, resetRetryCount, updateLastRetryAt } = await import('../../src/database/services/movies');
    const { processMovie } = await import('../../src/services/scheduler');
    const res = await request(app).post('/api/movies/1/retry');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ success: true, message: 'Retry started' });
    expect(updateLastRetryAt).toHaveBeenCalledWith(1);
    expect(updateMovieStatus).toHaveBeenCalledWith(1, 'pending');
    expect(resetRetryCount).toHaveBeenCalledWith(1);
