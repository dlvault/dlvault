...
#    REGISTRY_AUTH_FILE  path to a file holding base64 `user:token` for
#                        private registries (consumed + deleted after login)
#    REGISTRY_AUTH       same value inline via env — fallback for manual runs;
#                        prefer the file, env vars leak via `docker inspect`
# ============================================================
...
# Credentials preferably arrive via REGISTRY_AUTH_FILE (mode-0600 file in the
# shared /status volume, written by the main container) — read once, then
# delete so the secret never outlives the login. REGISTRY_AUTH stays as an
# inline-env fallback for manual runs. For ghcr.io/dlvault/dlvault as a
# public image, this whole block is a no-op.
if [ -n "${REGISTRY_AUTH_FILE:-}" ] && [ -f "$REGISTRY_AUTH_FILE" ]; then
  REGISTRY_AUTH=$(cat "$REGISTRY_AUTH_FILE")
...
if [ -n "$PREVIOUS_IMAGE_ID" ] && [ "$PREVIOUS_IMAGE_ID" = "$NEW_IMAGE_ID" ]; then
  # Same image SHA — but we still recreate the container so it picks up any
  # fresh manifest metadata (revision labels, annotations) that may have
  # changed in the registry. Skipping was an optimization that broke on
  # builds where buildx cache-hits everything and only labels move.
  log "Image-SHA unverändert — Container wird trotzdem neu erstellt um Metadata zu refreshen."
fi
...
# Env-var carry-forward — but only the vars that the NEW image's ENV layer
# DOESN'T define itself. Image-baked vars (GIT_COMMIT, NODE_ENV,
# PLAYWRIGHT_BROWSERS_PATH …) must come from the new image, otherwise the
# old container's frozen values get re-injected via `-e` and override the
# fresh ENV layer. That's the root cause of the "Update-Banner kommt nach
# Update sofort zurück" loop: process.env.GIT_COMMIT stays at the *previous*
# build's SHA forever because each updater run preserves the old one.
#
# `docker image inspect` returns the image-level ENV declarations (from the
# Dockerfile's `ENV` instructions) — those are the keys we must skip when
# replaying the old container's env on the new run.
IMAGE_BAKED_KEYS=$(docker image inspect --format='{{range .Config.Env}}{{println .}}{{end}}' "$IMAGE_NAME" 2>/dev/null | awk -F= 'NF>1 {print $1}' | sort -u)
log "Image-baked ENV keys (will NOT carry over from old container): $(echo "$IMAGE_BAKED_KEYS" | tr '\n' ' ')"

while IFS= read -r envvar; do
  [ -z "$envvar" ] && continue
  key="${envvar%%=*}"
  if echo "$IMAGE_BAKED_KEYS" | grep -qx "$key"; then
    # Skip — let the new image's ENV layer set this one.
    continue
  fi
  RUN_ARGS+=("-e" "$envvar")
...
# ── 6. Cleanup: alte dangling images entfernen ───────────────
# Ohne diesen Schritt sammeln sich bei jedem Update die alten dlvault- und
# dlvault-updater-Image-Layer als <none>:<none> an und fluten /var/lib/docker.
# `prune -f` entfernt ausschließlich untagged/dangling Images — laufende
# Container und alle getaggten Images (inkl. dem frisch gepullten :latest)
# bleiben unangetastet.
phase "cleanup"
log "Entferne alte dangling Images..."
docker image prune -f 2>&1 | tee -a "$STATUS_FILE" || log "Warnung: image prune fehlgeschlagen (nicht kritisch)"
