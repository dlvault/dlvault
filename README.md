...
- **Automatic backups** — scheduled archives containing the database, the encryption key and your installed plugins, restorable from the settings page
...
Mounting the Docker socket is what lets the dashboard's "Update" button
replace the running container. It is optional: without it dlvault runs
normally and you update the container however you normally would.

Be aware of what it grants. Source plugins run as ordinary in-process
code, so a mounted socket is inherited by every plugin you install —
which makes installing a plugin equivalent to trusting its author with
Docker, and therefore with the host. Only mount it if you accept that,
and only install plugins from sources you trust.
...
| `HOST_DATA_DIR` | auto-detected | Host path of the `/app/data` volume. Normally leave unset — it is read from the container's own mounts. Only needed as an override if that detection fails |
| `API_TOKEN` | — | When set, every API request must authenticate. Scripts send `Authorization: Bearer <token>`; the web interface asks for it once and stores it in an HttpOnly cookie |
...
| `/var/run/docker.sock` | optional — enables one-click updates | Lets the updater sidecar pull a new image and recreate the container. Read-only mounts will not work. Without this mount everything else works; only the dashboard's Update button is disabled. **This grants control over Docker, and so over the host.** Plugins run in-process and inherit it — see the warning under Quick start. |

One-click updates need `/app/data` on a real bind mount, not an anonymous volume: the updater bind-mounts the same host directory to stream its progress back. The host-side path is detected from the container's own mounts; `HOST_DATA_DIR` exists only to override that if detection fails.
...
- **Post-processing** — extract, rename templates (`{title}`, `{year}`, `{quality}`, `{season}`, `{imdbid}`)
- **Backups** — schedule, retention, destination folder, restore
