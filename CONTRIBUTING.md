...
- **Plugins for sources that distribute or facilitate access to copyrighted
  ...
  the core ships with no bundled sources at all. PRs adding plugins for
  sources that don't respect copyright will be closed without merge,
  regardless of how the plugin is framed.
...
- **Features whose primary use case is bypassing copyright protections**
  (scaffolding aimed at specific protected services, DRM circumvention
  helpers, etc.). The generic `context.secrets` API is for plugins that
  need user-supplied API keys for legitimate third-party services.
