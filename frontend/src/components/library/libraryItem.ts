...
export function enrichItems(items: LibraryItem[], movies: Movie[]): EnrichedLibraryItem[] {
  ...
  return items.map(item => {
    ...
    // The media server's REAL file quality beats the desired quality — the
    // badge should say what's on disk, not what dlvault aimed for. Shows and
    // provider-less items keep the desired-quality fallback above.
    if (item.videoQuality) enriched.quality = item.videoQuality;
    return enriched;
  });
}
