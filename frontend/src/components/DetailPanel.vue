      ...
      >
        ...
        <div class="panel-body">
          ...
          <div v-if="movie.seasons && movie.seasons.length" class="panel-section">
            ...
            <div class="seasons">
              <div
                v-for="s in movie.seasons"
                :key="s.number"
                :class="['season-row', { complete: s.complete, empty: s.aired === 0 }]"
              >
...
import type { DetailContext, MovieStatus, PanelMovie, PanelSeason } from '../types/index';
import {
...
const STAGE: Record<MovieStatus, { label: string; mono: string; color: string }> = {
  ...
  extracting:  { label: 'Entpackt',        mono: 'ENTPACKT',        color: 'var(--stage-extracting)' },
  ...
  not_available:    { label: 'Noch nicht verfügbar',  mono: 'NOCH NICHT VERFÜGBAR', color: 'var(--stage-not_available)' },
...
// concrete per-axis reason (e.g. "3 unter Audio-Mindest") shows in the activity
// log below — this notice just frames what the bucket means.
const notFoundNotice = computed<{ title: string; body: string } | null>(() => {
  switch (props.movie?.status) {
    case 'not_available':
      return { title: 'Noch nicht verfügbar', body: 'An der Quelle wurden (noch) keine Releases gefunden — der Titel ist dort vermutlich noch nicht gelistet. dlvault sucht beim nächsten Sync erneut.' };
    case 'no_download':
      return { title: 'Kein Download verfügbar', body: 'Releases wurden gefunden, aber es ließen sich keine nutzbaren Hoster-Links auflösen (offline oder blockiert). Details siehe Aktivität.' };
    case 'quality_mismatch':
      return { title: 'Anforderungen nicht erfüllt', body: 'Ein Release wurde gefunden, erfüllt aber die Filter nicht (Auflösung, Audio, Sprache oder Release-Typ). Der konkrete Grund steht in der Aktivität unten.' };
    case 'not_found':
