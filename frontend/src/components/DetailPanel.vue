      ...
      >
        ...
        <div class="panel-body">
          ...
          <div class="panel-actions">
            ...
            <button v-if="context === 'library'" class="btn btn-ghost" @click="$emit('openFolder', movie)">
              <Folder :size="14" />Ordner zeigen
            </button>
            <button v-else-if="context === 'queue' && movie.status !== 'not_found'" class="btn btn-ghost" @click="$emit('research', movie)">
...
import {
  X, Check, RefreshCw, Play, Folder, Pause, Trash2,
...
const CONTEXTS: Record<DetailContext, { label: string; icon: unknown; primaryAction: string; primaryIcon: unknown }> = {
  ...
  library:   { label: 'Mediathek',     icon: Library,     primaryAction: 'In Jellyfin öffnen', primaryIcon: Play },
  ...
  return PIPELINE_STAGES.map((s, i) => {
    ...
    if (state === 'active') {
      if (s.key === 'searching' && m.pipeline?.startedSearch) {
        ...
        detail = `⚠ stuck · ${m.pipeline.attempts ?? 0} Versuche${m.pipeline.lastChecked ? ` · zuletzt ${m.pipeline.lastChecked}` : ''}`;
