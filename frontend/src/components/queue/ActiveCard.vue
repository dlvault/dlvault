<template>
  <div class="qx-active-card" :style="{ '--stage-color': stageColor }">
...
const stageLabel = computed(() => isExtracting.value ? 'EXTRACTING' : 'DOWNLOADING');
const stageColor = computed(() => isExtracting.value ? 'var(--stage-extracting)' : 'var(--stage-downloading)');
const subStatus = computed(() => isExtracting.value ? 'finaler Schritt' : 'läuft');
