  ...
  >
    ...
    <div class="qx-row-text">
      ...
      <div class="qx-row-meta">
        ...
        <span v-if="isStuck" class="item attn">· stuck</span>
      </div>
...
import { timeAgo, sourceHost } from '../../composables/useFormatters';
...
const stageLabel = computed(() => STAGE_LABELS[props.movie.status] || props.movie.status);
const stageColor = computed(() => `var(--stage-${props.movie.status === 'not_found' ? 'not_found' : props.movie.status})`);
