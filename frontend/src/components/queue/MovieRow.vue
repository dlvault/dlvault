...
import { timeAgo, sourceHost, displayStatus } from '../../composables/useFormatters';
...
const STAGE_LABELS: Record<string, string> = {
  ...
  // 'not_found' is split into its three reason buckets via displayStatus.
  not_available:    'Noch nicht verfügbar',
  no_download:      'Kein Download',
  quality_mismatch: 'Keine passende Qualität',
};

// Resolve 'not_found' into its reason bucket so the badge shows *why* (label +
// stage colour), instead of a generic "Nicht gefunden".
const displayStat = computed(() => displayStatus(props.movie));
const stageLabel = computed(() => STAGE_LABELS[displayStat.value] || displayStat.value);
