      ...
      <div class="hero-active">
        ...
        <div class="hero-meta">
          ...
          <div class="hero-strip">
            <div v-if="music" class="hero-strip-item music">
              <span class="music-ico"><Music :size="15" /></span>
              <div>
                <div class="lbl">Musik direkt{{ music.count > 1 ? ' · ' + music.count + ' aktiv' : '' }}</div>
                <div class="val"><em>{{ music.label }}</em><span v-if="music.speed"> · {{ music.speed }}</span></div>
              </div>
            </div>
...
.hero-strip-item.music { flex-direction: row; align-items: center; gap: 10px; }
.music-ico {
  display: grid;
  place-items: center;
  width: 30px;
