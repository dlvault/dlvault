<template>
  <Teleport to="body">
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Quelle wechseln"
      @click.self="onClose"
      @keydown.esc.stop="onClose"
    >
      <div ref="modalRoot" class="modal-box modal-box-wide">
        ...
        <ul v-else class="release-list">
          <li
            ...
            :key="r.index"
            class="release-row"
            :class="{ selected: selected === r.index, rejected: r.rejectionReasons.length > 0 }"
          ...
          >
            ...
            <!-- Filter verdict — informational only, the release stays selectable -->
            <span v-if="r.rejectionReasons.length > 0" class="rel-reject">
...
interface ReleaseRow {
  ...
  /** Filter verdict from the backend — empty array = passes the configured filters. */
...
// The only modal in the app that had none of this: Esc did nothing, Tab walked
// focus out into the page behind it, and the body kept scrolling under the overlay.
const modalRoot = ref<HTMLElement>();
const visible = ref(true);
useModalA11y(visible, modalRoot);

// A keydown handler on the backdrop only fires while focus is inside it, so the
// document-level listener is what actually makes Esc reliable.
...
onMounted(async () => {
  ...
  try {
    const res = await searchReleases({
      ...
      // So the verdicts reflect this title's effective filter, not just the global one.
      qualityOverride: props.movie.qualityOverride ?? null,
    });
...
.release-row.rejected:not(.selected) .rel-q,
.release-row.rejected:not(.selected) .rel-title,
.release-row.rejected:not(.selected) .rel-meta {
  opacity: 0.55;
}
.rel-reject {
  grid-column: 2 / -1;
