...
const actions: PaletteItem[] = [
  {
    ...
    action: async () => {
      // triggerSync resolves to `{ ok, error }` — an object, so the old
      // `ok ? …` test was always true and the palette reported success even
      // when the sync was rejected (already running, or rate-limited).
      const res = await syncStore.triggerSync();
      toast.value?.add(
        res.ok ? 'Sync gestartet!' : (res.error || 'Sync fehlgeschlagen'),
