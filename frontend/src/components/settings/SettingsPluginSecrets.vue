  ...
  <SettingsSection label="Plugin-Secrets">
    ...
    <template v-else>
      <SettingsRow
        ...
        :hint="hintFor(s)"
      >
        <SecretInput v-model="settings[storageKey(s.key)]" :setting-key="storageKey(s.key)" />
      </SettingsRow>
    </template>
  </SettingsSection>
</template>
...
/**
 * SettingsRow renders `hint` with v-html, on the documented assumption that its
 * sources are static literals in our own codebase. These are not: the name and
 * description come from an installed plugin's manifest. Escape the dynamic parts
 * so only the static markup below survives.
 */
...
function hintFor(s: AggregatedSecret): string {
  const requested = s.requestedBy.map(p => escapeHtml(p.name)).join(', ');
  const desc = s.description ? `${escapeHtml(s.description)}<br />` : '';
