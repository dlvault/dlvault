<template>
  <div class="secret-input">
    ...
    <div class="si-footer">
      <span v-if="cleared" class="si-tag warn"><span class="dot"></span>Wird beim Speichern gelöscht</span>
      <span v-else-if="dirty" class="si-tag busy"><span class="dot"></span>Wird beim Speichern übernommen</span>
      ...
      <!-- Without this there is no way to REMOVE a configured secret: clearing
           the field restores the mask (= "no change"), so the only way to get
           rid of an OMDb key or Telegram token was to type a junk value. -->
      <button v-if="stored" type="button" class="si-clear" :disabled="clearing" @click="clearSecret">
        {{ clearing ? 'Entferne…' : 'Entfernen' }}
      </button>
      <button v-else-if="cleared" type="button" class="si-clear" @click="undoClear">
        Doch behalten
      </button>
    </div>
  </div>
...
import { deleteSecret } from '../../api/index';
...
const props = withDefaults(defineProps<{
  ...
  optional?: boolean;
  /**
   * The settings key this field writes, e.g. `omdb.api_key`. Enables the
   * "Entfernen" action, which needs a dedicated endpoint — the normal save path
   * ignores empty values for secrets on purpose.
   */
  settingKey?: string;
}>(), { modelValue: '', placeholder: 'API key / token', optional: false, settingKey: '' });
...
/**
 * The user asked to delete a secret that IS stored: the model holds '' while a
 * mask is remembered. Distinct from "never configured", where there is no mask.
 */
const cleared = computed(() => props.modelValue === '' && !!knownMask.value);
 ...
 *
 * Goes through the dedicated endpoint rather than the delta save: PUT
 * /api/settings deliberately ignores an empty value for a sensitive key (so
 * stale UI state can't wipe an OAuth token), which is exactly why clearing the
 * field alone could never delete anything. Without a `settingKey` we can only
 * stage it locally.
 */
async function clearSecret() {
  ...
  clearing.value = true;
  try {
    await deleteSecret(props.settingKey);
    knownMask.value = null;
    emit('update:modelValue', '');
  } catch {
    // Leave the stored state visible — nothing was removed.
  } finally {
...
.si-footer { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.si-clear {
  padding: 0;
  font-size: 11.5px;
  color: var(--text-3);
...
.si-clear:hover { color: var(--danger, #e5484d); }
