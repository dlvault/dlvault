<template>
  <SettingsSection label="TMDb (Suche &amp; Entdecken)">
    <div class="info-note">
      Versorgt Suche, „Beliebt diese Woche" und Poster — auf Deutsch, passend zur
      eingestellten Release-Sprache. Damit ist dlvault dafür auf nichts anderes
      angewiesen; ohne Schlüssel greift ersatzweise Seerr bzw. der Watchlist-Provider.
      Kostenlos auf <a href="https://www.themoviedb.org/settings/api" target="_blank" rel="noopener">themoviedb.org</a> registrieren.
    </div>
    <SettingsRow label="TMDb API Key" hint="Beide Varianten funktionieren &mdash; der kurze API Key ebenso wie das lange API Read Access Token. Wird verschl&uuml;sselt gespeichert.">
      <div class="row-action">
        <SecretInput v-model="settings['tmdb.api_key']" setting-key="tmdb.api_key" placeholder="TMDb API Key" optional />
        <button class="btn btn-ghost" type="button" :disabled="testingTmdb" @click="testTmdbConnection">
          <LoadingSpinner v-if="testingTmdb" inline />
          <RefreshCw v-else :size="14" />
          <span>{{ testingTmdb ? 'Teste…' : 'TMDb testen' }}</span>
        </button>
      </div>
      <div v-if="tmdbStatus" :class="['status-tag', tmdbStatus === 'ok' ? 'ok' : 'err']">
        <span class="dot"></span>
        {{ tmdbStatus === 'ok' ? 'Verbunden' : (tmdbError || 'Fehler') }}
      </div>
    </SettingsRow>
  </SettingsSection>
  ...
  <SettingsSection label="OMDb (Film-Metadaten)">
    ...
    <SettingsRow label="OMDb API Key" hint="Wird verschlüsselt gespeichert.">
      <div class="row-action">
        <SecretInput v-model="settings['omdb.api_key']" setting-key="omdb.api_key" placeholder="OMDb API Key" optional />
...
async function testTmdbConnection() {
  ...
  tmdbError.value = '';
  try {
    // Test what is stored, not what is typed — the field may hold an unsaved edit.
    await saveAll();
    await api.post('/settings/tmdb/test');
    tmdbStatus.value = 'ok';
    toast.value?.add('TMDb verbunden', 'success');
  } catch (e: any) {
    tmdbStatus.value = 'error';
    tmdbError.value = e?.response?.data?.error || '';
    toast.value?.add(tmdbError.value || 'TMDb nicht erreichbar', 'error');
  } finally {
