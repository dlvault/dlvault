...
/**
 * True when the server runs with API_TOKEN set and this browser has not yet
 * exchanged the token for its session cookie. App.vue watches this and shows the
 * token prompt; every view keeps rendering its own empty/error state meanwhile.
 */
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    } else if (error.response.status === 401 && error.response.data?.authRequired) {
      // Token auth is on and we're not authenticated (yet, or the cookie expired).
...
// Auth — only meaningful when the server was started with API_TOKEN.
export const getAuthStatus = () => api.get<{ required: boolean; authenticated: boolean }>('/auth/status');
export const authLogin = (token: string) => api.post<{ ok: boolean }>('/auth/login', { token });

// Settings
export const getSettings = () => api.get('/settings');
export const updateSettings = (data: Record<string, string>) => api.put('/settings', data);
/** Remove a stored secret. PUT ignores empty values for secrets by design. */
export const deleteSecret = (key: string) => api.delete(`/settings/secret/${encodeURIComponent(key)}`);
...
export const triggerJDUpdate = () => api.post('/settings/jdownloader/update');
...
export const getSeerrStatus = () => api.get('/settings/seerr/status');
export const testSeerr_api = () => api.post('/settings/seerr/test');
export const disconnectWatchlistProvider = (provider: string) =>
  api.post(`/settings/watchlist/${provider}/disconnect`);
export const registerArrInSeerr = (baseUrl: string) =>
  api.post('/settings/seerr/register', { baseUrl });
export const testTmdb = () => api.post('/settings/tmdb/test');
...
// Shows only — set "download from season N onwards" (null clears it).
export const setSeasonCutoff = (id: number, cutoff: number | null) =>
  api.put(`/movies/${id}/season-cutoff`, { cutoff });
// Per-title quality-filter override: 'relaxed' | 'any' | null (= global filter).
export const setQualityOverride = (id: number, mode: 'relaxed' | 'any' | null) =>
  api.put(`/movies/${id}/quality-override`, { mode });

// Sync
export const runSync = () => api.post('/sync/run');
export const getSyncStatus = () => api.get('/sync/status');
export const getLogs = (limit?: number, action?: string) => api.get('/sync/logs', { params: { limit, action } });
...
// Search
export const searchReleases = (data: { query: string; year?: number; mediaType?: string; qualityOverride?: string | null }) => api.post('/search', data);
...
// Health (detailed). force=true skips the 30s server-side snapshot cache —
// use right after an action (e.g. JD update) so the response reflects new state.
export const getHealthDetailed = (force = false) =>
  api.get('/health/detailed', { params: force ? { force: 'true' } : {} });
...
/**
 * Stage a restore. The server applies it on its next start, so it answers and
 * then exits — a network error right after this call is expected, not a failure.
 */
export const restoreBackup = (filename: string) =>
  api.post(`/settings/backups/${encodeURIComponent(filename)}/restore`);
