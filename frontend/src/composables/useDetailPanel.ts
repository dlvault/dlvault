...
import { timeAgo, activityVerb, activityText, sourceHost, trailingParenthetical } from './useFormatters';
import type {
...
// Pull the concrete failure reason from the newest matching diagnostic log so the
// detail-panel notice can show it inline. The activity list can be long, scrolled
// off, or — for items older than the 90-day log prune — empty while the not_found
// sub-bucket (a persisted column) lives on. logs arrive newest-first.
const REASON_LOGS: Partial<Record<MovieStatus, string[]>> = {
  quality_mismatch: ['quality_mismatch'],
  no_download: ['no_hoster', 'links_offline'],
};
function notFoundDetailFor(detail: MovieDetail, status: MovieStatus): string | null {
  const actions = REASON_LOGS[status];
  if (!actions) return null;
  const log = (detail.logs || []).find(l => actions.includes(l.action) && l.details);
  if (!log?.details) return null;
  // The reason summary is the TRAILING parenthetical, e.g.
  // "(2 unter Qualitäts-Mindest, 1 unter Audio-Mindest; gewünscht ≥ 1080p)".
  return trailingParenthetical(log.details) ?? log.details.trim();
}
...
function panelFromDetail(detail: MovieDetail, opts: BuildOpts): PanelMovie {
  ...
  return {
    ...
    seasonCutoff: detail.season_cutoff ?? null,
    activity: mapActivity(detail),
    notFoundDetail: notFoundDetailFor(detail, status),
    hasSource: !!detail.source_url,
    qualityOverride: detail.quality_override ?? null,
  };
}
...
export function useDetailPanel() {
  ...
  async function openFromPackage(
    ...
    try {
      ...
      const panel = panelFromDetail(res.data, { context: 'downloads', status, pkg, extracting });
      // Keep the package identity: the panel's `id` is now the dlvault movie id,
      // but the downloads view removes/pauses by JD uuid.
      panel.sourceId = pkg.uuid;
      movie.value = panel;
    } catch {
  ...
  async function openFromLibraryItem(item: LibraryItem, matchedMovie: Movie | null) {
    ...
    try {
      ...
      // Keep the media-server item id: `panel.id` is the dlvault movie id now,
      // and every library delete handler looks the entry up by the id it listed.
      panel.sourceId = item.id;
      movie.value = panel;
    } catch {
