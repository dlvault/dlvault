<template>
  <div class="app">
    ...
    <nav class="sidebar" aria-label="Hauptnavigation">
      <div class="logo">
        <router-link to="/" class="brand" aria-label="Zum Dashboard">
      ...
      <ul class="nav-links">
        ...
        <li>
          <router-link to="/search" active-class="active" aria-label="Suche">
...
import {
  LayoutDashboard, ListVideo, Library, Download, Search,
  Settings, FileText, MoreHorizontal, AlertTriangle,
...
import { serverReachable } from './api/index';
...
// Sidebar count badges — derived from sync store. Mediathek intentionally has
// NO badge: its only meaningful count (downloaded) clashed with the library
// page total (the full Plex/Jellyfin library), which read as a bug.
...
onMounted(() => {
  ...
  //
  // The old `&& !syncStore.loading` guard could never pass: `loading` starts as
  // true and is only cleared inside fetchAll(), so a hard load on /settings or
  // /logs (neither calls fetchAll) left the "Warteschlange" badge blank until
  // the user visited the Dashboard.
  if (!syncStore.status.totalMovies) {
...
.brand {
  ...
  align-items: center;
  gap: 10px;
  /* It's a router-link to the dashboard now — keep the plain-text look. */
