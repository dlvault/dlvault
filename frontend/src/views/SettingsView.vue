          ...
          <ul class="sx-tabs" role="tablist">
            <li v-for="tab in group.tabs" :key="tab.id">
              ...
              >
                ...
                <span :class="['stat-dot', tabStatus(tab)]"></span>
              </button>
            </li>
          </ul>
        </div>
...
import { getSettings, updateSettings, listPlugins, getHealthDetailed } from '../api/index';
...
import {
  ...
  Library, Sliders, Clock, Send, Save, Search, Wand2,
...
// ── Health for tab status dots ──────────────────────────────
interface ServiceInfo { configured: boolean; connected: boolean; error?: string }
const healthServices = ref<Record<string, ServiceInfo>>({});
...
function tabStatus(tab: TabDef): 'ok' | 'err' | 'warn' | 'na' {
  if (tab.service) {
    const s = healthServices.value[tab.service];
    if (!s || !s.configured) return 'na';
    return s.connected ? 'ok' : 'err';
  }
  // Non-service tabs: stay neutral unless we have a specific signal.
  if (tab.id === 'plugin-secrets') {
    // Show warn if any required secret is missing (best-effort: presence-of-keys
    // doesn't tell us required-ness without plugin manifests, so default 'ok'
    // when secret-store keys exist, 'warn' otherwise — the tab is only visible
    // because something requested secrets).
    return hasStoredSecrets.value ? 'ok' : 'warn';
  }
  return 'na';
}
...
async function saveAll() {
  ...
  try {
    ...
    await loadSettings();
    // Refresh service health so status dots reflect newly-saved credentials.
    loadHealth();
  } catch (e: unknown) {
...
.stat-dot.ok   { background: var(--ok); box-shadow: 0 0 4px var(--ok); }
.stat-dot.err  { background: var(--err); animation: sxPulse 1.6s ease-in-out infinite; }
.stat-dot.warn { background: var(--warn); }
.stat-dot.na   { background: var(--text-3); opacity: 0.5; }
@keyframes sxPulse {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.4; }
}
