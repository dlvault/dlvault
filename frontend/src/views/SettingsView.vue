<template>
  <div class="sx-shell" :class="{ 'sx-editing': changeCount > 0 }">
    <SkeletonLoader v-if="settingsLoading" variant="table" :count="8" />

    <div v-else class="sx-grid" :class="{ 'mobile-detail': mobileDetail }">
          ...
          <ul class="sx-tabs" role="tablist">
            <li v-for="tab in group.tabs" :key="tab.id">
              ...
              >
                ...
                <span v-if="errorTabs.has(tab.id)" class="err-dot" title="Ungültiges Feld in diesem Tab"></span>
              </button>
            </li>
          </ul>
        </div>
      ...
      <main class="sx-content">
        <button class="sx-back" type="button" @click="mobileDetail = false">
          <ChevronLeft :size="16" /><span>Übersicht</span>
        </button>
...
import {
  ...
  Library, Sliders, Clock, Send, Save, Search, Wand2, Film, Baby, ChevronLeft,
...
const query = ref('');
// Mobile master-detail: false = section menu, true = the selected panel.
...
// Per-key diff against the saved snapshot. Drives the SaveBar badge AND the
// delta save below — only these keys go over the wire.
const changedKeys = computed<string[]>(() => {
  if (!savedSettings.value) return [];
  let saved: Record<string, string> = {};
  try { saved = JSON.parse(savedSettings.value); } catch { return []; }
  const keys = new Set([...Object.keys(settings.value), ...Object.keys(saved)]);
  return [...keys].filter(k => settings.value[k] !== saved[k]);
});
const changeCount = computed(() => changedKeys.value.length);
const hasUnsavedChanges = computed(() => changeCount.value > 0);

// ── Error localization ──────────────────────────────────────
// errors is keyed by settings key ('jellyfin.url' …); map keys to the tab that
// owns them so the sidebar can mark the tab and the SaveBar can jump there —
// previously a typo in one tab blocked saving ALL tabs with a generic toast.
const KEY_TAB_OVERRIDES: Record<string, string> = {
  'plex.server_url': 'media-server', // Plex auth lives in watchlist, the server URL in media-server
};
const PREFIX_TAB: Record<string, string> = {
  'trakt.': 'watchlist', 'watchlist.': 'watchlist', 'plex.': 'watchlist',
  'plugins.': 'plugins', 'secret-store.': 'plugin-secrets', 'omdb.': 'metadata',
  'jdownloader.': 'jdownloader', 'captcha.': 'jdownloader',
  'bandwidth.': 'bandwidth', 'rename.': 'rename',
  'jellyfin.': 'media-server', 'library.': 'media-server',
  'quality.': 'quality', 'integrity.': 'quality',
  'kids.': 'kids', 'paths.': 'kids',
  'scheduler.': 'scheduler', 'postprocess.': 'scheduler',
  'telegram.': 'telegram', 'backup.': 'backup',
};
function tabForKey(key: string): string | null {
  if (KEY_TAB_OVERRIDES[key]) return KEY_TAB_OVERRIDES[key];
  const prefix = Object.keys(PREFIX_TAB).find(p => key.startsWith(p));
  return prefix ? PREFIX_TAB[prefix] : null;
}
...
// Tabs that currently hold an invalid field → red dot in the sidebar.
const errorTabs = computed<Set<string>>(() => {
  const tabs = new Set<string>();
  for (const key of Object.keys(errors.value)) {
    const t = tabForKey(key);
    if (t) tabs.add(t);
  }
  return tabs;
});

// One-line hint for the SaveBar, e.g. "Fehler in: Telegram" / "… +1".
const errorHint = computed<string | null>(() => {
  const tabs = [...errorTabs.value];
  if (tabs.length === 0) return null;
  const first = tabLabel(tabs[0]);
  return tabs.length > 1 ? `Fehler in: ${first} +${tabs.length - 1}` : `Fehler in: ${first}`;
});

function jumpToFirstError() {
  const key = Object.keys(errors.value)[0];
  const tab = key ? tabForKey(key) : null;
...
// Decoupled save: only CHANGED keys go over the wire (delta), and invalid
// fields are skipped instead of blocking everything — a typo in Telegram no
// longer holds Quality/JDownloader changes hostage. Skipped fields stay
// marked + unsaved (SaveBar remains), so nothing is silently lost.
// The delta also keeps the backend's restart triggers honest: a full-object
// save used to match every `updates['telegram.*']`-style check and restarted
// Telegram/bandwidth/backup/scheduler on EVERY save click.
async function saveAll() {
  const changed = changedKeys.value;
  const invalid = changed.filter(k => errors.value[k]);
  const delta: Record<string, string> = {};
  for (const k of changed) {
    if (!errors.value[k] && settings.value[k] !== undefined) delta[k] = settings.value[k];
  }

  if (Object.keys(delta).length === 0) {
    if (invalid.length > 0) {
      toast.value?.add(`Ungültiges Feld in „${tabLabel(tabForKey(invalid[0]) || '')}" — bitte korrigieren`, 'warning');
      jumpToFirstError();
    }
    return;
  }
  ...
  saving.value = true;
  try {
    // Pending invalid edits must survive the re-fetch below.
    const pendingInvalid = Object.fromEntries(invalid.map(k => [k, settings.value[k]]));
    await updateSettings(delta);
    await loadSettings(); // fresh snapshot from server (re-masks new secrets)
    Object.assign(settings.value, pendingInvalid);
    if (invalid.length > 0) {
      const tabs = [...new Set(invalid.map(k => tabForKey(k)).filter(Boolean))].map(t => tabLabel(t!));
      toast.value?.add(
        `${Object.keys(delta).length} Änderung${Object.keys(delta).length !== 1 ? 'en' : ''} gespeichert — übersprungen wegen ungültiger Felder: ${tabs.join(', ')}`,
        'warning',
      );
    } else {
...
function discardChanges() {
  if (!savedSettings.value) return;
  try {
    settings.value = JSON.parse(savedSettings.value);
    // Values are back to their saved (valid) state — stale field errors would
    // otherwise keep the tab dots/SaveBar hint alive until the next blur.
    errors.value = {};
  } catch { /* leave as-is */ }
}
...
/* Mobile master-detail back link — hidden on desktop (both panes visible). */
.sx-back { display: none; }
/* The fixed SaveBar only appears while editing; reserve its clearance only then,
   so an idle page doesn't show a phantom empty block at the bottom. */
.sx-editing .sx-content { padding-bottom: 96px; }
.sx-content-header {
  margin-bottom: 26px;
  display: flex;
...
@media (max-width: 900px) {
  ...
  .sx-content { padding: 20px 16px 28px; }
  .sx-title { font-size: 22px; }

  /* Master-detail: menu and panel are separate screens. Tapping a section
     reveals the panel (hides the menu); the back link returns to the menu —
     no scrolling past the whole menu to reach the content. */
  .sx-grid:not(.mobile-detail) .sx-content { display: none; }
  .sx-grid.mobile-detail .sx-side { display: none; }
  .sx-back {
    display: inline-flex;
    align-items: center;
    gap: 6px;
  ...
  .sx-back:active { background: var(--surface-3); }
}
