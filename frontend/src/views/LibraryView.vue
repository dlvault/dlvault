<template>
  <div class="lx">
    ...
    <header class="lx-header">
      <h1 class="lx-title">
        Mediathek <span class="serif">{{ tab === 'music' ? musicCount : filteredItems.length }}</span>
      </h1>
      <div class="lx-tools">
    ...
    <!-- `items` holds movies+shows only; music comes from a separate endpoint.
         Gating the whole view (tabs included) on items.length made the Musik tab
         unreachable on a server whose only populated library is music. -->
    <template v-else-if="source !== 'none' && (items.length > 0 || musicCount > 0)">
      ...
      <div class="lx-bar">
        <div class="lx-tabs" role="tablist" aria-label="Mediathek-Typ">
          ...
          <button
            ...
            :class="['lx-tab', { active: tab === 'music' }]"
            @click="tab = 'music'"
          >
            Musik<span class="lx-tab-count">{{ musicCount }}</span>
          </button>
        </div>
      ...
      <!-- Smart-Filter: Facetten + gespeicherte Ansichten -->
      <div v-if="genreOptions.length || qualityOptions.length || yearBounds.min" class="lx-filters">
        <details class="lx-facet" :class="{ on: facetGenres.length }">
          <summary>Genre<span v-if="facetGenres.length" class="n">{{ facetGenres.length }}</span></summary>
          <div class="lx-facet-menu">
            <label v-for="o in genreOptions" :key="o.value" class="lx-facet-opt">
              <input type="checkbox" :checked="facetGenres.includes(o.value)" @change="toggleGenre(o.value)" />
              <span class="v">{{ o.value }}</span><span class="c">{{ o.count }}</span>
            </label>
          </div>
        </details>

        <details v-if="qualityOptions.length" class="lx-facet" :class="{ on: facetQuality.length }">
          <summary>Qualität<span v-if="facetQuality.length" class="n">{{ facetQuality.length }}</span></summary>
          <div class="lx-facet-menu">
            <label v-for="o in qualityOptions" :key="o.value" class="lx-facet-opt">
              <input type="checkbox" :checked="facetQuality.includes(o.value)" @change="toggleQuality(o.value)" />
              <span class="v">{{ o.value }}</span><span class="c">{{ o.count }}</span>
            </label>
          </div>
        </details>

        <details v-if="yearBounds.min" class="lx-facet" :class="{ on: yearMin != null || yearMax != null }">
          <summary>Jahr<span v-if="yearMin != null || yearMax != null" class="n">1</span></summary>
          <div class="lx-facet-menu year">
            <input type="number" class="lx-year" :placeholder="String(yearBounds.min)" :min="yearBounds.min ?? undefined" :max="yearBounds.max ?? undefined"
                   :value="yearMin ?? ''" @change="e => setYear((e.target as HTMLInputElement).value === '' ? null : +(e.target as HTMLInputElement).value, yearMax)" />
            <span class="dash">–</span>
            <input type="number" class="lx-year" :placeholder="String(yearBounds.max)" :min="yearBounds.min ?? undefined" :max="yearBounds.max ?? undefined"
                   :value="yearMax ?? ''" @change="e => setYear(yearMin, (e.target as HTMLInputElement).value === '' ? null : +(e.target as HTMLInputElement).value)" />
          </div>
        </details>

        <span v-if="savedViews.length" class="lx-vsep"></span>
        <span v-if="savedViews.length" class="lx-saved-lbl">Ansichten</span>
        <button v-for="v in savedViews" :key="v.id" type="button"
                class="lx-saved" :class="{ active: activeViewId === v.id }"
                @click="applyView(v)" @contextmenu.prevent="deleteView(v.id)"
                :title="'Klick: anwenden · Rechtsklick: löschen'">{{ v.name }}</button>
        <button v-if="facetCount" type="button" class="lx-addview" @click="onSaveView">＋ Ansicht speichern</button>
      </div>

      <!-- Aktive Filter -->
      <div v-if="facetChips.length" class="lx-chips">
        <span v-for="c in facetChips" :key="c.key" class="lx-chip">
          <span class="lbl">{{ c.label }}</span>{{ c.value }}
          <button type="button" class="x" @click="removeChip(c)" :aria-label="c.label + ' ' + c.value + ' entfernen'">✕</button>
        </span>
        <button type="button" class="lx-reset" @click="resetFacets">Alle zurücksetzen</button>
      </div>

      <!-- Musik-Sammlung: eigener Album-Grid (Jellyfin/Plex-Musikbibliothek) -->
      <MusicLibraryGrid v-if="tab === 'music'" @loaded="(n) => (musicCount = n)" />
    ...
    <!-- No items at all -->
    <div v-if="!loading && source !== 'none' && items.length === 0 && musicCount === 0" class="card">
      <EmptyState
        icon="Search"
...
import MusicLibraryGrid from '../components/library/MusicLibraryGrid.vue';
import { enrichItems, type EnrichedLibraryItem } from '../components/library/libraryItem';
import { useLibraryFacets } from '../composables/useLibraryFacets';
...
const tab = ref<'movie' | 'show' | 'music'>('movie');
const musicCount = ref(0);

// Eager-load the music album count so the "Musik" tab shows a number before
// it's opened. The full album grid loads on demand when the tab is active.
onMounted(async () => {
  try {
    const res = await fetch('/api/library/music');
    if (res.ok) {
      const data = await res.json();
      musicCount.value = data.total ?? (data.items?.length || 0);
    }
  } catch { /* leave at 0 */ }
});
const search = ref('');
...
// The empty state used to test only for 'movie', so the Musik tab (and any
// future type) read "Keine Serien in der Mediathek".
const emptyStateTitle = computed(() => {
  if (tab.value === 'movie') return 'Keine Filme in der Mediathek';
  if (tab.value === 'music') return 'Keine Alben in der Mediathek';
  return 'Keine Serien in der Mediathek';
});
...
const tabItems = computed(() => enrichedItems.value.filter((i) => i.mediaType === tab.value));
const {
  genres: facetGenres, quality: facetQuality, yearMin, yearMax,
  genreOptions, qualityOptions, yearBounds,
  apply: applyFacets, activeCount: facetCount, chips: facetChips, removeChip,
  toggleGenre, toggleQuality, setYear, reset: resetFacets,
  views: savedViews, activeViewId, saveView, applyView, deleteView,
} = useLibraryFacets(tabItems);

function onSaveView() {
  const n = window.prompt('Name der Ansicht?');
  if (n && n.trim()) saveView(n);
}
...
const showFeatured = computed(() =>
  !debouncedSearch.value.trim() && facetCount.value === 0 && sort.value === 'title' && featuredItems.value.length >= 4,
);
...
async function onPanelDelete(pm: { id: number | string; sourceId?: number | string }) {
  // `sourceId` is the media-server item id the panel was opened from. For a
  // title dlvault also tracks, `pm.id` is the dlvault movie id instead — looking
  // that up here found nothing and the delete silently did nothing at all.
  const key = String(pm.sourceId ?? pm.id);
  const item = items.value.find(i => i.id === key);
  if (!item) {
...
/* ───── Smart-Filter ───── */
.lx-filters { display: flex; align-items: center; gap: 9px; flex-wrap: wrap; margin-bottom: 14px; }
.lx-facet { position: relative; }
.lx-facet > summary {
  list-style: none; cursor: pointer; height: 32px; display: inline-flex; align-items: center; gap: 8px;
  padding: 0 12px; border-radius: 999px; background: var(--surface); border: 1px solid var(--line);
  font-size: 12.5px; font-weight: 500; color: var(--text-secondary); user-select: none;
}
.lx-facet > summary::-webkit-details-marker { display: none; }
.lx-facet > summary::after { content: ""; width: 7px; height: 7px; border-right: 1.5px solid var(--text-3); border-bottom: 1.5px solid var(--text-3); transform: rotate(45deg) translateY(-2px); }
.lx-facet[open] > summary { border-color: var(--line-2); color: var(--text-primary); }
.lx-facet.on > summary { border-color: var(--accent); color: var(--text-primary); background: var(--accent-soft); }
.lx-facet .n { font-family: var(--font-mono); font-size: 10px; color: var(--accent); }
.lx-facet-menu {
  position: absolute; top: calc(100% + 6px); left: 0; z-index: 30; min-width: 200px; max-height: 320px; overflow-y: auto;
  background: var(--surface); border: 1px solid var(--line-2); border-radius: var(--r-md);
  box-shadow: var(--shadow-2); padding: 6px;
}
.lx-facet-menu.year { display: flex; align-items: center; gap: 8px; min-width: 0; padding: 10px; }
.lx-facet-menu .dash { color: var(--text-3); }
.lx-year { width: 76px; padding: 7px 9px; background: var(--surface-2); border: 1px solid var(--line); border-radius: var(--r-sm); color: var(--text-primary); font-family: var(--font-sans); font-size: 13px; }
.lx-year:focus { outline: none; border-color: var(--accent); }
.lx-facet-opt { display: flex; align-items: center; gap: 9px; padding: 7px 9px; border-radius: var(--r-sm); font-size: 13px; color: var(--text-secondary); cursor: pointer; }
.lx-facet-opt:hover { background: var(--surface-2); color: var(--text-primary); }
.lx-facet-opt input { accent-color: var(--accent); }
.lx-facet-opt .v { flex: 1; }
.lx-facet-opt .c { font-family: var(--font-mono); font-size: 10px; color: var(--text-3); }

.lx-vsep { width: 1px; height: 20px; background: var(--line-2); margin: 0 3px; }
.lx-saved-lbl { font-family: var(--font-mono); font-size: 10px; letter-spacing: 0.08em; text-transform: uppercase; color: var(--text-3); }
.lx-saved { height: 32px; padding: 0 13px; border-radius: 999px; font-size: 12.5px; font-weight: 500; background: var(--surface-2); border: 1px solid var(--line); color: var(--text-secondary); cursor: pointer; }
.lx-saved:hover { color: var(--text-primary); border-color: var(--line-2); }
.lx-saved.active { background: var(--accent); border-color: var(--accent); color: #1a0d10; font-weight: 600; }
.lx-addview { height: 32px; padding: 0 12px; border-radius: 999px; font-size: 12.5px; font-weight: 500; background: transparent; border: 1px dashed var(--line-2); color: var(--text-3); cursor: pointer; }
.lx-addview:hover { color: var(--text-secondary); border-color: var(--text-3); }

.lx-chips { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 16px; }
.lx-chip { height: 28px; padding: 0 6px 0 11px; border-radius: 999px; display: inline-flex; align-items: center; gap: 8px; font-size: 12px; background: var(--surface-2); border: 1px solid var(--line-2); color: var(--text-primary); }
.lx-chip .lbl { color: var(--text-3); font-family: var(--font-mono); font-size: 10px; letter-spacing: 0.04em; }
.lx-chip .x { width: 16px; height: 16px; border-radius: 50%; background: var(--surface-3); border: none; color: var(--text-3); font-size: 11px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; }
.lx-chip .x:hover { color: var(--text-primary); }
.lx-reset { background: none; border: none; font-size: 12px; color: var(--accent); font-weight: 500; cursor: pointer; }

@media (max-width: 768px) {
  .lx-filters { gap: 7px; }
  .lx-facet-menu { min-width: 180px; }
}
</style>
