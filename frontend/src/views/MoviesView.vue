...
import { displayStatus } from '../composables/useFormatters';
...
const filteredMovies = computed(() => {
  ...
  if (statusFilter.value && statusFilter.value !== 'all') {
    result = result.filter(m => groupKey(m) === statusFilter.value);
  }
...
const STAGE_META: Record<string, { mono: string; color: string }> = {
  ...
  quality_mismatch: { mono: 'WRONG QUALITY',     color: 'var(--stage-quality_mismatch)' },
  no_download:      { mono: 'NO DOWNLOAD',       color: 'var(--stage-no_download)' },
  not_available:    { mono: 'NOT YET AVAILABLE', color: 'var(--stage-not_available)' },
...
const chips = computed(() => {
  ...
  return [
    ...
    { key: 'quality_mismatch', label: 'Keine passende Qualität', count: counts.quality_mismatch || 0,       color: 'var(--stage-quality_mismatch)' },
    { key: 'no_download',      label: 'Kein Download',         count: counts.no_download || 0,              color: 'var(--stage-no_download)' },
    { key: 'not_available',    label: 'Noch nicht verfügbar',  count: counts.not_available || 0,            color: 'var(--stage-not_available)' },
  ];
});
