<template>
  <div class="dl">
    ...
    <template v-else>
      ...
      <div v-else-if="anyPipelineActive" class="dl-hero fade-in">
...
import { Play, Pause, Square, Trash2, AlertTriangle, Boxes, Music } from 'lucide-vue-next';
...
const activeMusicCount = computed(
  () =>
...
const hasJdActive = computed(() => packages.value.length > 0);
...
const musicSpeedBytes = computed(() =>
  musicStore.downloads.reduce((s, d) => {
    const st = (d.status || '').toLowerCase();
    const active = st !== 'completed' && st !== 'error' && st !== 'failed';
    return s + (active ? (d.speedBps || 0) : 0);
  }, 0),
);
