    ...
    <div class="section-header">
      ...
      <div class="section-actions">
        <span
          ...
          :class="{ 'is-on': syncStore.status.schedulerRunning }"
          :title="syncStore.status.schedulerRunning ? 'Scheduler aktiv · automatischer Sync läuft' : 'Scheduler aus · nur manueller Sync'"
        >
          <span class="dot"></span>
          Scheduler
        </span>
...
/* Tiny scheduler heartbeat — same mono eyebrow as .last-sync, just with
   a status dot. Green when running, dim red when off. */
.scheduler-indicator {
  display: inline-flex;
  align-items: center;
  gap: 6px;
...
.scheduler-indicator .dot {
  ...
  border-radius: 50%;
  background: var(--err);
  box-shadow: 0 0 6px rgba(240, 123, 110, 0.45);
  opacity: 0.6;
}
.scheduler-indicator.is-on .dot {
  background: var(--ok);
  box-shadow: 0 0 6px var(--ok);
  opacity: 1;
  animation: schedulerPulse 2.4s ease-in-out infinite;
}
