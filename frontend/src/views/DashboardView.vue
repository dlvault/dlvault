<template>
  <div class="dashboard">
    ...
    <div class="section-header">
      <h1>
        Dashboard <span class="serif">heute</span>
        <span v-if="commitHash" class="version-badge" :title="`Build ${commitHash}`">{{ shortCommit }}</span>
      </h1>
      <div class="section-actions">
        <span v-if="lastSyncRelative" class="last-sync">Letzter Sync · {{ lastSyncRelative }}</span>
        <button class="btn btn-primary" @click="triggerSync" :disabled="syncStore.syncing">
          <RefreshCw :size="14" />
          <span>{{ syncStore.syncing ? 'Sync läuft...' : 'Jetzt synchronisieren' }}</span>
        </button>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <HeroActive
        ...
        :speed-limit-display="speedLimitDisplay"
      ...
      <MiniStats
        :total-movies="syncStore.status.totalMovies"
        :pending="syncStore.status.pending"
        :downloading="syncStore.status.downloading"
        :scheduler-running="syncStore.status.schedulerRunning"
        :week-added="weekAdded"
        :oldest-pending-age="oldestPendingAge"
      />
    </template>
...
import {
  ...
  getUpdateState, startUpdate, updateStreamUrl,
} from '../api';
...
import MiniStats from '../components/dashboard/MiniStats.vue';
...
// Best-effort timestamp for a movie — backend exposes added_at + last_checked_at,
// both can be null on fresh rows. Falling back to '' means localeCompare won't
// crash on null receivers, just buckets timestamp-less rows last.
function movieTs(m: { last_checked_at?: string | null; added_at?: string | null }): string {
  return m.last_checked_at || m.added_at || '';
}
...
const speedLimitDisplay = ref<string | null>(null); // future: read from settings; not surfaced today

const activeIndex = ref(1); // for "X von Y aktiv" — always 1 for now
...
// Idle hero only shows 4
const recentIdleItems = computed(() => recentShelfItems.value.slice(0, 4));
...
const weekAdded = computed(() => bucketLast7Days(['movie_added', 'watchlist_sync']));
const weekCompleted = computed(() => bucketLast7Days(['sent_to_jdownloader', 'release_found']));

const weekDelta = computed(() => {
  if (weekAdded.value === 0 && weekCompleted.value === 0) return null;
  return { added: weekAdded.value, completed: weekCompleted.value };
});
...
const oldestPendingAge = computed(() => {
  ...
  for (const m of items) {
    const a = m.added_at || m.last_checked_at || '';
    const b = oldest.added_at || oldest.last_checked_at || '';
    if (a && (!b || a < b)) oldest = m;
  }
  const ts = oldest.added_at || oldest.last_checked_at;
...
async function loadHealth() {
  try { const res = await getHealthDetailed(); health.value = res.data; }
  catch { /* ignore */ }
}
