...
export interface LibraryItem {
  ...
  addedAt?: string | null;
  /** Real file quality from the media server ('4K' | '1080p' | '720p' | 'SD').
   *  null/absent for shows and items without stream info — the UI then falls
   *  back to dlvault's desired quality. */
...
export interface PanelMovie {
  id: number | string;
  /**
   * The id of the object the panel was OPENED from, when that differs from `id`.
   *
   * Opening a library entry that dlvault also tracks replaces the panel with the
   * DB-backed detail, whose `id` is the dlvault movie id — so the library item's
   * media-server id was lost and every "Löschen" handler (which looks the entry
   * up in its own list) silently found nothing. Same for a downloads package,
   * whose id is a JD uuid.
   */
